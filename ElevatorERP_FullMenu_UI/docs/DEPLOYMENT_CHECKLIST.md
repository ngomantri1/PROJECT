...
>
> Nguyên tắc bắt buộc: **Local commit và push thành công → VPS pull đúng commit → mới build Docker**. Nếu `git push` hoặc `git pull` báo lỗi thì dừng tại đó; build tiếp chỉ tạo lại phiên bản source cũ.

### C1. Đẩy code từ local lên GitHub

- [ ] Chạy trên Windows PowerShell và xem kỹ danh sách file trước khi commit:
...
Trước khi `git add .`, kiểm tra `.env` và `.data/` không nằm trong phần staged. Sau `git push`, kết quả đúng phải có:

- Dòng đẩy nhánh dạng `main -> main` (hoặc `Everything up-to-date` nếu commit đã được đẩy từ trước).
- `git status --short --branch` chỉ còn `## main...origin/main`, không có danh sách file bên dưới.
- Ghi lại mã commit từ `git log -1 --oneline`, ví dụ `629d91e`, để đối chiếu trên VPS.

Không SSH sang VPS khi local vẫn báo `ahead of 'origin/main'` hoặc còn file staged/chưa commit ngoài chủ ý.
...
- [ ] Kiểm tra Git/SSH và kéo source trên VPS:

```bash
cd /opt/elevator-erp
git status --short --branch
ssh -G git@github.com >/dev/null
git pull --ff-only
git log -1 --oneline
```

Mã commit trên VPS phải giống mã commit vừa ghi lại ở C1. Nếu thấy `Bad configuration option`, `Could not read from remote repository`, conflict hoặc commit vẫn cũ thì **không chạy Docker**; sửa Git/SSH theo B3 rồi chạy lại bước này.

- [ ] Chỉ sau khi commit đã đúng, build và khởi động dịch vụ:
...
`nginx` phải được restart sau khi frontend/backend được tạo lại để phân giải địa chỉ container mới, tránh `502 Bad Gateway` hoặc tiếp tục trỏ tới container cũ.

- [ ] Kiểm tra HTTP ngay trên VPS bằng một lệnh riêng:

```bash
curl -fsSI http://127.0.0.1/
```

Kết quả đúng cần có `HTTP/1.1 200 OK`. Không nối thêm lệnh Docker vào cuối URL của `curl`.

- [ ] Nếu HTTP lỗi, xem trạng thái và log trước khi thao tác tiếp:
...
- [ ] Khi kiểm tra từ máy local, mở cửa sổ ẩn danh hoặc nhấn `Ctrl + F5` để tránh cache trình duyệt:
...
Xác nhận pull thành công và commit đúng, sau đó mới chạy:
...
Chỉ tiếp tục build khi `git pull --ff-only` thành công và commit VPS giống commit local đã push.
...
Nếu giao diện vẫn cũ dù commit VPS đã đúng, nhấn `Ctrl + F5`. Chỉ khi đã xác nhận source đúng mà Docker vẫn dùng image cũ mới chạy:
...
Các dòng log Docker như `=>`, `CACHED`, `exporting layers` là kết quả hiển thị, không phải lệnh để copy và chạy lại trong terminal.
...
### E3. Khôi phục website sau khi VPS bị tắt hoặc khởi động lại

> Khi VPS đang tắt hoàn toàn, bật VPS từ trang quản trị của nhà cung cấp trước và chờ SSH hoạt động. Các container đã có `restart: unless-stopped` nên thường tự chạy khi Docker khởi động; các bước dưới đây dùng để kiểm tra và đưa toàn bộ dịch vụ về đúng trạng thái. Không cần `--build` nếu không có code mới.

- [ ] Đăng nhập lại VPS từ Windows PowerShell:

```powershell
ssh root@<IP_SERVER>
```

- [ ] Khuyến nghị chạy lần lượt từng lệnh trong lần kiểm tra đầu tiên để đọc kết quả của từng bước:
...
- `postgres` và `redis` là `healthy`; `backend`, `frontend`, `nginx` là `Up`.
- `curl` trả về `HTTP/1.1 200 OK`.

- [ ] Nếu Docker chưa `active`, bật Docker rồi chạy lại các bước trên:
...
- [ ] Có thể copy/paste cả khối an toàn sau. Dấu `&&` bảo đảm lệnh sau chỉ chạy khi lệnh trước thành công:
...
Không copy/paste các dòng kết quả như `CACHED`, `exporting layers`, `Up` hoặc `HTTP/1.1 200 OK` trở lại terminal.

- [ ] Nếu website chưa hoạt động, xem log:
...
Quy trình này không xóa hoặc ghi đè PostgreSQL, uploads và Data Protection Keys.

### E4. Khởi động lại toàn bộ hệ thống khi VPS vẫn đang chạy
...
### E6. Sao lưu uploads và Data Protection Keys trên VPS
...
### E7. Chuẩn bị HTTPS khi có tên miền
