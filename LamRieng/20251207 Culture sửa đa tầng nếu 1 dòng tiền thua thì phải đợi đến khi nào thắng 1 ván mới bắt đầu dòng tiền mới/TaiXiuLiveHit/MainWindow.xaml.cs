                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                if (abxStr == "bet")
                                {
                                    ...
                                    InsertPendingBetRow(side, amount, accNow);
...
";
        ...
        private GameContext BuildContext()
        {
            return new GameContext
            {
                ...
                UiInsertPendingBet = (side, stake) => Dispatcher.Invoke(() =>
                {
                    InsertPendingBetRow(side, stake);
                }),
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        private void InsertPendingBetRow(string side, long amount, long? accountOverride = null)
        {
            long account = 0;
            if (accountOverride.HasValue)
            {
                account = accountOverride.Value;
            }
            else
            {
                try { account = (long)ParseMoneyOrZero(LblAmount?.Text ?? "0"); }
                catch { account = 0; }
            }
