...
• Ví dụ: 15 = đặt khi còn ~15% thời gian phiên.";
        ...
        private record AppConfig
        {
            ...
            public string TrialUntil { get; set; } = "";
            public int BetStrategyIndex { get; set; } = 1; // mặc định "2) Thế cầu C/L (chẵn/lẻ tự nhập)"
            public string BetSeq { get; set; } = "";       // giá trị ô "CHUỖI CẦU"
            public string BetPatterns { get; set; } = "CC-C;LL-L";  // giá trị ô "CÁC THẾ CẦU"
            public string MoneyStrategy { get; set; } = "MultiChain";// cố định quản lý vốn đa tầng
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        private void SetConfigEditable(bool enabled)
        {
            ...
            {
                CmbBetStrategy.SelectedIndex = DefaultBetStrategyIndex;
                CmbBetStrategy.IsEnabled = false; // cố định, không cho chọn
            }
            if (TxtChuoiCau != null) TxtChuoiCau.IsReadOnly = !enabled;   // KHÓA khi đang chạy
            if (TxtTheCau != null)
            {
                TxtTheCau.IsReadOnly = true; // cố định, không cho sửa
                TxtTheCau.Text = DefaultBetPatterns;
            }

            // Nhóm Quản lý vốn
            if (CmbMoneyStrategy != null)
            {
                ApplyMoneyStrategyToUI(DefaultMoneyStrategyId);
                CmbMoneyStrategy.IsEnabled = false; // cố định quản lý vốn đa tầng
            }
        ...
        private void LoadConfig()
        {
            try
            {
                ...
                // Cố định chiến lược / thế cầu / quản lý vốn theo yêu cầu
                _cfg.BetStrategyIndex = DefaultBetStrategyIndex;
                _cfg.BetPatterns = DefaultBetPatterns;
                _cfg.BetPatternsCL = DefaultBetPatterns;
                _cfg.MoneyStrategy = DefaultMoneyStrategyId;
                ...
                {
                    ApplyMoneyStrategyToUI(DefaultMoneyStrategyId);
                    CmbMoneyStrategy.IsEnabled = false;
                }
        ...
        private async Task SaveConfigAsync()
        {
            ...
            try
            {
                ...
                // Chiến lược / thế cầu / quản lý vốn được cố định, không lưu từ UI
                _cfg.BetStrategyIndex = DefaultBetStrategyIndex;
...
<body><div class='note'>" + System.Net.WebUtility.HtmlEncode(note) + @"</div></body></html>";
  ...
  try{
    ...
    return 'ok';
        ...
        {
            // Cố định quản lý vốn đa tầng, ép về mặc định và không lưu
            if (CmbMoneyStrategy != null)
                ApplyMoneyStrategyToUI(DefaultMoneyStrategyId);
            _cfg.MoneyStrategy = DefaultMoneyStrategyId;
            LoadStakeCsvForCurrentMoneyStrategy();
...
";
        ...
        private void UpdateBetStrategyUi()
        {
            try
            {
                var idx = CmbBetStrategy?.SelectedIndex ?? DefaultBetStrategyIndex;
                if (RowChuoiCau != null)
        ...
        {
            // Cố định chiến lược, luôn ép về mặc định
            if (CmbBetStrategy != null && CmbBetStrategy.SelectedIndex != DefaultBetStrategyIndex)
                CmbBetStrategy.SelectedIndex = DefaultBetStrategyIndex;
            ...
            // Không lưu chiến lược vào config
        }
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        private void CheckCutAndStopIfNeeded()
        {
            ...
            {
                _ = HandleCutProfitAutoRestartAsync(cutProfit);
                return;
            }
        ...
        private async Task HandleCutProfitAutoRestartAsync(double cutProfit)
        {
            if (_cutStopTriggered) return;
            _cutStopTriggered = true;
            var newEpoch = Interlocked.Increment(ref _betEpoch);
            ...
            {
                Log("[CUT] Lỗi khi dừng: " + ex.Message);
            }
            ...
            {
                Log("[CUT] Lỗi khi tự chạy lại: " + ex.Message);
                _cutStopTriggered = false; // cho phép thử lại nếu không start được
            }
        }
    ...
    TextNorm.U(Regex.Replace(raw ?? "", @"[,\s\-]+", "")); // bỏ , khoảng trắng, -
        ...
        private async void TxtTheCau_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            // Luôn ép thế cầu về mặc định, không lưu vào config
            _cfg.BetPatterns = DefaultBetPatterns;
            _cfg.BetPatternsCL = DefaultBetPatterns;
            if (TxtTheCau != null && TxtTheCau.Text != DefaultBetPatterns)
                TxtTheCau.Text = DefaultBetPatterns;
        }
