(function () {
    ...
    function betParseMultiChipToken(raw) {
        const token = String(raw || '').trim().toLowerCase();
        if (!token)
            return '';
        const m = token.match(/^(\d+)(k|m)?$/i);
        if (!m)
            return '';
    ...
    function betParseMultiChipTokenFromNode(el) {
        if (!el)
            return '';
        try {
            const cls = String(el.className || '');
            const m = cls.match(/\bchip_[a-z]_([0-9]+k?|[0-9]+m?)\b/i);
            if (m && m[1])
                return betParseMultiChipToken(m[1]);
        } catch (_) {}
        return '';
    }

    function betMultiChipTokenToAmount(token) {
        const normalized = betParseMultiChipToken(token);
        if (!normalized)
            return 0;
        const m = normalized.match(/^(\d+)(k|m)?$/i);
        if (!m)
            return 0;
        const num = Number(m[1]);
        const suffix = String(m[2] || '').toLowerCase();
    ...
    function betAmountToMultiChipToken(amount) {
    ...
    function betFindMultiRootById(id) {
        const needle = String(id || '').trim();
        if (!needle)
            return null;
        const docs = betCollectDocs();
        let fallback = null;
        const matcher = new RegExp(`^groupMultiple_\\d+_${needle}$`, 'i');
        for (const doc of docs) {
            let nodes = [];
            try {
                nodes = Array.from(doc.querySelectorAll('div[id^="groupMultiple_"]'));
            } catch (_) {}
            for (const el of nodes) {
                const rootId = String((el && el.id) || '').trim();
                if (!matcher.test(rootId))
                    continue;
                if (/_opencard$/i.test(rootId))
                    continue;
                if (betIsInsideOverlay(el))
                    continue;
                if (betIsVisible(el))
                    return el;
                if (!fallback)
    ...
    function betFindMultiTargetByRoot(root, side) {
        ...
        const suffix = s === 'player'
            ? '_lightPlayer'
            : (s === 'banker' ? '_lightBanker' : (s === 'tie' ? '_lightTie' : ''));
        if (!suffix)
            return null;
        const doc = root.ownerDocument || document;
        const el = doc.getElementById(root.id + suffix);
        return el || null;
    }
    ...
    function betFindMultiConfirmByRoot(root) {
        ...
        if (el)
            return el;
        try {
            return root.querySelector('.multiConfirmBtn.mouse_pointer, [id$="_confirm"]');
        } catch (_) {}
        return null;
    }
    ...
    function betFindAncestorById(node, needle, attrs) {
        let cur = node;
            ...
            try {
                ...
                if (cur.getAttribute) {
                    for (const attr of attrs) {
                        if ((cur.getAttribute(attr) || '') === target)
                            return cur;
                    }
                }
            } catch (_) {}
    ...
    function betIsInsideOverlay(node) {
        try {
            if (!node || !node.closest)
                return false;
            return !!node.closest('#__abx_table_overlay_root');
        } catch (_) {}
        return false;
    }

    function betResolveRootFromOverlay(id) {
        try {
    ...
    function betIsTargetInsideRoot(root, target) {
        if (!root || !target)
            return false;
        try {
        ...
        try {
            const rr = root.getBoundingClientRect ? root.getBoundingClientRect() : null;
            const tr = target.getBoundingClientRect ? target.getBoundingClientRect() : null;
            if (!rr || !tr)
                return false;
    ...
    function betTargetMatchesAnyId(target, candidates) {
        if (!target || !candidates || !candidates.length)
            return false;
        for (const needle of candidates) {
            if (betFindAncestorById(target, needle, BET_ID_ATTRS))
                return true;
        }
        return false;
    }
        ...
        for (const doc of docs) {
            ...
            for (const needle of candidates) {
                for (const el of nodes) {
                    if (!el)
                        continue;
                    if (betFindAncestorById(el, needle, BET_ID_ATTRS)) {
                        if (betIsVisible(el))
                            return el;
                        if (!fallback)
                            fallback = el;
    ...
    function betFindTargetByBetcode(root, side) {
        if (!root || !root.querySelector)
            return null;
        const s = betNormalizeSide(side);
        const multiTarget = betFindMultiTargetByRoot(root, s);
        if (multiTarget)
            return multiTarget;
    ...
    function betGetLabelNode(root, code) {
        ...
        if (betIsMultiBetRoot(root)) {
            if (code === '0')
                return betFindMultiTargetByRoot(root, 'player');
            if (code === '1')
                return betFindMultiTargetByRoot(root, 'banker');
            if (code === '2')
                return betFindMultiTargetByRoot(root, 'tie');
        }
    ...
    function betAmountToLabels(amount) {
        ...
        const multi = betAmountToMultiChipToken(n);
        if (multi) {
            labels.push(multi);
            labels.push(multi.toLowerCase());
            labels.push(multi.toUpperCase());
        }
    ...
    function betLabelToAmount(label) {
        ...
        const suffixMatch = raw.match(/^(\d+)(K|M)$/);
        if (suffixMatch) {
            const num = Number(suffixMatch[1]);
            const suffix = suffixMatch[2];
            if (!Number.isFinite(num) || num <= 0)
                return 0;
            return suffix === 'M' ? num * 1000000 : num * 1000;
        }
    ...
    function betFindChipByAmount(amount, doc) {
        const multiToken = betAmountToMultiChipToken(amount);
        if (multiToken) {
            const docs = [];
            if (doc && doc.querySelectorAll)
                docs.push(doc);
            for (const extraDoc of betCollectDocs()) {
                if (extraDoc && !docs.includes(extraDoc))
                    docs.push(extraDoc);
            }
            const multiNeedle = betParseMultiChipToken(multiToken);
            for (const curDoc of docs) {
                ...
                for (const el of nodes) {
                    ...
                    if (!token)
                        continue;
                    if (token === String(multiNeedle || '').toLowerCase() && betIsVisible(el))
                        return el;
                }
            }
        }
    ...
    function betIsChipActive(el) {
        ...
        if (token) {
            const clsSources = [
                ...
                el.firstElementChild && el.firstElementChild.className || ''
            ].filter(Boolean).join(' ');
            return /\b(active|selected|on|checked|focus|focused|current|lF_lN|lL_lT)\b/i.test(clsSources);
        }
    ...
    function betGetSelectedChipAmount(doc) {
        ...
        for (const el of nodes) {
            ...
            const token = betParseMultiChipTokenFromNode(el);
            if (token) {
                const amt = betMultiChipTokenToAmount(token);
    ...
    function betCollectChipValues(doc) {
        ...
        for (const curDoc of docs) {
            ...
            list.push(...(withBtn.length ? withBtn : nodes)
                .map(el => {
                    const token = betParseMultiChipTokenFromNode(el);
                    if (token)
    ...
    function betBuildChipPlan(amount, doc) {
        ...
        const fallback = Array.from(new Set([
            ...Object.keys(CHIP_LABEL_BY_AMOUNT).map(v => Number(v)).filter(v => Number.isFinite(v)),
            10, 20, 50, 100, 500, 1000, 5000, 10000, 20000, 50000
        ]));
    ...
    async function betExecuteMultiAbxStyleAsync(amountValue, root, side, logBetWarn) {
        const s = betNormalizeSide(side);
        if (!amountValue || !root || !s)
            return false;
        const target = betFindMultiTargetByRoot(root, s);
        if (!target) {
            if (typeof logBetWarn === 'function')
                logBetWarn('abx-flow target not found rootId=' + ((root && root.id) || '') + ' side=' + s);
            return false;
        }
        const confirmBtn = betFindMultiConfirmByRoot(root);
        if (!confirmBtn) {
            if (typeof logBetWarn === 'function')
                logBetWarn('abx-flow confirm not found rootId=' + ((root && root.id) || ''));
            return false;
        }
        ...
        const chipToken = betParseMultiChipTokenFromNode(chipNode);
        const chipDoc = chipNode.ownerDocument || document;
        const rootDoc = root.ownerDocument || document;
        try {
            console.log('[BET-WM] abx-flow start', {
                rev: HOME_JS_REV,
                rootId: (root && root.id) || '',
                side: s,
                amount: amountValue,
    ...
    function betEnqueueTask(fn, timeoutMs, meta) {
        try {
            ...
            if (window.__abx_bet_running !== true)
                window.__abx_bet_running = false;
            if (!Number.isFinite(window.__abx_bet_seq))
            ...
            return new Promise(resolve => {
                ...
                try {
                    console.log('[BET-WM][queue] enqueue', {
                        ...
                        seq: item.seq,
                        timeoutMs: item.timeoutMs,
                        size: window.__abx_bet_queue.length
                    });
                } catch (_) {}
                if (window.__abx_bet_running)
                    return;
                const drain = () => {
                    const item = window.__abx_bet_queue.shift();
                    if (!item) {
                        window.__abx_bet_running = false;
                        return;
                    }
    ...
    function betIsAreaActive(root, side) {
        if (!root)
            return false;
        if (betIsMultiBetRoot(root)) {
            const code = side === 'player' ? '0' : (side === 'banker' ? '1' : '2');
            const area = betGetLabelNode(root, code);
            if (!area)
                return false;
            const cls = [
                area.className || '',
                area.parentElement && area.parentElement.className || ''
            ].filter(Boolean).join(' ');
    ...
    function betDispatchClick(el) {
        ...
        try {
            ...
            const fallback = new (doc.defaultView || window).MouseEvent('click', { bubbles: true, cancelable: true, view: doc.defaultView || window });
    ...
    function betDispatchClickOnce(el, mode, opts = {}) {
        ...
        try {
            ...
            if (kind === 'click') {
                ...
                const ev = new win.MouseEvent('click', { bubbles: true, cancelable: true, view: win });
    ...
    function betShowToast(msg, ttlMs) {
        try {
            const id = '__abx_bet_toast';
            let el = document.getElementById(id);
            if (!el) {
                el = document.createElement('div');
                el.id = id;
                el.style.cssText = [
                    'position:fixed', 'top:12px', 'right:12px', 'z-index:2147482000',
                    'max-width:420px', 'background:#081028', 'color:#fefefe',
                    'border:1px solid rgba(255,255,255,0.15)', 'border-radius:10px',
                    'box-shadow:0 10px 26px rgba(0,0,0,.55)', 'padding:10px 12px',
                    'font:13px/1.4 Consolas,monospace', 'pointer-events:none'
                ].join(';');
                document.body.appendChild(el);
            }
            el.textContent = String(msg || '').slice(0, 220);
            if (window.__abx_bet_toast_timer)
                clearTimeout(window.__abx_bet_toast_timer);
            window.__abx_bet_toast_timer = setTimeout(() => {
                try { el.remove(); } catch (_) {}
            }, Math.max(200, Number(ttlMs) || 3000));
        } catch (_) {}
    }
    ...
    window.__cw_bet_debug = function (tableId, side, amount, isVirtual) {
        return window.__cw_bet(tableId, side, amount, isVirtual, true);
    };
    window.__cw_bet_probe = function (tableId, side, amount) {
        const id = String(tableId || '').trim();
        const s = betNormalizeSide(side);
    ...
    window.__abx_hw_pushTablesNow = function () {
        return postVisibleLobbyTables();
    };

    window.__abx_hw_startTablePush = function (intervalMs) {
        try {
            const ms = Math.max(800, Math.floor(+intervalMs || 2500));
            if (window.__abx_hw_table_push_tid) {
                clearInterval(window.__abx_hw_table_push_tid);
                window.__abx_hw_table_push_tid = 0;
            }
            window.__abx_hw_table_push_tid = setInterval(function () {
                try { postVisibleLobbyTables(); } catch (_) {}
            }, ms);
            try { postVisibleLobbyTables(); } catch (_) {}
            return true;
        } catch (_) {
    ...
    window.__abx_hw_stopTablePush = function () {
        try {
            if (window.__abx_hw_table_push_tid) {
                clearInterval(window.__abx_hw_table_push_tid);
                window.__abx_hw_table_push_tid = 0;
    ...
    function isLoggedInFromDOM() {
        ...
        // 3) Thấy nút/khối Đăng xuất hoặc số dư trong vùng header để tránh quét toàn trang
    ...
    function probeIframeOnce() {
        ...
        if (!paths || !paths.length) {
            ...
            // Không probe fallback nếu path này đã fail (tránh lặp 404 + iframe warning)
    ...
    function ensureObserver() {
        if (!canRunAuthLoop())
            ...
            const run = () => {
                ...
                // 2) Chỉ kick auth probe theo nhịp để tránh bão timer khi DOM mutation dày.
        try {
                    const now = Date.now();
    ...
    // ====== Luong cu: scan/click auto mo game live qua bridge frame/top ======
    ...
    // Luong cu: click truc tiep game live trong trang lobby PP (same-origin)
    async function clickBaccNhieuBanInPPLobby(maxWaitMs = 10000) {
        ...
        log('[legacy] Dang tim game live trong lobby PP...');
        const t0 = Date.now();
        while (Date.now() - t0 < maxWaitMs) {
            if (!isOnPPLobby()) {
        ...
        log('[legacy] Khong tim thay game live trong lobby PP (het wait).');
    ...
    async function clickBaccNhieuBanFromHome() {
        ...
        // Luong cu: Home + PP truc tuyen + game live PP
                    try {
                if (typeof updateInfo === 'function')
                updateInfo('[legacy] Dang co gang mo game live (PP) tu trang Home...');
            ...
            // 4) tim card game live (PP)
                if (typeof updateInfo === 'function')
                updateInfo('[legacy] Dang tim game live (PP)...');
        ...
        cand.sort((a, b) => b.score - a.score);
            ...
            if (typeof updateInfo === 'function')
                    updateInfo('[legacy] Khong tim thay game live (PP).');
                return 'no-game';
                  }
            ...
            if (!ok) {
                if (typeof updateInfo === 'function') {
                    const info = [
                        '[legacy] Khong the click vao game live (PP). Nut co the bi khoa hoac trang chan dieu huong.',
                        'URL: ' + (location.href || ''),
                        'Title: ' + (document.title || '')
                    ].join('\\n');
                    updateInfo(info);
            ...
            if (typeof updateInfo === 'function')
                updateInfo('[legacy] Da click vao game live (PP).');

            return 'ok';
        } catch (e) {
            try { console && console.warn && console.warn('[HomeWatch][legacy] clickBaccNhieuBanFromHome error:', e); } catch (_) {}
    ...
    if (!window.__abx_bacc_guard_reset) {
    ...
    }    // Luong cu: auto retry trong lobby PP, cu thay lobby thi thu click game live
      if (!window.__abx_bacc_lobby_retry) {
        ...
        return;
                  ...
                  try { updateInfo && updateInfo('[legacy-auto] Dang thu click game live trong lobby...'); } catch (_) {}
    ...
    function rememberRoomDom(name, node) {
        ...
        function ensurePinState(roomId, shouldPin) {
            ...
            if (!root) {
                ...
                // Không gọi scrollCardIntoView ở nhánh not-found:
                // hàm này sẽ quét DOM lặp lại vô ích khi card chưa tồn tại, gây lag lúc mới vào.
        ...
        function setBetStats(id, winAmount, winCount, lossCount, outcome) {
            const tableId = String(id || '').trim();
            if (!tableId)
                return false;
            ...
            const outcomeText = String(outcome || '').trim().toLowerCase();
            ...
            if (st) {
                st.betStats = stats;
                if (stats) {
                    const prevWins = Number.isFinite(st.lastBetStatsWinCount) ? st.lastBetStatsWinCount : (st.winCount || 0);
                    const prevLosses = Number.isFinite(st.lastBetStatsLossCount) ? st.lastBetStatsLossCount : (st.lossCount || 0);
                    const nextWins = Math.max(0, stats.winCount || 0);
                    const nextLosses = Math.max(0, stats.lossCount || 0);

                    st.winCount = nextWins;
                    st.lossCount = nextLosses;
                    st.lastBetStatsWinCount = nextWins;
                    st.lastBetStatsLossCount = nextLosses;
                    st.lastBetStatsSig = String(nextWins) + '|' + String(nextLosses) + '|' + String(stats.winAmount || 0) + '|' + String(stats.outcome || '');
                    applyWinLossTotals(st);

                    if (st.view && st.view.winAmountValue) {
                        const winText = formatWinAmountValue(stats.winAmount);
                        st.view.winAmountValue.textContent = winText;
                        st.lastWinAmountText = winText;
                    }

                    const isReset = nextWins === 0 && nextLosses === 0 && (prevWins > 0 || prevLosses > 0);
                    let outcome = stats.outcome || '';
                    if (!isReset) {
                        if (!outcome) {
                            if (nextWins > prevWins)
                                outcome = 'win';
                    ...
                    st.lastBetOutcome = outcome;
                    st.lastBetOutcomeSig = st.lastBetStatsSig;
                    ...
                    if (st.view && st.view.winLoseValue) {
                        if (outcome === 'win') {
                            ...
                            st.view.winLoseValue.style.color = '#ef4444';
...
function deriveWinLoseColor(text) {
        ...
        function deriveBetDoorColor(value) {
            ...
            if (raw === 'BANKER' || raw === 'B')
                return '#ef4444';
            if (raw === 'TIE' || raw === 'T' || raw === "HA'A" || raw.includes('TIE') || raw.includes('HOA'))
                return '#22c55e';
            if (lowered.includes('hoa') || lowered.includes('tie'))
                return '#f59e0b';
            return '';
        ...
        function resetWinLossTotals(st) {
            ...
            st.lossCount = 0;
            st.lastOutcomeSig = '';
            st.lastBetStatsWinCount = 0;
            st.lastBetStatsLossCount = 0;
            st.lastBetOutcome = '';
            st.lastBetOutcomeSig = '';
            applyWinLossTotals(st);
        }
        ...
        function renderPanelState(data) {
            ...
            const applyStickyText = (el, value, lastKey) => {
                ...
                if (hasValue) {
                    if (st[lastKey] !== raw) {
                        el.textContent = raw;
                        st[lastKey] = raw;
                    } else if (el.textContent !== raw) {
                        el.textContent = raw;
                    }
                    return;
                }
                if (st[lastKey]) {
                    if (el.textContent !== st[lastKey])
                        el.textContent = st[lastKey];
            ...
            if ((!resolvedWinLoseText || resolvedWinLoseText === '--') && st.lastWinLoseText && st.lastWinLoseText !== '--') {
                ...
                if (st.lastWinLoseText === resolvedWinLoseText && st.lastWinLoseColor)
                    resolvedWinLoseColor = st.lastWinLoseColor;
            }
            if (view.winLoseValue) {
                if (resolvedWinLoseText && resolvedWinLoseText !== '--') {
                    if (st.lastWinLoseText !== resolvedWinLoseText) {
                        view.winLoseValue.textContent = resolvedWinLoseText;
                        st.lastWinLoseText = resolvedWinLoseText;
                    }
                } else if (!st.lastWinLoseText) {
                    const fallback = resolvedWinLoseText || '--';
                    view.winLoseValue.textContent = fallback;
                    st.lastWinLoseText = fallback;
                }
                if (st.lastWinLoseColor !== resolvedWinLoseColor) {
                    view.winLoseValue.style.color = resolvedWinLoseColor || '';
                    st.lastWinLoseColor = resolvedWinLoseColor;
                }
            }
        ...
        function createPanel(room, idx) {
            ...
            const st = {
                ...
                lastBetPlanSig: '',
                lastBetStatsSig: '',
                lastBetStatsWinCount: 0,
                lastBetStatsLossCount: 0,
                lastBetOutcome: '',
                lastBetOutcomeSig: '',
                winCount: 0,
                lossCount: 0,
