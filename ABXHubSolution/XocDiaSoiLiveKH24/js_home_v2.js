...
;
    ...
    // node có đang hiển thị không
    function isNodeVisible(n) {
        return !!(n && n.activeInHierarchy !== false);
    }

    // popup login đã bật chưa?
    function isLoginPopupVisible() {
        var popupBtn = findNodeByTail('MainGame/Canvas/loginView/offset-login/btn-Login');
        return isNodeVisible(popupBtn);
    }

    // nút login ở header có đang hiển thị không?
    function isHeaderLoginVisible() {
        var headerBtn = findNodeByTail('MainGame/Canvas/widget-header/group-login/btn-Login');
        return isNodeVisible(headerBtn);
    }

    // hàm public để gọi từ C# / console
    window.cwEnsureLoginPopup = async function () {
        try {
            // 1) nếu popup đã mở rồi thì thôi
            if (isLoginPopupVisible()) {
                console.log('[cwLogin] popup login đã mở sẵn → không click nữa');
                return 'already_open';
            }

            // 2) nếu nút header login không còn → có thể đã đăng nhập
            if (!isHeaderLoginVisible()) {
                console.log('[cwLogin] header login không hiển thị → có thể đã đăng nhập → không click');
                return 'login_button_hidden';
            }

            // 3) click nút header login
            var headerBtn = findNodeByTail('MainGame/Canvas/widget-header/group-login/btn-Login');
            if (!headerBtn) {
                console.warn('[cwLogin] không tìm thấy nút header login để click');
                return 'not_found';
            }

            if (typeof emitClick === 'function') {
                emitClick(headerBtn);
            } else {
                // fallback cực kỳ đơn giản
                headerBtn.emit && headerBtn.emit('click');
            }

            // đợi 1 chút cho popup vẽ ra
            await sleep(200);

            // 4) kiểm tra lại lần nữa xem popup có hiện thật chưa
            if (isLoginPopupVisible()) {
                console.log('[cwLogin] đã click và popup xuất hiện');
                return 'opened';
            } else {
                console.warn('[cwLogin] đã click nhưng chưa thấy popup (có thể bị chặn bởi layer khác)');
                return 'clicked_but_not_open';
            }
        } catch (e) {
            console.warn('[cwLogin] error', e);
            return 'error';
        }
    };
