(() => {
    ...
    const RESULT_BOARD_ROOTS = [
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/ld_bg/box_ketqua'
    ];
    ...
    function findNodeByExactPath(path) {
        try {
    ...
    function collectBeadsFast() {
        const out = [];
        const seen = new Set();
        for (let r = 0; r < RESULT_BOARD_ROOTS.length; r++) {
            const root = findNodeByExactPath(RESULT_BOARD_ROOTS[r]);
            if (!root) continue;
            walkFromNode(root, node => {
                if (!nodeActive(node) || seen.has(node)) return;
                seen.add(node);
                const path = fullPath(node, 180);
                const comps = node._components || [];
                for (let i = 0; i < comps.length; i++) {
    ...
    function scoreExactCandidate(items) {
        if (!items || !items.length) return [];
        const uniq = Array.from(new Set(items));
        const prefix = fullPath(findNodeByExactPath(RESULT_BOARD_ROOTS[0]) || findNodeByExactPath(RESULT_BOARD_ROOTS[1]) || getScene(), 180);
    ...
    function scanCandidates() {
        const fastBeads = collectBeadsFast();
        if (fastBeads.length >= 24) {
            const exact = scoreExactCandidate(fastBeads);
            if (exact.length) return exact;
        }
        const beads = collectBeadsWide();
