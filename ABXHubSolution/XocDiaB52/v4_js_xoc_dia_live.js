(() => {
    ...
    var COUNTDOWN_TAILS = [
        // B52 / MainXocDia scene. These are intentionally suffixes; the
        // fallback scanner below still handles renamed parents.
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/lblTimeWait',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/lblTime',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/lbTime',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/Time/lblTime',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/TimeNode/lblTime',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/CountDownNode',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/CountdownNode',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/countDown/lblTime',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/waittingRoom/lblTimeWait',
        'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/waitingRoom/lblTimeWait'
    ];
    function readCountdownSec() {
        ...
        function pathMatchesTail(pathL, tailL) {
            if (!pathL || !tailL)
                return false;
            return pathL === tailL || pathL.endsWith('/' + tailL) || pathL.endsWith(tailL);
        }
        function exactTailHit(pathL) {
            var tails = COUNTDOWN_TAILS || [];
            for (var i = 0; i < tails.length; i++) {
                var tailL = String(tails[i] || '').toLowerCase();
                if (pathMatchesTail(pathL, tailL))
                    return true;
            }
            return false;
        }
        function badCountdownPath(pathL) {
            return (
                pathL.indexOf('minigamenode') !== -1 ||
                pathL.indexOf('jackpotsnode') !== -1 ||
                pathL.indexOf('jackpot') !== -1 ||
                pathL.indexOf('chat') !== -1 ||
                pathL.indexOf('broadcast') !== -1 ||
                pathL.indexOf('roomscene') !== -1 ||
                pathL.indexOf('avatar') !== -1 ||
                pathL.indexOf('playername') !== -1 ||
                pathL.indexOf('thongke') !== -1 ||
                pathL.indexOf('soicau') !== -1 ||
                pathL.indexOf('history') !== -1 ||
                pathL.indexOf('ketqua') !== -1 ||
                pathL.indexOf('result') !== -1 ||
                pathL.indexOf('totalbet') !== -1 ||
                pathL.indexOf('money') !== -1 ||
                pathL.indexOf('chip') !== -1
            );
        }
        function scoreCountdownCandidate(n, path, sec) {
            var pathL = String(path || '').toLowerCase();
            if (badCountdownPath(pathL))
                return -9999;
            if (pathL.indexOf('mainxocdia') === -1 || pathL.indexOf('xocdiaviewmodel') === -1)
                return -9999;

            var score = 0;
            if (pathL.indexOf('mainxocdia') !== -1)
                score += 30;
            if (pathL.indexOf('xocdiaviewmodel') !== -1)
                score += 35;
            if (pathL.indexOf('mainuiparent') !== -1)
                score += 10;
            if (pathL.indexOf('minigamenode') !== -1)
                score += 8;
            if (/(countdown|count_down|countdownnode|count|timer|time|thoigian|thoi_gian|wait|waiting|waitting|lbltime|lbtime)/.test(pathL))
                score += 80;
            if (/(lbl|label|lb)[^/]*(time|count|wait)/.test(pathL))
                score += 25;

            try {
                if (S && S._progTail && path === S._progTail)
                    score += 40;
            } catch (_) {}

            try {
                var r = wRect(n);
                var cx = _num(r.x, 0) + _num(r.w, 0) * 0.5;
                var cy = _num(r.y, 0) + _num(r.h, 0) * 0.5;
                var iw = Math.max(1, innerWidth || 1);
                var ih = Math.max(1, innerHeight || 1);
                var dx = Math.abs((cx / iw) - 0.5);
                if (dx < 0.12)
                    score += 25;
                else if (dx < 0.25)
                    score += 10;
                if (cy > ih * 0.10 && cy < ih * 0.62)
                    score += 10;
            } catch (_) {}
        ...
        walkNodes(function (n) {
            ...
            if (sec == null || sec > 60)
                return;
            var path = fullPath(n, 140);
            var score = scoreCountdownCandidate(n, path, sec);
            if (score < 40)
                return;
            var item = {
                sec: sec,
    ...
    var XD_COUNTDOWN_BAR_MAX_SEC = 120;
    var XD_COUNTDOWN_BAR_MIN_SEC = 4;
    var XD_COUNTDOWN_BAR_LOCK_STALE_MS = 2500;
    var XD_COUNTDOWN_BAR_ZERO_HOLD_MS = 30000;
    var XD_COUNTDOWN_BAR_READY_CONFIDENCE = 3;
    var XD_COUNTDOWN_BAR_STATE = {
        map: {},
        key: '',
        manualKey: '',
        last: null,
    ...
    function getNodeScreenRect(n) {
        try {
            var r = wRect(n);
            var x = (r && r.sx != null) ? Number(r.sx) : Number(r && r.x);
            var y = (r && r.sy != null) ? Number(r.sy) : Number(r && r.y);
            var w = (r && r.sw != null) ? Number(r.sw) : Number(r && r.w);
            var h = (r && r.sh != null) ? Number(r.sh) : Number(r && r.h);
    ...
    function componentName(c) {
        try {
            return String((c && (c.__classname__ || c.name || (c.constructor && c.constructor.name))) || '');
        } catch (_) {
            return '';
        }
    }
    function nodeVisible(n) {
        try {
            if (!n)
                return false;
            if (n.active === false || n.activeInHierarchy === false || n._activeInHierarchy === false)
                return false;
            if (typeof n.opacity !== 'undefined' && Number(n.opacity) <= 3)
                return false;
        } catch (_) {}
        return true;
    }
    function xocDiaCountdownPathAllowed(pathL) {
        if (!pathL)
            return false;
        if (pathL.indexOf('mainxocdia') === -1)
            return false;
        if (pathL.indexOf('xocdiaviewmodel') === -1)
            return false;
        if (pathL.indexOf('countdownstartgame') === -1)
            return false;
        if (pathL.indexOf('minigamenode') !== -1 ||
            pathL.indexOf('jackpot') !== -1 ||
            pathL.indexOf('chat') !== -1 ||
            ...
            pathL.indexOf('result') !== -1 ||
            pathL.indexOf('ketqua') !== -1 ||
            pathL.indexOf('listlabel') !== -1 ||
            pathL.indexOf('totall') !== -1 ||
            pathL.indexOf('totalc') !== -1 ||
            pathL.indexOf('totalle') !== -1 ||
            pathL.indexOf('totalchan') !== -1 ||
            pathL.indexOf('/hud/') !== -1 ||
            pathL.indexOf('/icexit') !== -1 ||
            pathL.indexOf('textwin') !== -1 ||
            pathL.indexOf('bgmd5') !== -1 ||
            pathL.indexOf('btndatlai') !== -1 ||
            pathL.indexOf('muccuoclb') !== -1)
            return false;
        return true;
    }
    function barRectScore(rect) {
        if (!rect)
            return -999;
        var iw = Math.max(1, Number(innerWidth) || 1);
        var ih = Math.max(1, Number(innerHeight) || 1);
        if (rect.w < 35 || rect.h < 3 || rect.w > iw * 0.75 || rect.h > ih * 0.20)
            return -999;
        var score = 0;
        ...
        if (ny > 0.12 && ny < 0.72)
            score += 20;
        if (rect.w >= 100 && rect.w <= 520)
            score += 15;
        if (rect.h >= 6 && rect.h <= 45)
            score += 10;
        return score;
    }
    function addCountdownBarCandidate(out, n, source, ratio, extraScore, extra) {
        ratio = Number(ratio);
        if (!isFinite(ratio))
            return;
        ...
        if (ratio > 1.02)
            return;
        ratio = clamp01(ratio);
        var path = fullPath(n, 160);
        var pathL = String(path || '').toLowerCase();
        if (!xocDiaCountdownPathAllowed(pathL))
            return;
        if (!nodeVisible(n))
            return;
        var rect = getNodeScreenRect(n);
        var score = barRectScore(rect);
        if (score < 0)
            return;
        if (pathL.indexOf('countdownstartgame') !== -1)
            score += 140;
        if (pathL.indexOf('countdownprogress') !== -1)
            score += 140;
        if (/\/countdownprogress$/.test(pathL))
            score += 220;
        if (/\/countdownprogress\/bar$/.test(pathL))
            score += 180;
        if (pathL.indexOf('dealerxocdia') !== -1)
            score += 80;
        if (pathL.indexOf('cardealerxocdia') !== -1)
            score += 80;
        if (pathL.indexOf('count') !== -1 || pathL.indexOf('time') !== -1 || pathL.indexOf('wait') !== -1 || pathL.indexOf('progress') !== -1 || pathL.indexOf('bar') !== -1)
            score += 45;
        if (pathL.indexOf('bet') !== -1 || pathL.indexOf('cuoc') !== -1 || pathL.indexOf('dat') !== -1)
            score += 18;
        if (source === 'progressbar')
            score += 80;
        else if (source === 'sprite_fill')
            score += 65;
        else if (source === 'width_ratio')
            score += 35;
        else if (source === 'scale_x')
            score += 20;
    ...
    function readProgressBarRatio(c) {
        try {
            if (c && typeof c.progress !== 'undefined') {
                var v = Number(c.progress);
                if (isFinite(v))
                    return v;
            }
        } catch (_) {}
    ...
    function readSpriteFillRatio(c) {
        try {
            if (!c)
                return null;
            var v = null;
            if (typeof c.fillRange !== 'undefined')
                v = Number(c.fillRange);
            else if (typeof c._fillRange !== 'undefined')
                v = Number(c._fillRange);
            if (!isFinite(v))
                return null;
            return Math.abs(v);
        } catch (_) {
    ...
    function readScaleXRatio(n) {
        try {
            var v = null;
            if (typeof n.scaleX !== 'undefined')
                v = Number(n.scaleX);
            else if (n.scale && typeof n.scale.x !== 'undefined')
                v = Number(n.scale.x);
            if (!isFinite(v))
                return null;
            v = Math.abs(v);
    ...
    function readWidthRatio(n, rect) {
        try {
            if (!rect || rect.w < 10)
                return null;
            var best = null;
            var p = n.parent || n._parent || null;
            for (var hop = 0; p && hop < 5; hop++, p = p.parent || p._parent || null) {
                if (!nodeVisible(p))
                    continue;
                var pr = getNodeScreenRect(p);
                if (!pr || pr.w <= rect.w + 12 || pr.w < 60)
                    continue;
                var dy = Math.abs(pr.cy - rect.cy);
                if (dy > Math.max(35, pr.h * 1.4))
                    continue;
                var ratio = rect.w / Math.max(1, pr.w);
                if (ratio <= 0 || ratio > 1.02)
                    continue;
                var sc = 20 - hop * 2;
                if (pr.w >= 100 && pr.w <= 600)
                    sc += 10;
                if (!best || sc > best.score)
                    best = {
                        ratio: ratio,
                        score: sc,
                        parentTail: fullPath(p, 160)
                    };
            }
            return best;
        } catch (_) {
    ...
    function collectXocDiaCountdownBarCandidates() {
        var out = [];
        walkNodes(function (n) {
            if (!n || !nodeVisible(n))
                return;
            ...
            if (!xocDiaCountdownPathAllowed(pathL))
                return;
            var rect = getNodeScreenRect(n);
            if (barRectScore(rect) < 0)
                return;

            var pb = null;
            try {
                if (cc && cc.ProgressBar)
                    pb = getComp(n, cc.ProgressBar);
            } catch (_) {}
            var pbr = readProgressBarRatio(pb);
            if (pbr != null)
                addCountdownBarCandidate(out, n, 'progressbar', pbr, 0, {
                    comp: 'cc.ProgressBar'
                });
            ...
            try {
                if (cc && cc.Sprite)
                    sp = getComp(n, cc.Sprite);
            } catch (_) {}
            var sfr = readSpriteFillRatio(sp);
            if (sfr != null)
                addCountdownBarCandidate(out, n, 'sprite_fill', sfr, 0, {
                    comp: 'cc.Sprite'
                });
            ...
            for (var i = 0; i < comps.length; i++) {
                var c = comps[i];
                var cn = componentName(c).toLowerCase();
                var cr = readProgressBarRatio(c);
                if (cr != null && cn.indexOf('progress') !== -1)
                    addCountdownBarCandidate(out, n, 'progressbar', cr, 8, {
                        comp: componentName(c)
                    });
                var fr = readSpriteFillRatio(c);
                if (fr != null && (cn.indexOf('sprite') !== -1 || typeof c.fillRange !== 'undefined' || typeof c._fillRange !== 'undefined'))
                    addCountdownBarCandidate(out, n, 'sprite_fill', fr, 5, {
                        comp: componentName(c)
                    });
            }

            var wr = readWidthRatio(n, rect);
            if (wr)
                addCountdownBarCandidate(out, n, 'width_ratio', wr.ratio, wr.score, {
                    parentTail: wr.parentTail
                });

            var sx = readScaleXRatio(n);
            if (sx != null)
                addCountdownBarCandidate(out, n, 'scale_x', sx, 0, null);
        });
    ...
    function updateCountdownBarState(c, now) {
        var map = XD_COUNTDOWN_BAR_STATE.map || (XD_COUNTDOWN_BAR_STATE.map = {});
        var st = map[c.key] || {
            key: c.key,
            tail: c.tail,
            source: c.source,
        ...
        st.tail = c.tail;
        st.source = c.source;
        st.score = Math.max(Number(st.score) || 0, Number(c.score) || 0);
        st.lastSeen = now;
        st.currentRatio = c.ratio;
        st.rect = c.rect;

        if (st.lastRatio != null && st.lastTs) {
            var dtSec = (now - st.lastTs) / 1000;
            var deltaDown = Number(st.lastRatio) - Number(c.ratio);
            if (dtSec > 0.05 && dtSec < 3.5 && deltaDown > 0.0012) {
                var dur = dtSec / deltaDown;
                if (dur >= XD_COUNTDOWN_BAR_MIN_SEC && dur <= XD_COUNTDOWN_BAR_MAX_SEC) {
                    if (st.durationSec == null)
                        st.durationSec = dur;
                    else
                        st.durationSec = st.durationSec * 0.72 + dur * 0.28;
                    st.confidence = Math.min(20, (Number(st.confidence) || 0) + 1);
                    st.secLeft = clamp01(c.ratio) * st.durationSec;
                    st.direction = 'down';
                }
            } else if (Number(c.ratio) > Number(st.lastRatio) + 0.12) {
                st.roundResetTs = now;
                st.secLeft = (st.durationSec != null) ? clamp01(c.ratio) * st.durationSec : null;
            } else if (st.durationSec != null) {
                st.secLeft = clamp01(c.ratio) * st.durationSec;
            }
        }

        st.lastRatio = c.ratio;
        st.lastTs = now;
        map[c.key] = st;
        return st;
    }
    function findCountdownBarCandidate(candidates, key) {
        if (!key)
            return null;
        for (var i = 0; i < candidates.length; i++) {
            if (candidates[i] && candidates[i].key === key)
                return candidates[i];
        }
        return null;
    }
    function countdownBarStateReady(st) {
        return !!(st && st.durationSec != null && st.confidence >= XD_COUNTDOWN_BAR_READY_CONFIDENCE && st.secLeft != null);
    }
    function countdownBarSecFromState(st) {
        if (!st)
            return null;
        var ratio = clamp01(Number(st.currentRatio) || 0);
        var sec = Number(st.secLeft);
        if (!isFinite(sec) && st.durationSec != null)
            sec = ratio * Number(st.durationSec);
        if (!isFinite(sec))
            return null;
        if (ratio <= 0.025 || sec <= 0.8)
            sec = 0;
        return Math.max(0, Math.min(XD_COUNTDOWN_BAR_MAX_SEC, sec));
    }
    function commitCountdownBarState(st, locked) {
        var sec = countdownBarSecFromState(st);
        if (sec == null)
            return null;
        XD_COUNTDOWN_BAR_STATE.key = st.key;
        XD_COUNTDOWN_BAR_STATE.last = st;
        if (sec <= 0.0001) {
            XD_COUNTDOWN_BAR_STATE.zeroKey = st.key || '';
            XD_COUNTDOWN_BAR_STATE.zeroTail = st.tail || '';
            XD_COUNTDOWN_BAR_STATE.zeroSince = Date.now();
        } else if (!XD_COUNTDOWN_BAR_STATE.zeroKey || XD_COUNTDOWN_BAR_STATE.zeroKey === st.key) {
            XD_COUNTDOWN_BAR_STATE.zeroKey = '';
            XD_COUNTDOWN_BAR_STATE.zeroTail = '';
            XD_COUNTDOWN_BAR_STATE.zeroSince = 0;
        }
        try {
            S._progTail = st.tail || '';
            S._progIsSec = true;
            window.__cw_prog_tail = S._progTail;
            window.__cw_prog_dbg = {
                source: 'xocdia_countdown_bar',
                sec: sec,
                ratio: st.currentRatio,
            ...
            window.__cw_countdown_bar_dbg = window.__cw_prog_dbg;
        } catch (_) {}
        return sec;
    }
    function hasFreshCountdownBarLock() {
        try {
            var key = XD_COUNTDOWN_BAR_STATE.manualKey || XD_COUNTDOWN_BAR_STATE.key || '';
            if (!key)
                return false;
            var st = XD_COUNTDOWN_BAR_STATE.map && XD_COUNTDOWN_BAR_STATE.map[key];
            if (!st)
                return false;
            return (Date.now() - (Number(st.lastSeen) || 0)) <= XD_COUNTDOWN_BAR_LOCK_STALE_MS;
        } catch (_) {
            return false;
        }
    }
    function readXocDiaCountdownBarSec() {
        var now = Date.now();
        var candidates = collectXocDiaCountdownBarCandidates();
        for (var i = 0; i < candidates.length; i++) {
            updateCountdownBarState(candidates[i], now);
        }

        var map = XD_COUNTDOWN_BAR_STATE.map || {};
        var lockedKey = XD_COUNTDOWN_BAR_STATE.manualKey || XD_COUNTDOWN_BAR_STATE.key || '';
        var zeroKey = XD_COUNTDOWN_BAR_STATE.zeroKey || '';
        var zeroSince = Number(XD_COUNTDOWN_BAR_STATE.zeroSince) || 0;
        if (lockedKey) {
            var lockedCand = findCountdownBarCandidate(candidates, lockedKey);
            var lockedState = map[lockedKey] || null;
            if (lockedCand && countdownBarStateReady(lockedState)) {
                return commitCountdownBarState(lockedState, true);
            }
            if (lockedState) {
                var staleMs = now - (Number(lockedState.lastSeen) || 0);
                var lastSec = countdownBarSecFromState(lockedState);
                if (staleMs <= XD_COUNTDOWN_BAR_LOCK_STALE_MS && lastSec != null && lastSec <= 1.5)
                    return commitCountdownBarState(lockedState, true);
                if (zeroKey && zeroKey === lockedKey && zeroSince > 0 && (now - zeroSince) <= XD_COUNTDOWN_BAR_ZERO_HOLD_MS) {
                    if (lockedCand && Number(lockedState.currentRatio) >= 0.08 && lastSec != null && lastSec > 1.2) {
                        XD_COUNTDOWN_BAR_STATE.zeroKey = '';
                        XD_COUNTDOWN_BAR_STATE.zeroTail = '';
                        XD_COUNTDOWN_BAR_STATE.zeroSince = 0;
                        return commitCountdownBarState(lockedState, true);
                ...
                if (staleMs <= XD_COUNTDOWN_BAR_LOCK_STALE_MS && XD_COUNTDOWN_BAR_STATE.manualKey && countdownBarStateReady(lockedState))
                    return commitCountdownBarState(lockedState, true);
            }
            if (XD_COUNTDOWN_BAR_STATE.manualKey) {
                try {
                    window.__cw_countdown_bar_dbg = {
                        ready: false,
                        locked: true,
                        manualKey: XD_COUNTDOWN_BAR_STATE.manualKey,
            ...
            if (zeroKey && zeroKey === lockedKey && zeroSince > 0 && (now - zeroSince) <= XD_COUNTDOWN_BAR_ZERO_HOLD_MS)
                return 0;
            if (lockedState && (now - (Number(lockedState.lastSeen) || 0)) <= XD_COUNTDOWN_BAR_LOCK_STALE_MS)
                return null;
            XD_COUNTDOWN_BAR_STATE.key = '';
        }

        if (zeroKey && zeroSince > 0 && (now - zeroSince) <= XD_COUNTDOWN_BAR_ZERO_HOLD_MS) {
            var zeroState = map[zeroKey] || null;
            var zeroCand = findCountdownBarCandidate(candidates, zeroKey);
            if (zeroCand && zeroState && countdownBarStateReady(zeroState) && Number(zeroState.currentRatio) >= 0.08) {
                XD_COUNTDOWN_BAR_STATE.key = zeroKey;
                XD_COUNTDOWN_BAR_STATE.zeroKey = '';
                XD_COUNTDOWN_BAR_STATE.zeroTail = '';
                XD_COUNTDOWN_BAR_STATE.zeroSince = 0;
                return commitCountdownBarState(zeroState, true);
            }
            try {
                window.__cw_countdown_bar_dbg = {
                    ready: false,
                    zeroHold: true,
                    zeroKey: zeroKey,
                    zeroTail: XD_COUNTDOWN_BAR_STATE.zeroTail || '',
                    sinceMs: now - zeroSince,
                    candidates: candidates.slice(0, 12),
                    state: XD_COUNTDOWN_BAR_STATE
                };
            } catch (_) {}
            return 0;
        }
        ...
        for (var k = 0; k < candidates.length; k++) {
            var st = map[candidates[k].key];
            if (!countdownBarStateReady(st))
                continue;
            var rank = (Number(st.score) || 0) + (Number(st.confidence) || 0) * 85;
            if (lockedKey && candidates[k].key === lockedKey)
                rank += 400;
            if ((st.durationSec != null) && st.durationSec >= 6 && st.durationSec <= 60)
                rank += 120;
            if ((now - (Number(st.lastSeen) || 0)) <= 600)
                rank += 40;
            if ((st.secLeft != null) && st.secLeft < 0.9)
                rank += 50;
        ...
        return commitCountdownBarState(best.state, false);
    }
    async function scanXocDiaCountdownBar(sampleMs) {
        sampleMs = Number(sampleMs) || 3500;
        if (sampleMs < 800)
            sampleMs = 800;
        if (sampleMs > 15000)
            sampleMs = 15000;
        var a = collectXocDiaCountdownBarCandidates();
        var t0 = Date.now();
        await sleep(sampleMs);
        var b = collectXocDiaCountdownBarCandidates();
        var t1 = Date.now();
        var first = {};
        ...
        for (var j = 0; j < b.length; j++) {
            ...
            if (!y)
                continue;
            var delta = Number(y.ratio) - Number(x.ratio);
            var dur = (delta > 0.001) ? ((t1 - t0) / 1000 / delta) : null;
            var secLeft = (dur != null) ? clamp01(x.ratio) * dur : null;
            rows.push({
                key: x.key,
                source: x.source,
                ratio0: Math.round(y.ratio * 1000) / 1000,
                ratio1: Math.round(x.ratio * 1000) / 1000,
                delta: Math.round(delta * 1000) / 1000,
                durationSec: dur == null ? null : Math.round(dur * 10) / 10,
                secLeft: secLeft == null ? null : Math.round(secLeft * 10) / 10,
                score: x.score,
                rect: x.rect ? [Math.round(x.rect.x), Math.round(x.rect.y), Math.round(x.rect.w), Math.round(x.rect.h)].join(',') : '',
                tail: x.tail,
                extra: x.extra || null
            });
        }
        rows.sort(function (m, n) {
            var md = Math.max(0, Number(m.delta) || 0);
            var nd = Math.max(0, Number(n.delta) || 0);
            var ms = (m.durationSec != null && m.durationSec >= XD_COUNTDOWN_BAR_MIN_SEC && m.durationSec <= XD_COUNTDOWN_BAR_MAX_SEC) ? 1000 : 0;
            var ns = (n.durationSec != null && n.durationSec >= XD_COUNTDOWN_BAR_MIN_SEC && n.durationSec <= XD_COUNTDOWN_BAR_MAX_SEC) ? 1000 : 0;
            return (ns + nd * 500 + (n.score || 0)) - (ms + md * 500 + (m.score || 0));
        });
        var out = {
            sampleMs: t1 - t0,
            before: a.length,
        ...
        try {
            window.__cw_xd_countdown_scan = out;
            console.log('[CW_XD_COUNTDOWN_SCAN]', out);
            if (out.rows && out.rows.length)
                console.table(out.rows.slice(0, 12));
        } catch (_) {}
        return out;
    }
    function drawCountdownBarOverlay(dbg) {
        try {
            var id = '__cw_xd_countdown_overlay';
            var d = document.getElementById(id);
            if (!d) {
                d = document.createElement('div');
                d.id = id;
                d.style.cssText = 'position:fixed;z-index:2147483647;pointer-events:none;border:2px solid #00ff55;background:rgba(0,255,85,.10);color:#fff;font:12px Consolas,monospace;text-shadow:0 1px 2px #000;padding:2px 4px;box-sizing:border-box';
            ...
            d.style.width = Math.max(1, Math.round(r.w)) + 'px';
            d.style.height = Math.max(14, Math.round(r.h)) + 'px';
            d.textContent = 'XD ' + Math.round(Number(dbg.sec) * 10) / 10 + 's ' + (dbg.kind || '');
        } catch (_) {}
    }
    function setXocDiaCountdownBarLock(keyOrIndex) {
        try {
            if (keyOrIndex == null || keyOrIndex === '' || keyOrIndex === false) {
                XD_COUNTDOWN_BAR_STATE.manualKey = '';
                return {
                    ok: true,
                    manualKey: '',
                    note: 'manual lock cleared'
                };
            }
            var candidates = collectXocDiaCountdownBarCandidates();
            var hit = null;
            if (typeof keyOrIndex === 'number') {
                hit = candidates[keyOrIndex] || null;
            } else {
                var s = String(keyOrIndex);
                for (var i = 0; i < candidates.length; i++) {
                    var c = candidates[i];
                    if (!c)
                        continue;
                    if (c.key === s || c.tail === s || c.key.indexOf(s) !== -1 || c.tail.indexOf(s) !== -1) {
                        hit = c;
                        break;
                    }
                }
            ...
            XD_COUNTDOWN_BAR_STATE.manualKey = hit.key;
            XD_COUNTDOWN_BAR_STATE.key = hit.key;
            updateCountdownBarState(hit, Date.now());
            return {
                ok: true,
                manualKey: hit.key,
                tail: hit.tail,
    ...
    async function traceXocDiaCountdownBar(durationMs, intervalMs) {
        durationMs = Number(durationMs) || 22000;
        ...
        while (Date.now() - start <= durationMs) {
            var sec = readXocDiaCountdownBarSec();
            var dbg = null;
            try {
                dbg = window.__cw_prog_dbg || window.__cw_countdown_bar_dbg || null;
            } catch (_) {
                dbg = null;
            }
            drawCountdownBarOverlay(dbg);
            var key = dbg && dbg.key ? String(dbg.key) : '';
            rows.push({
                t: Math.round((Date.now() - start) / 100) / 10,
                sec: sec == null ? null : Math.round(Number(sec) * 10) / 10,
                ratio: dbg && dbg.ratio != null ? Math.round(Number(dbg.ratio) * 1000) / 1000 : null,
                durationSec: dbg && dbg.durationSec != null ? Math.round(Number(dbg.durationSec) * 10) / 10 : null,
                confidence: dbg && dbg.confidence != null ? dbg.confidence : null,
                locked: dbg ? !!dbg.locked : false,
                switched: !!(prevKey && key && key !== prevKey),
                key: key,
                kind: dbg && dbg.kind ? dbg.kind : '',
                rect: dbg && dbg.rect ? [Math.round(dbg.rect.x), Math.round(dbg.rect.y), Math.round(dbg.rect.w), Math.round(dbg.rect.h)].join(',') : '',
                tail: dbg && dbg.tail ? dbg.tail : ''
            });
            if (key)
                prevKey = key;
        ...
        try {
            window.__cw_xd_countdown_trace = rows;
            console.log('[CW_XD_COUNTDOWN_TRACE]', {
                rows: rows.length,
                lock: XD_COUNTDOWN_BAR_STATE.key,
                manualLock: XD_COUNTDOWN_BAR_STATE.manualKey
            });
            console.table(rows);
        } catch (_) {}
        return rows;
    ...
    try {
        window.__cw_scanXocDiaCountdownBar = scanXocDiaCountdownBar;
        window.__cw_traceXocDiaCountdownBar = traceXocDiaCountdownBar;
        window.__cw_setXocDiaCountdownBarLock = setXocDiaCountdownBarLock;
        window.__cw_drawCountdownBarOverlay = function () {
            drawCountdownBarOverlay(window.__cw_prog_dbg || window.__cw_countdown_bar_dbg || null);
        };
        window.__cw_collectXocDiaCountdownBarCandidates = collectXocDiaCountdownBarCandidates;
    } catch (_) {}
    ...
    function collectProgress() {
        var barSec = readXocDiaCountdownBarSec();
        if (barSec != null)
            return barSec;
        S._progIsSec = false;
        S._progTail = '';
    ...
    var TAIL_RESULT_BOARD_ROOT = 'MainXocDia/Canvas/MainUIParent/XocDiaViewModel/ld_bg/box_ketqua';
    ...
    function collectResultBoardBeads() {
        var out = [];
        var root = null;
        try {
            root = findNodeByTailStrictLocal(TAIL_RESULT_BOARD_ROOT);
        } catch (_) {
            root = null;
        }
        if (!root)
        ...
        while (q.length) {
            ...
            for (var i = 0; i < comps.length; i++) {
                ...
                var bx = pickCoord(r.sx, r.x);
                var by = pickCoord(r.sy, r.y);
                if (bx < -10 || bx > innerWidth + 10 || by < -10 || by > innerHeight + 10)
                    continue;
                if ((r.w || 0) < 4 || (r.h || 0) < 4 || (r.w || 0) > 80 || (r.h || 0) > 80)
                    continue;
                out.push({
                    v: v,
                    x: bx,
    ...
    function readTKSeqBeads() {
        var cells = collectResultBoardBeads();
        var which = 'box_ketqua';
        if (!cells.length)
            return {
                seq: '',
                which: null,
    ...
    function refreshSeq55ByProg(p, forceScan) {
        ...
        var B52_TOTAL_MONEY_TAIL_NEEDLE = 'mainxocdia/canvas/mainuiparent/xocdiaviewmodel/ld_bg/listlabel/totalmoney';

        function groupMoneyRowsByY(items, tol) {
            tol = Math.max(10, Number(tol) || 56);
            var rows = [];
            var sorted = (items || []).slice().sort(function (a, b) {
            ...
            for (var i = 0; i < sorted.length; i++) {
                ...
                row.items.push(it);
                row.cy = row.items.reduce(function (s, x) {
                    return s + x.y;
                }, 0) / row.items.length;
            }
        ...
        function readB52TotalsByListLabel() {
            var list = [];
            try {
                var labels = collectLabels();
                for (var i = 0; i < labels.length; i++) {
                    var L = labels[i];
                    var fullL = String(L.fullTl || '').toLowerCase();
                    if (!fullL || fullL.indexOf(B52_TOTAL_MONEY_TAIL_NEEDLE) === -1)
                        continue;
                    if (L.val == null || !isFinite(L.val) || L.val <= 10000)
                        continue;
                    list.push({
                        raw: String(L.text || ''),
                        val: L.val,
            ...
            var rows = groupMoneyRowsByY(list, 56);
            var row2 = null;
            var row4 = null;
            for (var r = 0; r < rows.length; r++) {
                if (!row2 && rows[r].items.length === 2)
                    row2 = rows[r];
                if (!row4 && rows[r].items.length === 4)
                    row4 = rows[r];
            }
            if (!row2 && rows.length) {
                row2 = rows.slice().sort(function (a, b) {
                    return Math.abs(a.items.length - 2) - Math.abs(b.items.length - 2);
                })[0];
            }
            if (!row4 && rows.length) {
                row4 = rows.slice().sort(function (a, b) {
                    return Math.abs(a.items.length - 4) - Math.abs(b.items.length - 4);
                })[0];
            }

            var upper = row2 && row2.items ? row2.items.slice().sort(function (a, b) {
                return a.x - b.x;
            }) : [];
            var lower = row4 && row4.items ? row4.items.slice().sort(function (a, b) {
                return a.x - b.x;
            }) : [];

            if (upper.length > 2)
                upper = [upper[0], upper[upper.length - 1]];
            if (lower.length > 4)
                lower = lower.slice(0, 4);

            if (upper.length < 2 || lower.length < 4)
                return null;
...
/** pick first item by tail (sorted by y,x) */
        ...
        function chanLeDebugLines(list, title) {
            var picked = readB52TotalsByListLabel();
            var rawC = picked && picked.C ? picked.C.raw : '';
            var rawL = picked && picked.L ? picked.L.raw : '';
            var tailC = picked && picked.C ? picked.C.tail : TAIL_CHAN;
            var tailL = picked && picked.L ? picked.L.tail : TAIL_LE;
            if (!rawC) {
            ...
            var lines = ['(' + (title || 'Chan/Le by active totals source') + ')'];
            lines.push("CHAN\t'" + String(rawC || '') + "'\t" + String(moneyOf(rawC)));
            lines.push("LE\t'" + String(rawL || '') + "'\t" + String(moneyOf(rawL)));
            lines.push("tailCHAN\t'" + String(tailC || '') + "'");
            lines.push("tailLE\t'" + String(tailL || '') + "'");
            return lines;
        ...
        window.cwPickChan = function () {
            var picked = readB52TotalsByListLabel();
            var raw = picked && picked.C ? picked.C.raw : readNodeTextByTailListExact([TAIL_CHAN], {
                moneyOnly: true
            });
            return {
                txt: raw || '',
                val: moneyOf(raw),
                tail: picked && picked.C ? picked.C.tail : TAIL_CHAN
            };
        };

        window.cwPickLe = function () {
            var picked = readB52TotalsByListLabel();
            var raw = picked && picked.L ? picked.L.raw : readNodeTextByTailListExact([TAIL_LE], {
                moneyOnly: true
            });
            return {
                txt: raw || '',
                val: moneyOf(raw),
                tail: picked && picked.L ? picked.L.tail : TAIL_LE
            };
        };
    ...
    function totals(S) {
        ...
        var pickedB52 = null;
        try {
            pickedB52 = readB52TotalsByListLabel();
        } catch (_) {
            pickedB52 = null;
        }
        try {
            rawC = pickedB52 && pickedB52.C ? pickedB52.C.raw : readNodeTextByTailListExact([TAIL_CHAN], {
                moneyOnly: true
            });
        } catch (_) {
            rawC = '';
        ...
        try {
            rawL = pickedB52 && pickedB52.L ? pickedB52.L.raw : readNodeTextByTailListExact([TAIL_LE], {
                moneyOnly: true
            });
        } catch (_) {
            rawL = '';
        ...
        try {
            rawTT = pickedB52 && pickedB52.TT ? pickedB52.TT.raw : readNodeTextByTailListExact([TAIL_TUTRANG], {
                moneyOnly: true
            });
        } catch (_) {
            rawTT = '';
        ...
        try {
            rawT3T = pickedB52 && pickedB52.T3T ? pickedB52.T3T.raw : readNodeTextByTailListExact([TAIL_3TRANG], {
                moneyOnly: true
            });
        } catch (_) {
            rawT3T = '';
        ...
        try {
            rawT3D = pickedB52 && pickedB52.T3D ? pickedB52.T3D.raw : readNodeTextByTailListExact([TAIL_3DO], {
                moneyOnly: true
            });
        } catch (_) {
            rawT3D = '';
        ...
        try {
            rawTD = pickedB52 && pickedB52.TD ? pickedB52.TD.raw : readNodeTextByTailListExact([TAIL_TUDO], {
                moneyOnly: true
            });
        } catch (_) {
            rawTD = '';
    ...
    function normalizeStatusText(txt) {
        ...
        if (!txt)
            return '';
        var folded = foldStatusText(txt);
        if (!folded)
            return '';
        if (folded.indexOf('CHO DAT CUOC') !== -1)
            return 'CHỜ ĐẶT CƯỢC';
        if (folded.indexOf('NGUNG DAT CUOC') !== -1 || folded.indexOf('DUNG DAT CUOC') !== -1 || folded.indexOf('HET DAT CUOC') !== -1 || folded.indexOf('KHOA CUOC') !== -1)
            return 'NGỪNG ĐẶT CƯỢC';
        if (folded.indexOf('CHO KET QUA') !== -1 || folded.indexOf('DANG CHO KET QUA') !== -1)
            return 'ĐANG CHỜ KẾT QUẢ';
        if (folded.indexOf('DOI MO BAT') !== -1 || folded.indexOf('MO BAT') !== -1 || folded.indexOf('PHIEN MOI') !== -1)
            return 'ĐỢI MỞ BÁT';
        if (folded.indexOf('KET QUA') !== -1)
            return 'ĐANG CHỜ KẾT QUẢ';
        return txt;
    }
    function statusPathAllowed(pathL) {
        if (!pathL)
            return false;
        if (pathL.indexOf('mainxocdia') === -1)
            return false;
    ...
    function statusRectScore(r) {
        if (!r)
            return -999;
        var iw = Math.max(1, Number(innerWidth) || 1);
        var ih = Math.max(1, Number(innerHeight) || 1);
        if (r.w < 80 || r.h < 12 || r.w > iw * 0.55 || r.h > ih * 0.16)
            return -999;
        var nx = (r.x + r.w * 0.5) / iw;
        var ny = (r.y + r.h * 0.5) / ih;
        var score = 0;
        if (nx > 0.32 && nx < 0.68)
            score += 40;
        if (ny > 0.40 && ny < 0.72)
    ...
    function scanSceneStatusText() {
        var now = Date.now();
        if (S._statusScanAt && (now - S._statusScanAt) <= 180) {
            return {
                text: S._statusScanText || '',
                tail: S._statusTail || '',
                fullTail: S._statusFullTail || '',
                rect: S._statusRect || null,
                cached: true
            };
        }
        var labels = collectLabels();
        ...
        for (var i = 0; i < labels.length; i++) {
            var L = labels[i];
            var raw = String(L && L.text || '').replace(/\s+/g, ' ').trim();
            if (!raw)
                continue;
            if (moneyOf(raw) != null || /^\d+$/.test(raw))
                continue;
            var pathL = String(L.fullTl || L.tl || '').toLowerCase();
            if (!statusPathAllowed(pathL))
                continue;
            var text = normalizeStatusText(raw);
            if (!text)
                continue;
            var folded = foldStatusText(text);
            var rect = {
                x: Number(L.sx != null ? L.sx : L.x) || 0,
                y: Number(L.sy != null ? L.sy : L.y) || 0,
                w: Number(L.sw != null ? L.sw : L.w) || 0,
                h: Number(L.sh != null ? L.sh : L.h) || 0
            };
            var score = statusRectScore(rect);
            if (score < 0)
                continue;
            if (folded.indexOf('CHO DAT CUOC') !== -1)
                score += 240;
            else if (folded.indexOf('NGUNG DAT CUOC') !== -1 || folded.indexOf('DUNG DAT CUOC') !== -1 || folded.indexOf('HET DAT CUOC') !== -1)
                score += 220;
            else if (folded.indexOf('CHO KET QUA') !== -1 || folded.indexOf('DANG CHO KET QUA') !== -1 || folded.indexOf('KET QUA') !== -1)
                score += 210;
            else if (folded.indexOf('DOI MO BAT') !== -1 || folded.indexOf('MO BAT') !== -1 || folded.indexOf('PHIEN MOI') !== -1)
                score += 180;
            if (pathL.indexOf('alert') !== -1 || pathL.indexOf('status') !== -1)
                score += 80;
            if (pathL.indexOf('countdownstartgame') !== -1 || pathL.indexOf('dealerxocdia') !== -1)
                score += 35;
            var hit = {
                text: text,
                tail: L.tail || '',
        ...
        S._statusScanAt = now;
        S._statusScanText = best ? (best.text || '') : '';
        S._statusTail = best ? (best.tail || '') : '';
        S._statusFullTail = best ? (best.fullTail || '') : '';
        S._statusRect = best ? (best.rect || null) : null;
        try {
            window.__cw_status_dbg = best ? {
                text: best.text,
                tail: best.tail,
                fullTail: best.fullTail,
                rect: best.rect,
                score: best.score
            } : {
                text: '',
    ...
    function statusByProg(p) {
        try {
            ...
            if (p == null) {
                ...
                p = Number(p);
                status = (!isFinite(p)) ? 'ĐỢI MỞ BÁT'
                 : (p > 0 ? 'CHỜ ĐẶT CƯỢC' : 'ĐANG CHỜ KẾT QUẢ');
            }
            window.__cw_status_dbg = {
                source: 'prog_only',
                prog: p,
                status: status
            };
        ...
        return (p == null) ? 'ĐỢI MỞ BÁT'
         : ((isFinite(Number(p)) && Number(p) > 0) ? 'CHỜ ĐẶT CƯỢC' : 'ĐANG CHỜ KẾT QUẢ');
    }
