...
using static XocDiaB52.MainWindow;
...
namespace XocDiaB52
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private const string AppLocalDirName = "XocDiaB52"; // đổi thành tên bạn muốn
        // ====== App paths ======
        ...
        private DateTime _lastDevToolsShortcutUtc = DateTime.MinValue;
        ...
        private const double CountdownMaxDefaultSec = 20d;
        private const double CountdownMaxHardCapSec = 120d;
        private double _countdownMaxSec = CountdownMaxDefaultSec;
        private int? _lastCountdownDisplaySec = null;
        private double? _lastCountdownRawSec = null;
        private double? _lastCountdownDisplayRatio = null;
        ...
        private const string DEFAULT_URL = "https://play.b52.bi/"; // URL mặc định bạn muốn
...
- Có thể nhập một phần danh sách (ví dụ chỉ 4DO/4TRANG hoặc SAPDOI/CHAN/LE).";
        ...
        private record AppConfig
        {
            ...
            public string SideRateText { get; set; } = XocDiaB52.Tasks.SideRateParser.DefaultText;
        ...
        private sealed class StrategyTabState : INotifyPropertyChanged
        {
            ...
            public XocDiaB52.Tasks.IBetTask? ActiveTask { get; set; }
        ...
        private const string Wv2ZipResNameX64 = "XocDiaB52.ThirdParty.WebView2Fixed_win-x64.zip";
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        public MainWindow()
        {
            ...
            InitializeComponent();
            RegisterDevToolsShortcutHandlers();
        ...
        private void RegisterDevToolsShortcutHandlers()
        {
            var handler = new KeyEventHandler(DevToolsShortcut_KeyDown);

            // Keep this on the root content so it still works when the window content
            // is embedded into AutoBetHub instead of being shown as a standalone Window.
            if (Content is UIElement root)
            {
                root.AddHandler(Keyboard.PreviewKeyDownEvent, handler, true);
                root.AddHandler(Keyboard.KeyDownEvent, handler, true);
            }

            AddHandler(Keyboard.PreviewKeyDownEvent, handler, true);
            AddHandler(Keyboard.KeyDownEvent, handler, true);

            if (Web != null)
            {
                Web.PreviewKeyDown += DevToolsShortcut_KeyDown;
                Web.KeyDown += DevToolsShortcut_KeyDown;
            }
        }

        private void DevToolsShortcut_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (now - _lastDevToolsShortcutUtc < TimeSpan.FromMilliseconds(250))
                return;
            _lastDevToolsShortcutUtc = now;

            OpenWebDevToolsWindow();
        }
        ...
        private void OpenWebDevToolsWindow()
        {
            try
            {
                ...
                if (core == null)
                {
                    Log("[DevTools] WebView2 not ready.");
                    return;
                }
        ...
        {
            // _appDataDir bạn đã tạo ở Startup: %LOCALAPPDATA%\XocDiaB52
        ...
        private double UpdateCountdownMaxAndGetRatio(double seconds)
        {
        ...
        private int GetStableCountdownDisplaySec(double seconds)
        {
            seconds = Math.Max(0, Math.Min(CountdownMaxHardCapSec, seconds));

            var candidate = (seconds <= 0.0001d)
                ? 0
                : Math.Max(0, (int)Math.Floor(seconds + 1e-9));

            if (_lastCountdownDisplaySec == null)
            {
                _lastCountdownDisplaySec = candidate;
                _lastCountdownRawSec = seconds;
                return candidate;
            }

            var last = _lastCountdownDisplaySec.Value;
            // Bắt đầu vòng mới: countdown nhảy từ 0/thấp lên cao trở lại.
            if ((last <= 1 && candidate >= 3) || seconds >= last + 3)
            {
                _lastCountdownDisplaySec = candidate;
                _lastCountdownRawSec = seconds;
                return candidate;
        ...
        private double GetStableCountdownDisplayRatio(double seconds, double ratio)
        {
            seconds = Math.Max(0, Math.Min(CountdownMaxHardCapSec, seconds));
            ratio = Math.Max(0, Math.Min(1, ratio));

            if (_lastCountdownDisplayRatio == null)
            {
                _lastCountdownDisplayRatio = ratio;
                return ratio;
            }

            var lastSec = _lastCountdownDisplaySec ?? 0;
            var lastRatio = _lastCountdownDisplayRatio.Value;

            // Vòng mới: countdown nhảy lên lại rõ ràng.
            if ((lastSec <= 1 && seconds >= 3) || ratio >= lastRatio + 0.12 || seconds >= lastSec + 3)
            {
                _lastCountdownDisplayRatio = ratio;
                return ratio;
            }
        ...
        private static Brush BuildCountdownFillBrush(double ratio)
        {
            ratio = Math.Max(0, Math.Min(1, ratio));

            var red = Color.FromRgb(0xF0, 0x3A, 0x23);
            var yellow = Color.FromRgb(0xFF, 0xD5, 0x4A);

            if (ratio <= 0.06)
            {
                var solid = new SolidColorBrush(red);
                solid.Freeze();
                return solid;
            }

            var yellowPortion = 0.08 + 0.18 * ratio; // đầu ván vàng nhiều hơn, cuối ván thiên đỏ
            var split = Math.Max(0, Math.Min(0.96, 1.0 - yellowPortion));
            ...
            brush.GradientStops.Add(new GradientStop(red, 0.0));
            brush.GradientStops.Add(new GradientStop(red, split));
            brush.GradientStops.Add(new GradientStop(yellow, 1.0));
            brush.Freeze();
            return brush;
        }

        private void UpdateCountdownBarVisual(double ratio)
        {
            try
            {
                if (Fill != null)
                    Fill.Background = BuildCountdownFillBrush(ratio);
            }
            catch { }
        }
            ...
            try
            {
                ...
                if (TxtSideRatio != null)
                {
                    var sideTxt = string.IsNullOrWhiteSpace(_cfg.SideRateText)
                        ? XocDiaB52.Tasks.SideRateParser.DefaultText
                        : _cfg.SideRateText;
        ...
        {
            return _strategyTabs.Any(t => t.IsRunning && t.ActiveTask is XocDiaB52.Tasks.JackpotMultiSideTask);
        }
                                    ...
                                    if (snap != null)
                                    {
                                        ...
                                        _ = Dispatcher.BeginInvoke(new Action(() =>
                                        {
                                            try
                                            {
                                                ...
                                                {
                                                    var sec = Math.Max(0, Math.Min(CountdownMaxHardCapSec, snap.prog.Value));
                                                    var secInt = GetStableCountdownDisplaySec(sec);
                                                    var ratio = GetStableCountdownDisplayRatio(sec, UpdateCountdownMaxAndGetRatio(sec));
                                                    if (PrgBet != null)
                                                    {
                                                        PrgBet.Minimum = 0;
                                                        PrgBet.Maximum = 1;
                                                    ...
                                                    UpdateCountdownBarVisual(ratio);
                                                    if (LblProg != null) LblProg.Text = $"{secInt}s";
                                                }
                                                else
                                                {
                                                    ResetCountdownDisplayState();
                                                    if (PrgBet != null) PrgBet.Value = 0;
                                                    UpdateCountdownBarVisual(0);
  ...
  try{
    ...
    return 'ok';
        ...
        async void CmbMoneyStrategy_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (!IsAnyTabRunning() || IsActiveTabRunning())
                XocDiaB52.Tasks.MoneyHelper.ResetTempProfitForWinUpLoseKeep();
...
";
        ...
        {
            var def = XocDiaB52.Tasks.SideRateParser.DefaultText;
            if (TxtSideRatio != null)
        ...
        private GameContext BuildContext(StrategyTabState tab, bool useRawWinAmount = false)
        {
            ...
            return new GameContext
            {
                ...
                SideRateText = cfg.SideRateText ?? XocDiaB52.Tasks.SideRateParser.DefaultText,
        ...
        {
            StopAllTasks();
            XocDiaB52.Tasks.TaskUtil.ClearBetCooldown();
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                XocDiaB52.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new XocDiaB52.Tasks.SeqParityFollowTask(),     // 1
                    1 => new XocDiaB52.Tasks.PatternParityTask(),       // 2
                    2 => new XocDiaB52.Tasks.SeqMajorMinorTask(),       // 3
                    3 => new XocDiaB52.Tasks.PatternMajorMinorTask(),   // 4
                    4 => new XocDiaB52.Tasks.SmartPrevTask(),           // 5
                    5 => new XocDiaB52.Tasks.RandomParityTask(),        // 6
                    6 => new XocDiaB52.Tasks.AiStatParityTask(),        // 7
                    7 => new XocDiaB52.Tasks.StateTransitionBiasTask(), // 8
                    8 => new XocDiaB52.Tasks.RunLengthBiasTask(),       // 9
                    9 => new XocDiaB52.Tasks.EnsembleMajorityTask(),    // 10
                    10 => new XocDiaB52.Tasks.TimeSlicedHedgeTask(),    // 11
                    11 => new XocDiaB52.Tasks.KnnSubsequenceTask(),     // 12
                    12 => new XocDiaB52.Tasks.DualScheduleHedgeTask(),  // 13
                    13 => new XocDiaB52.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 14
                    14 => new XocDiaB52.Tasks.AiExpertPanelTask(), // 15
                    15 => new XocDiaB52.Tasks.Top10PatternFollowTask(), // 16
                    16 => new XocDiaB52.Tasks.JackpotMultiSideTask(), // 17
                    17 => new XocDiaB52.Tasks.SeqParityHotBackTask(), // 18
                    _ => new XocDiaB52.Tasks.SmartPrevTask(),
                };
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        // đặt trong MainWindow.xaml.cs (project XocDiaB52)
        ...
        private void UpdateTabWin(StrategyTabState tab, double net, string moneyStrategyId)
        {
            ...
            try
            {
                XocDiaB52.Tasks.MoneyHelper.NotifyTempProfit(moneyStrategyId, net);
            }
            catch { /* ignore */ }
        ...
        private async Task<string> LoadAppJsAsyncFallback()
        {
            try
            {
                ...
                var resName = FindResourceName("v4_js_xoc_dia_live.js")
                              ?? "XocDiaB52.v4_js_xoc_dia_live.js";
        ...
        private async Task<string> LoadHomeJsAsync()
        {
            try
            {
                var resName = FindResourceName("js_home_v2.js")
                              ?? "XocDiaB52.js_home_v2.js"; // fallback tên logic
        ...
        private void SetStatusText(string? status)
        {
            try
            {
                ...
                else
                {
                    ...
                    else if (string.Equals(status, "ĐANG CHỜ KẾT QUẢ", StringComparison.OrdinalIgnoreCase))
    ...
    TextNorm.U(Regex.Replace(raw ?? "", @"[,\s\-]+", "")); // bỏ , khoảng trắng, -
        ...
        private static bool ValidatePatternsCL(string s, out string err)
        {
            ...
            foreach (var raw in rules)
            {
                ...
                // LHS: chỉ B/P/C/L + khoảng trắng; độ dài 1–20 sau khi bỏ khoảng trắng
                ...
                {
                    err = $"Quy tắc {idx}: độ dài <mẫu_quá_khứ> phải 1–20 ký tự B/P.";
                    return false;
                }
        ...
        private bool ValidateInputsForCurrentStrategy()
        {
            ...
            {
                if (!XocDiaB52.Tasks.SideRateParser.TryParse(T(TxtSideRatio), out _, out var err))
                {
        ...
        private void SyncStrategyFieldsToUI()
        {
            ...
            if (idx == 16 && TxtSideRatio != null) TxtSideRatio.Text = _cfg.SideRateText ?? XocDiaB52.Tasks.SideRateParser.DefaultText;
        }
        ...
        private void ShowErrorsForCurrentStrategy()
        {
            ...
            if (idx == 16)
            {
                ...
                bool ok = XocDiaB52.Tasks.SideRateParser.TryParse(s, out _, out var e3);
