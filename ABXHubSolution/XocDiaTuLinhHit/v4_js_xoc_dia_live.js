(() => {
    ...
    function scan200Text() {
        var allTexts = buildTextRects().filter(function (t) {
            return t.w > 0 && t.h > 0 &&
                t.x + t.w >= 0 && t.y + t.h >= 0 &&
                t.x <= innerWidth && t.y <= innerHeight;
        }).sort(function (a, b) {
            return a.y - b.y;
        });
        var texts = allTexts.slice(0, 300)
            .map(function (t) {
                return {
                    text: t.text,
                    x: t.x,
        ...
        console.log('(Text index x300 visible=' + allTexts.length + ')\ttext\tx\ty\tw\th\ttail');
        ...
        var chipRows = [];
        var seenChip = {};
        function addChipRow(source, amount, node, raw) {
            try {
                var hit = node ? clickableOf(node, 8) : null;
                var r = hit ? wRect(hit) : (node ? wRect(node) : raw);
                if (!r)
                    r = { x: 0, y: 0, w: 0, h: 0 };
                var key = source + '|' + amount + '|' + Math.round(r.x) + '|' + Math.round(r.y) + '|' + (node ? tailOf(node, 16) : '');
                if (seenChip[key])
                    return;
                seenChip[key] = 1;
                chipRows.push({
                    ...
                    w: Math.round(r.w || 0),
                    h: Math.round(r.h || 0),
                    clickable: !!(hit && clickable(hit)),
                    node: node ? (node.name || '') : '',
                    tail: node ? tailOf(node, 16) : (raw && raw.tail ? raw.tail : '')
                });
            } catch (e) {}
        }

        try {
            var exact = (typeof getChipRects === 'function') ? getChipRects() : [];
            for (var ci = 0; ci < exact.length; ci++) {
                addChipRow('getChipRects', exact[ci].val, null, exact[ci]);
            }
        } catch (e) {
            console.warn('[Scan200Text] getChipRects error', e);
        }

        try {
            var map = {};
            if (typeof window.cwScanChips === 'function')
                map = window.cwScanChips() || {};
            else if (typeof wideScan === 'function')
                map = wideScan() || {};
            var keys = Object.keys(map).sort(function (a, b) {
                return (+a) - (+b);
            });
            for (var mi = 0; mi < keys.length; mi++) {
                var info = map[keys[mi]] || {};
                addChipRow('cwScanChips', +keys[mi], info.node || info.entry || null, null);
            }
        } catch (e) {
            console.warn('[Scan200Text] cwScanChips error', e);
        }

        chipRows.sort(function (a, b) {
            if ((a.amount || 0) !== (b.amount || 0))
                return (a.amount || 0) - (b.amount || 0);
            return a.y - b.y;
        });
        console.log('(Chip scan from Scan200Text) count=' + chipRows.length + '\tsource\tamount\tx\ty\tw\th\tclickable\tnode\ttail');
        for (var cr = 0; cr < chipRows.length; cr++) {
            var c = chipRows[cr];
            console.log(cr + "\t" + c.source + "\t" + c.amount + "\t" + c.x + "\t" + c.y + "\t" + c.w + "\t" + c.h + "\t" + c.clickable + "\t'" + c.node + "'\t'" + c.tail + "'");
        }
        try {
            console.table(chipRows);
        } catch (e) {
