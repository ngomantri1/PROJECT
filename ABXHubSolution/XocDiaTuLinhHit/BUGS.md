...
- `TaskUtil.PlaceBet`/JS ack hien coi viec gui lenh xuong JS la thanh cong theo nghiep vu van hanh; rui ro con lai la C# co the ghi history/cooldown ngay ca khi game that su khong nhan click.
...
- JS bet hien khong retry/bet lai dua tren tong cuoc; sau khi focus/click flow tra ok thi post `bet` ve C# theo yeu cau "gui lenh xuong JS coi nhu thanh cong".
...
- `Scan200Text` da scan ra phinh qua `cwScanChips`, log thay `Entry_1..Entry_9` voi amount `500..5000000`.
- Phinh 500 da map exact `Entry_1`; `BET_UNIT=500`, stake 500 khong bi normalize len 1000.
- Side tail dat cuoc 7 cua da chot bang `SIDE_TAIL_EXACT[WANT]`, khong fallback regex.
- `emitClick()` cho cua da quay ve internal-only, khong canvas/DOM/mouse; focus phinh dung `focusChipNode()` rieng.
- Canvas Watch co `CW_HIDE_CANVAS_WATCH` de an mac dinh, `window.__cw_show_panel()` va watchdog ep hien panel khi flag cho hien, clear trong `teardown`.
- Countdown/progress HIT da chot doc 1 nguon `HUD/countDownProgress`; `prog` giu dang ratio `0..1`, `progSec` la giay theo round 20s, UI mac dinh 0s khi khong co `progTail`.
...
- Neu muon doi nghiep vu sang strict ack sau nay: parse ket qua `__cw_bet` trong C# va chi set cooldown/lastBet khi JS/game xac nhan thanh cong that; hien tai dang chu y coi send JS la thanh cong.
...
- Doi `prog` tu ratio sang giay se lam sai cac flow con doc ratio; neu can giay thi tiep tuc dung field rieng `progSec` va `DecisionSeconds`.
...
- Neu Canvas Watch bi an: kiem tra `CW_HIDE_CANVAS_WATCH`; neu dang `true` thi doi `false` va reload/reinject, sau do chay console `window.__cw_show_panel && window.__cw_show_panel()`; neu undefined thi reload/reinject JS.
