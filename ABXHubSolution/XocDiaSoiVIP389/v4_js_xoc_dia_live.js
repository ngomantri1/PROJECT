(() => {
    ...
    function __cw_urlScore(ctx) {
        ...
        if (hostL.indexOf('livearena') !== -1)
            sc += 14;
        // Domain mới kiểu xocdialive.<brand> thường không có games.*,
        // cộng điểm để không bị kẹt ở wait_url.
        if (hostL.indexOf('xocdialive') !== -1)
            sc += 6;
        if (hostL.indexOf('vipbet') !== -1)
            sc += 3;
        ...
        var timer = setInterval(function () {
            try {
                ...
                if (sceneStrong && strongSceneHits >= 1)
                    probe.ok = true;
                var canBootByCc = !!(hasCc && probe.ok && (match.ok || sceneStrong));
                // Trang mới có thể không dùng Cocos (không có window.cc):
                // cho phép boot theo URL-match sau vài nhịp chờ ổn định.
                var canBootNoCc = !!(!hasCc && match.ok && tries >= 2);
                if ((canBootByCc || canBootNoCc) && __cw_claimBootOwner(probe)) {
                    clearInterval(timer);
                    window.__cw_waiting_v4 = 0;
                    window.__cw_scene_probe = probe;
                    __cw_emitProbe(
                        canBootNoCc ? 'ready_boot_no_cc' :
                        (sceneStrong && !match.ok ? 'ready_boot_scene' : 'ready_boot'),
                        probe);
                    __cw_boot();
                    return;
                }
    ...
    if (!(window.cc && cc.director && cc.director.getScene)) {
        try {
            __cw_emitProbe('boot_no_cc_continue', __bootProbe || {
                ok: true,
                score: 0,
                urlScore: 0
            });
    ...
    function __cw_hasCcScene() {
        try {
            return !!(window.cc && cc.director && typeof cc.director.getScene === 'function' && cc.director.getScene());
        } catch (_) {
            return false;
        }
    }
    var COUNTDOWN_TAIL_MAIN = 'root/left/bg_countdown/bg_count_down/lbl_countdown';
    var COUNTDOWN_DOM_TAIL_ID = 'span#countdown-time';
    var COUNTDOWN_DOM_TAIL_HINT = 'div#countdown-box';
    function readCountdownSecFromTextMapDom() {
        try {
            if (typeof buildTextRectsDom !== 'function')
                return null;
            ...
            for (var i = 0; i < texts.length; i++) {
                ...
                if (tl.indexOf(COUNTDOWN_DOM_TAIL_ID) === -1 && tl.indexOf(COUNTDOWN_DOM_TAIL_HINT) === -1)
                    continue;
                var score = 0;
                if (tl.indexOf(COUNTDOWN_DOM_TAIL_ID) !== -1)
                    score += 40;
                if (tl.indexOf(COUNTDOWN_DOM_TAIL_HINT) !== -1)
                    score += 18;
                if (tl.indexOf('time-text') !== -1)
                    score += 8;
                var ar = (Number(t.w) || 0) * (Number(t.h) || 0);
                score += Math.min(10, ar / 1200);
                cands.push({
                    sec: sec,
                    score: score,
                    x: Number(t.x) || 0,
    ...
    function readCountdownSecDom() {
        try {
            var byTextMap = readCountdownSecFromTextMapDom();
            if (byTextMap != null)
                return byTextMap;
            var cand = [];
            ...
            function isNoise(el, txt) {
                var sig = (String((el && el.className) || '') + ' ' +
                           String((el && el.id) || '') + ' ' + String(txt || '')).toLowerCase();
                // Loại chip/tiền cược để tránh nhặt 20/50/100
                if (/chip|coin|stake|bet|amount|tai|xiu|chan|le|odds|ratio/.test(sig))
                    return true;
                return false;
            }
            var nodes = document.querySelectorAll(
                '[class*=\"count\" i],[id*=\"count\" i],[class*=\"timer\" i],[id*=\"timer\" i],[class*=\"clock\" i],[id*=\"clock\" i],[data-countdown],[data-timer]'
            );
            var vw = innerWidth || 0, vh = innerHeight || 0;
            for (var i = 0; i < nodes.length; i++) {
                var el = nodes[i];
                if (!el)
                    continue;
                var txt = String(el.textContent || '').trim();
                var sec = parseSec(txt);
                if (sec == null || isNoise(el, txt))
                    continue;
                ...
                if (r.bottom < 0 || r.top > vh || r.right < 0 || r.left > vw)
                    continue;
                var centerBias = Math.abs((r.left + r.width / 2) - vw * 0.65);
                var score = 0;
                if (/count|timer|clock/i.test(String(el.className || '') + ' ' + String(el.id || '')))
                    score += 8;
                if (r.top < vh * 0.45)
                    score += 4;
                score += Math.max(0, 6 - Math.min(6, centerBias / 160));
                cand.push({
                    sec: sec,
                    score: score,
                    x: r.left,
                    y: r.top,
                    tail: (String(el.tagName || '').toLowerCase() + '#' + String(el.id || '') + '.' + String(el.className || '').trim().replace(/\s+/g, '.'))
                });
            }
            if (!cand.length)
                return null;
            cand.sort(function (a, b) { return (b.score - a.score) || (a.y - b.y) || (a.x - b.x); });
            var pick = cand[0];
            S._seenCountdown = true;
            S._progIsSec = true;
            S._progTail = String(pick.tail || '');
    ...
    var _domTextRectsCache = [];
    var _domTextRectsAt = 0;
    function domPathOf(el, limit) {
        limit = limit || 10;
        var parts = [];
        var cur = el;
        var hops = 0;
        while (cur && hops < limit) {
            ...
            var cls = String(cur.className || '').trim().replace(/\s+/g, '.');
            var part = tag;
            if (id)
                part += '#' + id;
            if (cls)
    ...
    function buildTextRectsDom() {
        var now = Date.now();
        if (_domTextRectsCache.length && (now - _domTextRectsAt) <= 320)
            return _domTextRectsCache.slice();
        var out = [];
        var seen = {};
        try {
            var mainRect = pickMainCanvasRect();
            var vw = innerWidth || 0;
            var vh = innerHeight || 0;
            var padX = mainRect ? Math.max(60, Math.round(mainRect.width * 0.16)) : 0;
            var padY = mainRect ? Math.max(40, Math.round(mainRect.height * 0.14)) : 0;
            var SHOW_TEXT = (window.NodeFilter && window.NodeFilter.SHOW_TEXT) ? window.NodeFilter.SHOW_TEXT : 4;
            var walker = document.createTreeWalker(document.body, SHOW_TEXT, null, false);
            var node = null;
            while ((node = walker.nextNode())) {
                var parent = node.parentElement;
                if (!parent)
                    continue;
                if (parent.closest && parent.closest('#__cw_root_allin'))
                    continue;
                var tn = String(parent.tagName || '').toLowerCase();
                if (/^(script|style|noscript|canvas|svg|path)$/i.test(tn))
                    continue;
                var text = String(node.nodeValue == null ? '' : node.nodeValue).replace(/\s+/g, ' ').trim();
                if (!text)
                    continue;
                var cs = null;
                try {
                ...
                for (var i = 0; i < rects.length; i++) {
                    ...
                    if (mainRect) {
                        if (cx < (mainRect.left - padX) || cx > (mainRect.right + padX) ||
                            cy < (mainRect.top - padY) || cy > (mainRect.bottom + padY))
                            continue;
                    }
                    ...
                    var key = x + '|' + y + '|' + w + '|' + h + '|' + text;
                    if (seen[key])
                        continue;
                    seen[key] = 1;
                    var domTail = domPathOf(parent, 10) + '/#text';
                    out.push({
                        idx: out.length + 1,
                        text: text,
                        x: x,
        ...
        _domTextRectsCache = out;
        _domTextRectsAt = now;
        return out.slice();
    }
    function buildTextRects() {
        if (!__cw_hasCcScene())
            return buildTextRectsDom();
        var out = [];
        walkNodes(function (n) {
            if (!nodeInGame(n))
                return;
    ...
    function readTKSeq() {
        ...
        try {
            ...
            return { seq: '', which: 'err', cols: [], cells: [], groups: [], seqGrouped: '' };
        }
    }
        ...
        var TAIL_USER_NAME = 'html/body.game_sedie.landscape/div#root/div#container.container.type7.closegr.on.noReturnUrl.tableStyle2.grouptype0.game108.group1.orgParentID49565.road2list/header.header/div.id_gold_box/div.id_gold_item.id_box/span#userNameBox.username.df/#text';
...
/** pick first item by tail (sorted by y,x) */
    ...
    function tick() {
        ...
        S.status = statusByProg(pNow == null ? null : pNow);
...
;
    (function () {
        ...
        try {
            if (!window.__cw_push_autostarted) {
                window.__cw_push_autostarted = 1;
                window.__cw_startPush(320);
            }
        } catch (_) {}
