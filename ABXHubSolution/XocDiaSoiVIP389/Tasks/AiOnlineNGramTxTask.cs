...
using static XocDiaSoiVIP389.Tasks.TaskUtil;
...
namespace XocDiaSoiVIP389.Tasks
{
    ...
    /// </summary>
    public sealed class AiOnlineNGramTxTask : IBetTask
    {
        public string DisplayName => "28) AI hoc tai cho (n-gram) cho tai xiu";
        public string Id => "ai-online-ngram-tx";
        ...
        public AiOnlineNGramTxTask() : this(null, warmStartSteps: 50, saveImmediatelyAfterWarm: true) { }

        public AiOnlineNGramTxTask(string statePath, int warmStartSteps = 50, bool saveImmediatelyAfterWarm = true)
        {
        ...
        private static string AppLocalDir()
            => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "XocDiaSoiVIP389", "ai");

        private static string GetDefaultStatePath()
        {
            var dir = AppLocalDir();
            Directory.CreateDirectory(dir);
            return Path.Combine(dir, "ngram_state_tx_v2.json");
        }
        ...
        private static string GetParamPath()
        {
            var dir = AppLocalDir();
            Directory.CreateDirectory(dir);
            return Path.Combine(dir, "ngram_params_tx_v2.json");
        }
        ...
        private static string GetStatePathV2()
        {
            var dir = AppLocalDir();
            Directory.CreateDirectory(dir);
            return Path.Combine(dir, "ngram_adaptive_state_tx_v2.json");
        }
        ...
        public async Task RunAsync(GameContext ctx, CancellationToken ct)
        {
            ...
            if (!_warmed)
            {
                ...
                string fullParity = SeqToTxString(initSnap?.seq ?? "");
            ...
            while (true)
            {
                ...
                string preParity = SeqToTxString(preSeq);
                ...
                if (undecidable)
                {
                    finalPick = _rng.NextDouble() < 0.5 ? 'X' : 'T';
                }
                else
                {
                    finalPick = (score >= 0) ? 'X' : 'T';
                }
                ...
                string postParity = SeqToTxString(postSnap?.seq ?? "");
                ...
                {
                    ctx.Log?.Invoke("[AI-NGram] warn: cannot observe actual tx for online update.");
                }
        ...
        private sealed class NGramModel
        {
            ...
            public (double score, double conf, int usedK, int support) ScoreAndConfidence(string tx, int kUseMax, int minSupport)
            {
                if (string.IsNullOrEmpty(tx)) return (0.0, 0.0, 0, 0);

                for (int k = Math.Min(Math.Min(kUseMax, _kMax), tx.Length); k >= 1; k--)
                {
                    int key = EncodeTailBits(tx, k);
                    var tab = _tables[k];
            ...
            public void Update(string preParity, char actual)
            {
                ...
                for (int k = 1; k <= _kMax; k++)
                {
                    ...
                    if (actual == 'X') cnt.c += 1.0; else cnt.l += 1.0;
            ...
            private static int EncodeTailBits(string tx, int k)
            {
                int bits = 0, n = tx.Length;
                for (int i = Math.Max(0, n - k); i < n; i++)
                    bits = (bits << 1) | (tx[i] == 'T' ? 1 : 0); // C=0, L=1
                return bits;
            }
        }
