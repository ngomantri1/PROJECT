...
namespace XocDiaSoiVIP389.Tasks
{
    ...
    /// - Luôn đánh liên tục.
    /// </summary>
    public sealed class KnnSubsequenceTxTask : IBetTask
    {
        public string DisplayName => "24) KNN chuoi con cho tai xiu";
        public string Id => "knn-subseq-tx";
            ...
            for (int k = Math.Min(KMax, n - 1); k >= KMin; k--)
            {
                ...
                for (int i = 0; i <= n - k - 1; i++)
                {
                    ...
                    if (d <= 1)
                    {
                        char next = p[i + k];
                        int w = (d == 0) ? 2 : 1;
                        if (next == 'X') scoreC += w; else if (next == 'T') scoreL += w;
                    }
                }

                if (scoreC + scoreL > 0)
                {
                    if (scoreC > scoreL) return 'X';
                    if (scoreL > scoreC) return 'T';
                    return Opp(p[^1]); // hòa
                }
            }
