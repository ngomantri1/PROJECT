...
{
    public sealed class SeqMajorMinorTxTask : IBetTask
    {
        public string DisplayName => "6) Chuoi cau I/N cho tai xiu";
        public string Id => "seq-ni-tx";

        private static string PickSideByNI(char ni, XocDiaSoiVIP389.CwTotals t)
        {
            // N = đánh cửa có tổng tiền NHIỀU hơn; I = đánh cửa có tổng tiền ÍT hơn
            long x = t?.X ?? 0, y = t?.T ?? 0;
            if (ni == 'N') return (x >= y) ? "XIU" : "TAI";
            return (x < y) ? "XIU" : "TAI";
        }
