...
namespace XocDiaSoiVIP389.Tasks
{
    ...
    /// </summary>
    public sealed class AiStatTxTask : IBetTask
    {
        public string DisplayName => "14) Bam cau T/X theo thong ke AI";
        public string Id => "ai-stat-tx";

        private const int DefaultMaxPatternLen = 6;

        private static string TxCharToSideSafe(char ch) => (ch == 'X') ? "XIU" : "TAI";
        ...
        {
            if (string.IsNullOrWhiteSpace(input)) return ('X', 0);

            var seq = new string(input
                .Where(ch => ch == 'X' || ch == 'c' || ch == 'T' || ch == 'l')
            ...
            for (int k = Math.Min(maxPatternLen, n - 1); k >= 1; k--)
            {
                ...
                {
                    if (cCount > lCount)
                        return ('X', (double)(cCount - lCount) / (cCount + lCount));
                    if (lCount > cCount)
                        return ('T', (double)(lCount - cCount) / (cCount + lCount));
                    ...
                    char last = seq[n - 1];
                    char opposite = (last == 'X') ? 'T' : 'X'; // cầu 1–1
                    return (opposite, 0.0);
                }
            }
        ...
        public async Task RunAsync(GameContext ctx, CancellationToken ct)
        {
            ...
            while (true)
            {
                ...
                string tx = SeqToTxString(baseSeq);

                // Luôn quyết định và vào lệnh — không bỏ nhịp
                var (next, conf) = PredictNextWithConfidence(tx, DefaultMaxPatternLen);
                string side = TxCharToSideSafe(next);
                long stake;
