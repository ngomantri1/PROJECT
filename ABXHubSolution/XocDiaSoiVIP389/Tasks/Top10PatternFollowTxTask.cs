...
namespace XocDiaSoiVIP389.Tasks
{
    ...
    /// </summary>
    public sealed class Top10PatternFollowTxTask : IBetTask
    {
        public string DisplayName => "32) Top10 tich luy (khoi tu 50 T/X)";
        public string Id => "top10-50-tx";

        private const int WindowLen = 10;
        private const int FrameLen = 50; // cửa sổ 50 phiên gần nhất

        // key: chuỗi 10 ký tự 'X'/'T'
        ...
        public async Task RunAsync(GameContext ctx, CancellationToken ct)
        {
            ...
            string clAll0 = SeqToTxString(snap0?.seq ?? "");
            ...
            while (true)
            {
                ...
                if (string.IsNullOrEmpty(curPattern))
                {
                    ...
                    if (string.IsNullOrEmpty(curPattern))
                    {
                        ...
                        char lastCl = SeqToTxString(baseSeq).LastOrDefault();
                        string fallbackSide = (lastCl == 'X') ? "XIU" : "TAI";
                        ...
                        string clAll2 = SeqToTxString(s2?.seq ?? "");
                ...
                string clAll = SeqToTxString(sAfter?.seq ?? "");
