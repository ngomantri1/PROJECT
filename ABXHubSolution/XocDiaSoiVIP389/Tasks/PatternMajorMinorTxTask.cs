...
namespace XocDiaSoiVIP389.Tasks
{
    ...
    /// </summary>
    public sealed class PatternMajorMinorTxTask : IBetTask
    {
        public string DisplayName => "8) The cau I/N cho tai xiu";
        public string Id => "pattern-ni-tx";
        ...
        private static string NOrIToSide(char ch, XocDiaSoiVIP389.CwTotals totals)
        {
            // Chuyển N/I thành TAI/XIU theo tổng tiền "ngay thời điểm đặt"
            long x = totals?.X ?? 0, y = totals?.T ?? 0;
            if (ch == 'N') return (x >= y) ? "XIU" : "TAI"; // hòa coi như N
            return (x < y) ? "XIU" : "TAI";                 // 'I'
        }
