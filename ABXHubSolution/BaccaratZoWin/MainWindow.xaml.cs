...
using static BaccaratZoWin.MainWindow;
...
namespace BaccaratZoWin
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private const string AppLocalDirName = "BaccaratZoWin"; // đổi thành tên bạn muốn
        // ====== App paths ======
        ...
        private readonly object _authorityBetSettleLock = new();
        private readonly List<TaskCompletionSource<AuthorityBetSettleInfo?>> _authorityBetSettleWaiters = new();
        private long _authorityBetSettleSerial = 0;
        private string _authorityBetSettleKey = "";
        private AuthorityBetSettleInfo? _latestAuthorityBetSettle;
        ...
        private int _betWhenRemainingSeconds = 10;
        ...
        private readonly ConcurrentDictionary<ulong, string> _mainFrameHrefCache = new();
        private readonly ConcurrentDictionary<int, string> _popupFrameHrefCache = new();
        ...
        private string _lastProgRxLogKey = "";
        private string _lastProgUiLogKey = "";
        ...
        private const int ProgUiTimerIntervalMs = 50;
        private const double ProgUiBacktrackToleranceSec = 0.18;
        private const double ProgUiResetNearZeroSec = 0.75;
        private const double ProgUiResetAcceptMinSec = 4.0;
        ...
        private static readonly TimeSpan PopupTickPullFallbackMinGap = TimeSpan.FromMilliseconds(900);
        private static readonly TimeSpan MainTickPullFallbackMinGap = TimeSpan.FromMilliseconds(1100);
...
• Ví dụ: 150000 (khi lỗ ≥ 150.000đ thì dừng).";
        ...
        @"ĐẶT KHI CÒN GIÂY
• Nhập số giây còn lại của phiên (1–45), không để trống.
• Ý nghĩa: chỉ đặt cược khi countdown còn lại ≤ giá trị này.
• Nếu nhập sai hoặc để trống, hệ thống sẽ tự đưa về giá trị hợp lệ gần nhất.
• Ví dụ: 10 = chỉ bắt đầu đặt khi còn 10 giây hoặc ít hơn.";

        const string TIP_DECISION_PERCENT_NI =
        @"ĐẶT KHI CÒN GIÂY
• Nhập số giây còn lại của phiên (1–45), KHÔNG phải phần trăm.
• Hệ thống sẽ chờ đến khi countdown còn lại ≤ ngưỡng này mới cho đặt cược.
• Nên để khoảng 8–15 giây nếu muốn vào sát cuối phiên.
• Ví dụ: 12 = chỉ bắt đầu đặt khi còn 12 giây hoặc ít hơn.";
...
• Áp dụng cho cả chuỗi tiền thường và quản lý vốn đa tầng.";
        ...
        @"CỬA ĐẶT & TỈ LỆ
- Logic nhiều cửa đã bị loại bỏ khỏi BaccaratZoWin.
- Trường này chỉ còn để tương thích cấu hình cũ và hiện không dùng trong Baccarat.";
        ...
        private record AppConfig
        {
            ...
            public int BetWhenRemainingSeconds { get; set; } = 10;
            public int BetWhenRemainingPercent { get; set; } = 0;
            ...
            public int PushIntervalMs { get; set; } = CW_PUSH_MS_DEFAULT;
        ...
        private sealed class StrategyTabState : INotifyPropertyChanged
        {
            ...
            public int RunDecisionSeconds { get; set; } = 0;
            public int RunBetWhenRemainingSeconds { get; set; } = 0;
            ...
            public BaccaratZoWin.Tasks.IBetTask? ActiveTask { get; set; }
        ...
        private const int CW_PUSH_MS_LEGACY_DEFAULT = 360;
        private const int CW_PUSH_MS_DEFAULT = 100;
        ...
        private const string Wv2ZipResNameX64 = "BaccaratZoWin.ThirdParty.WebView2Fixed_win-x64.zip";
...
(function(){
  try{
    ...
    function __abxToBridgeString(d){
      try{
        if (d == null) return null;
        if (typeof d === 'string'){
          var t = String(d || '').trim();
          if (!t) return null;
          if (t.indexOf('""abx""') >= 0) return t;
          return null;
        }
        if (typeof d === 'object'){
    ...
    try{
      if (window.chrome && window.chrome.webview && typeof window.chrome.webview.postMessage === 'function'){
        ...
        window.chrome.webview.postMessage = function(p){
          try{
            var s2 = __abxToBridgeString(p);
            if (s2 != null) __orig(s2);
            else __orig(p);
          }
          catch(e){
            try{
              var forwarded = (typeof p === 'string') ? (function(){
                try{
                  var t = String(p || '').trim();
                  if (!t || t.indexOf('""abx""') < 0) return null;
                  return JSON.parse(t);
                }catch(_){ return null; }
              })() : (p && p.abx ? p : null);
              if (forwarded) parent.postMessage(forwarded, '*');
            }catch(_){}
          }
        };
      }
      ...
      window.addEventListener('message', function(ev){
        try{
          ...
          if (window.chrome && window.chrome.webview) {
            ...
            try {
              if (typeof d === 'string') {
                try { parent.postMessage(JSON.parse(s), '*'); } catch(_){}
              } else {
  ...
  try{
    ...
    function __abxCanStartInWindow(w){
      try{
        ...
        if (/\/internal\/livestream_page\//i.test(href) && /bcrlive/i.test(href)) return true;
    ...
    function __abxPushMs(){
      var ms = Number(window.__abx_push_ms || 220);
      if (!(ms >= 180 && ms <= 1000)) ms = 220;
      return ms;
    }
      ...
      try{
        for (var i=0;i<window.frames.length;i++){
          try{
            var w = window.frames[i];
            var href = String((w.location && w.location.href) || '');
            var isExactLiveFrame = /\/internal\/livestream_page\//i.test(href) && /bcrlive/i.test(href);
            var hasModernHint =
              /\/activations\/baccarat(?:\/|\?|$)/i.test(href) ||
              (/(?:^|[?&])selectedgame=baccarat(?:[&#]|$)/i.test(href) &&
               !/(?:^|[?&])application=lobby(?:[&#]|$)/i.test(href)) ||
    ...
    try{
      function isClosed(win, st){
        ...
        return false;
          ...
          try{
            if ((!snap || (!snap.rawSeq && !(snap.totals && snap.totals.A != null) && !(Number(snap.progValid || 0) === 1))) &&
          ...
          try{
            ...
            if (!snap || ((!snap.rawSeq || !String(snap.rawSeq || '').trim()) &&
          ...
          var isZowinHost =
            /\/\/(?:[^\/]+\.)?web\.zowin\.(?:ph|tw|nu|vu)\//i.test(href);
          var isZowinLiveFrame =
            isZowinHost &&
            /\/internal\/livestream_page\//i.test(href) &&
            /bcrlive/i.test(href);
          var isZowinTopShell =
            isZowinHost &&
            !/\/internal\/livestream_page\//i.test(href);
          ...
          var progModeL = String(progMode || '').toLowerCase();
          var progRawNum = (progRaw != null && isFinite(Number(progRaw))) ? Number(progRaw) : null;
          var hasDomProg = hasProg && (progSourceL.indexOf('dom') >= 0 || progSourceL.indexOf('pseudo') >= 0);
          var hasCocosMainProg = hasProg && progSourceL.indexOf('cocos-countdown-main') >= 0;
          var hasActiveSecondsProg =
            (progValid === 1 &&
             progModeL === 'seconds' &&
             progRawNum != null &&
             progRawNum > 0.2);
          var hasZeroSecondsProg =
            (progValid === 1 &&
             progModeL === 'seconds' &&
             progRawNum != null &&
             progRawNum <= 0.2);
          ...
          if (hasActiveSecondsProg) score += 2600;
          else if (hasCocosMainProg) score += 700;
          if (hasZeroSecondsProg) score -= 260;
          if (hasDomProg) score += 450;
          if (hasStatus) score += 120;
          if (hasSeq) score += 100;
          if (hasHud) score += 10;
          if (isZowinLiveFrame) score += 8000;
          if (/c5_bcrlive_withautoplay|c5_bcrlive_withoutoplay|bcrlive_withautoplay|bcrlive_withoutoplay/i.test(href)) score += 2500;
          if (isZowinTopShell && !hasActiveSecondsProg && !hasCocosMainProg) score -= 900;
          ...
          return {
            ...
            sourceHref:href,
            sourceDepth:Number(depth || 0),
            ts:Date.now(),
            __score:score,
            __depth:Number(depth || 0),
            __href:href,
            __hasActiveSecondsProg: hasActiveSecondsProg ? 1 : 0,
            __hasCocosMainProg: hasCocosMainProg ? 1 : 0,
            __progRawNum: progRawNum
          };
        }catch(_){ return null; }
      }
      ...
      for (var ci=0; ci<candidates.length; ci++){
        ...
        var candHref = String(cand.__href || cand.sourceHref || '');
        var bestHref = String(best.__href || best.sourceHref || '');
        var candExactLive = /\/internal\/livestream_page\//i.test(candHref) && /bcrlive/i.test(candHref);
        var bestExactLive = /\/internal\/livestream_page\//i.test(bestHref) && /bcrlive/i.test(bestHref);
        if (candExactLive && !bestExactLive){ best = cand; continue; }
        var candActiveSecondsProg = Number(cand.__hasActiveSecondsProg || 0) === 1;
        var bestActiveSecondsProg = Number(best.__hasActiveSecondsProg || 0) === 1;
        if (candActiveSecondsProg && !bestActiveSecondsProg){ best = cand; continue; }
        if (candActiveSecondsProg === bestActiveSecondsProg){
          var candCocosMainProg = Number(cand.__hasCocosMainProg || 0) === 1;
          var bestCocosMainProg = Number(best.__hasCocosMainProg || 0) === 1;
          if (candCocosMainProg && !bestCocosMainProg){ best = cand; continue; }
          if (candCocosMainProg === bestCocosMainProg){
            var candProgRawNum = (cand.__progRawNum != null && isFinite(Number(cand.__progRawNum))) ? Number(cand.__progRawNum) : null;
            var bestProgRawNum = (best.__progRawNum != null && isFinite(Number(best.__progRawNum))) ? Number(best.__progRawNum) : null;
            if (candActiveSecondsProg && bestActiveSecondsProg &&
                candProgRawNum != null && bestProgRawNum != null &&
                candProgRawNum > (bestProgRawNum + 0.2)){
              best = cand;
              continue;
            }
          }
      ...
      try{ delete best.__hasActiveSecondsProg; }catch(_){}
      try{ delete best.__hasCocosMainProg; }catch(_){}
      try{ delete best.__progRawNum; }catch(_){}
      ...
      return '';
        ...
        private void PostToUi(Action action, System.Windows.Threading.DispatcherPriority priority = System.Windows.Threading.DispatcherPriority.Background)
        {
            if (action == null)
                return;
        ...
        private void QueueBridgeProbe(string stage, int timeoutMs = 1500)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    await LogBridgeProbeAsync(stage).WaitAsync(TimeSpan.FromMilliseconds(timeoutMs)).ConfigureAwait(false);
                }
                catch (TimeoutException)
                {
                    if (_enableJsPushDebug)
                        Log($"[BridgeProbe][TIMEOUT] stage={stage} | timeoutMs={timeoutMs}");
                }
                catch (Exception ex)
                {
                    if (_enableJsPushDebug)
                        Log($"[BridgeProbe][ERR] stage={stage} | err={ex.Message}");
                }
            });
        }
        ...
        private bool ShouldSkipNoisyLog(string msg)
        {
            ...
            if (COMPACT_RUNTIME_LOG && !isErrorLike)
            {
                ...
                if (msg.StartsWith("[NETSEQ][BOOT]", StringComparison.Ordinal) ||
                    msg.StartsWith("[NETSEQ][JS-STALE]", StringComparison.Ordinal) ||
                    msg.StartsWith("[NETSEQ][RAW-AUTHORITY]", StringComparison.Ordinal) ||
                    msg.StartsWith("[NETSEQ][COUNT-ONLY-HOLD]", StringComparison.Ordinal) ||
        ...
        {
            // _appDataDir bạn đã tạo ở Startup: %LOCALAPPDATA%\BaccaratZoWin
        ...
        // Giữ bảng điều khiển luôn tương tác được kể cả khi task đang chạy.
        // Tham số enabled được giữ lại để không phải sửa các callsite cũ.
        private void SetConfigEditable(bool enabled)
        {
            if (CmbBetStrategy != null) CmbBetStrategy.IsEnabled = true;
            if (TxtChuoiCau != null) TxtChuoiCau.IsReadOnly = false;
            if (TxtTheCau != null) TxtTheCau.IsReadOnly = false;
            if (TxtSideRatio != null) TxtSideRatio.IsReadOnly = false;
            if (BtnResetSideRatio != null) BtnResetSideRatio.IsEnabled = true;

            if (CmbMoneyStrategy != null) CmbMoneyStrategy.IsEnabled = true;
            if (ChkAutoResetStakeOnNonNegativeWin != null) ChkAutoResetStakeOnNonNegativeWin.IsEnabled = true;

            if (TxtStakeCsv != null) TxtStakeCsv.IsReadOnly = false;
            if (TxtDecisionSecond != null) TxtDecisionSecond.IsReadOnly = false;
            if (TxtCutProfit != null) TxtCutProfit.IsReadOnly = false;
            if (TxtCutLoss != null) TxtCutLoss.IsReadOnly = false;
        }
        ...
        private static int ClampBetWhenRemainingSeconds(int value) => Math.Clamp(value, 1, 45);
        ...
        private void SyncDecisionSecondsFromUi()
        {
        ...
        private void SyncBetWhenRemainingSecondsFromConfig(AppConfig? cfg)
        {
            var fallback = ClampDecisionSeconds(cfg?.DecisionSeconds ?? 10);
            var raw = cfg?.BetWhenRemainingSeconds ?? 0;
            if (raw <= 0)
            {
                var legacy = cfg?.BetWhenRemainingPercent ?? 0;
                raw = (legacy >= 1 && legacy <= 45) ? legacy : fallback;
            }
            var sec = ClampBetWhenRemainingSeconds(raw);
            _betWhenRemainingSeconds = sec;
            _decisionSeconds = sec;
            if (cfg != null)
            {
                cfg.BetWhenRemainingSeconds = sec;
                cfg.DecisionSeconds = sec;
                cfg.BetWhenRemainingPercent = sec;
            }
        }
        private void SyncBetWhenRemainingSecondsFromUi(bool normalizeText = true)
        {
            var fallback = _cfg?.BetWhenRemainingSeconds ?? _cfg?.DecisionSeconds ?? 10;
            fallback = ClampBetWhenRemainingSeconds(fallback);
            var rawText = T(TxtDecisionSecond, fallback.ToString(CultureInfo.InvariantCulture));
            var parsedOk = int.TryParse(rawText, out var parsed);
            var sec = parsedOk ? ClampBetWhenRemainingSeconds(parsed) : fallback;
            var normalized = !parsedOk || string.IsNullOrWhiteSpace(rawText) || parsed != sec;

            _betWhenRemainingSeconds = sec;
            _decisionSeconds = sec;
            if (_cfg != null)
            {
                _cfg.BetWhenRemainingSeconds = sec;
                _cfg.DecisionSeconds = sec;
                _cfg.BetWhenRemainingPercent = sec;
            }
            ...
            if (normalized)
                Log($"[CFG][BET-WHEN-REMAINING] raw={(string.IsNullOrWhiteSpace(rawText) ? "-" : rawText)} | normalized={sec} | reason=clamp-or-empty");
        }
        ...
        private void LoadConfig()
        {
            try
            {
                ...
                SyncBetWhenRemainingSecondsFromConfig(_cfg);
        ...
        private void ApplyRuntimeProfileFromConfig(bool log = false)
        {
            ...
            int configuredPushMs = _cfg.PushIntervalMs;
            bool migrateLegacyPerformancePush =
                !isDebug &&
                (configuredPushMs == CW_PUSH_MS_LEGACY_DEFAULT ||
                 configuredPushMs == 220);
            int desiredPush = migrateLegacyPerformancePush
                ? CW_PUSH_MS_DEFAULT
                : (configuredPushMs > 0
                    ? configuredPushMs
            ...
            // Tắt hẳn CDP tap cho Zowin hiện tại:
            // log thực tế cho thấy wsR tăng rất mạnh nhưng không parse ra observed/winner packet hữu ích,
            // nên chỉ làm nặng app và không phục vụ luồng bet JS hiện tại.
            _enableCdpNetworkTap = false;
            _enableCdpObservedContextTap = false;
            ...
            if (log)
            {
                if (migrateLegacyPerformancePush)
                    Log($"[RuntimeProfile] migrate legacy pushMs {configuredPushMs} -> {_cwPushMs} | profile={profile}");
  ...
  try {{
    ...
    return 'err:' + (e && e.message ? e.message : String(e));
        ...
        private void EnsureProgUiTimer()
        {
            ...
            {
                Interval = TimeSpan.FromMilliseconds(ProgUiTimerIntervalMs)
            };
        ...
        private static double ComputeCountdownRemainSeconds(double anchorValue, DateTime anchorUtc, DateTime nowUtc)
        {
            var safeAnchorUtc = anchorUtc == DateTime.MinValue ? nowUtc : anchorUtc;
            var elapsedSec = Math.Max(0, (nowUtc - safeAnchorUtc).TotalSeconds);
            return Math.Clamp(anchorValue - elapsedSec, 0, 30);
        }

        private double GetCurrentProgUiSeconds(DateTime nowUtc)
        {
            if (!_progUiHasValue || !_progUiIsSeconds)
                return 0;
            return ComputeCountdownRemainSeconds(_progUiAnchorValue, _progUiAnchorUtc, nowUtc);
        }
        ...
        private void RenderProgUiFromAnchor()
        {
            ...
            if (_progUiIsSeconds)
            {
                var remain = Math.Clamp(_progUiAnchorValue, 0, 30);
                var remainInt = remain <= 0 ? 0 : (int)Math.Floor(remain + 1e-9);
                if (PrgBet != null)
                {
                    PrgBet.Minimum = 0;
                    PrgBet.Maximum = 30;
                ...
                var progUiLogKey = $"seconds|{remainInt}|{remain:0.###}";
                if (!string.Equals(_lastProgUiLogKey, progUiLogKey, StringComparison.Ordinal))
                {
                    _lastProgUiLogKey = progUiLogKey;
                    Log($"[PROG][UI] mode=seconds | raw={remain:0.###} | label={remainInt}s");
                }
                return;
            }
            ...
            if (LblProg != null)
                LblProg.Text = $"{pctInt}%";
            var pctUiLogKey = $"percent|{pctInt}|{pct:0.###}";
            if (!string.Equals(_lastProgUiLogKey, pctUiLogKey, StringComparison.Ordinal))
            {
                _lastProgUiLogKey = pctUiLogKey;
                Log($"[PROG][UI] mode=percent | raw={pct:0.###} | label={pctInt}%");
            }
        }
        ...
        private void ApplyActiveTabToUi()
        {
            ...
            try
            {
                ...
                SyncDecisionSecondsFromConfig(_cfg);
                SyncBetWhenRemainingSecondsFromConfig(_cfg);
                if (TxtDecisionSecond != null) TxtDecisionSecond.Text = _cfg.BetWhenRemainingSeconds.ToString(CultureInfo.InvariantCulture);
        ...
        private void ApplyUiToConfig(AppConfig cfg)
        {
            ...
            var betWhenRemainingSeconds = ClampBetWhenRemainingSeconds(I(T(TxtDecisionSecond, "10"), 10));
            cfg.BetWhenRemainingSeconds = betWhenRemainingSeconds;
            cfg.DecisionSeconds = betWhenRemainingSeconds;
            cfg.BetWhenRemainingPercent = betWhenRemainingSeconds;
            if (ReferenceEquals(cfg, _cfg))
            {
                _betWhenRemainingSeconds = betWhenRemainingSeconds;
                _decisionSeconds = betWhenRemainingSeconds;
            }
...
<body><div class='note'>" + System.Net.WebUtility.HtmlEncode(note) + @"</div></body></html>";
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                UpdateTooltips();             // gắn TIP_* cho Chuỗi/Thế + StakeCsv/Cắt lãi/Cắt lỗ/ngưỡng giây
        ...
        async void CmbMoneyStrategy_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (!IsAnyTabRunning() || IsActiveTabRunning())
                BaccaratZoWin.Tasks.MoneyHelper.ResetTempProfitForWinUpLoseKeep();
        ...
        private void UpdateTooltips()
        {
            ...
            string tip = idx switch
            {
                ...
                16 => "17) Logic nhiều cửa đã bị loại bỏ khỏi BaccaratZoWin.",
        ...
        private async void CoreWebView2_WebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            try
            {
                ...
                await DispatchIncomingWebMessageAsync(
                    msg,
                    ReferenceEquals(sender, _popupWeb?.CoreWebView2) ? "popup-msg" : "main-msg",
                    "WebMessageReceived");
            }
            catch (Exception ex)
            {
        ...
        private static bool ShouldProcessIncomingWebMessageOnBackground(string msg)
        {
            if (string.IsNullOrWhiteSpace(msg))
                return false;

            if (msg.IndexOf("\"abx\":\"result\"", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;
            if (msg.IndexOf("\"abx\":\"native_click\"", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;
            if (msg.IndexOf("\"abx\":\"trusted_click\"", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;

            return true;
        }

        private static bool ShouldIgnoreForeignWindowMessage(string msg, string source)
        {
            if (string.IsNullOrWhiteSpace(msg))
                return true;

            bool isFrameOrMsgSource =
                source.IndexOf("main-frame", StringComparison.OrdinalIgnoreCase) >= 0 ||
                source.IndexOf("popup-frame", StringComparison.OrdinalIgnoreCase) >= 0 ||
                source.IndexOf("main-msg", StringComparison.OrdinalIgnoreCase) >= 0 ||
                source.IndexOf("popup-msg", StringComparison.OrdinalIgnoreCase) >= 0;

            if (!isFrameOrMsgSource)
                return false;

            if (msg.IndexOf("\"abx\"", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;
        ...
        private Task DispatchIncomingWebMessageAsync(string msg, string source, string logTag)
        {
            if (ShouldIgnoreForeignWindowMessage(msg, source))
                return Task.CompletedTask;

            if (!ShouldProcessIncomingWebMessageOnBackground(msg))
                return HandleIncomingWebMessageAsync(msg, source);

            _ = Task.Run(async () =>
            {
                try
                {
                    await HandleIncomingWebMessageAsync(msg, source).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
                    Log($"[{logTag}] " + ex);
        ...
        private async Task HandleIncomingWebMessageAsync(string msg, string source = "direct")
        {
            ...
            try
            {
                ...
                if (abxStr == "tick")
                {
                    ...
                    {
                        var prevSeq = prevUiSnap.rawSeq ?? "";
                        var curSeq = snap.rawSeq ?? "";
                    ...
                    if (snap != null)
                    {
                        ...
                        var boardSeqDisplay = FilterResultDisplaySeq(snap.rawSeq ?? "");
                        var boardSeqVersion = Math.Max(snap.seqVersion ?? 0, boardSeqDisplay.Length);
                        ...
                        if (progValid)
                        {
                            progUi = (string.Equals(progModeUi, "seconds", StringComparison.OrdinalIgnoreCase) && snap.progRaw.HasValue)
                                ? snap.progRaw
                                : snap.prog;
                        }
                        var progRxKey = string.Join("|",
                            source ?? "",
                            progValid ? "1" : "0",
                            progModeUi ?? "",
                            snap.prog?.ToString("0.###", CultureInfo.InvariantCulture) ?? "-",
                            snap.progRaw?.ToString("0.###", CultureInfo.InvariantCulture) ?? "-",
                            GetJsonStringLoose(jrootTick, "origin") ?? "",
                            GetJsonStringLoose(jrootTick, "reason") ?? "",
                            snap.progSource ?? "",
                            snap.progTail ?? "");
                        if (!string.Equals(_lastProgRxLogKey, progRxKey, StringComparison.Ordinal))
                        {
                            _lastProgRxLogKey = progRxKey;
                            var sourceHref = GetJsonStringLoose(jrootTick, "sourceHref") ?? "";
                            var origin = GetJsonStringLoose(jrootTick, "origin") ?? "";
                            var reason = GetJsonStringLoose(jrootTick, "reason") ?? "";
                            Log($"[PROG][RX] src={(string.IsNullOrWhiteSpace(source) ? "-" : source)} | origin={(string.IsNullOrWhiteSpace(origin) ? "-" : origin)} | reason={(string.IsNullOrWhiteSpace(reason) ? "-" : reason)} | valid={(progValid ? 1 : 0)} | mode={(string.IsNullOrWhiteSpace(progModeUi) ? "-" : progModeUi)} | prog={(snap.prog?.ToString("0.###", CultureInfo.InvariantCulture) ?? "-")} | raw={(snap.progRaw?.ToString("0.###", CultureInfo.InvariantCulture) ?? "-")} | ui={(progUi?.ToString("0.###", CultureInfo.InvariantCulture) ?? "-")} | progSrc={(string.IsNullOrWhiteSpace(snap.progSource) ? "-" : Shrink(snap.progSource, 48))} | tail={(string.IsNullOrWhiteSpace(snap.progTail) ? "-" : Shrink(snap.progTail, 96))} | href={(string.IsNullOrWhiteSpace(sourceHref) ? "-" : Shrink(sourceHref, 120))}");
                        }
                        if (prevUiFresh)
                        {
                        ...
                        var seqForUi = FilterResultDisplaySeqWindow(snap.rawSeq ?? "");
                        if (!string.IsNullOrWhiteSpace(snap.rawSeq))
                        {
                            Log($"[SEQ][UI][RAW-DIRECT] src={source} | rawLen={seqForUi.Length} | evt={(string.IsNullOrWhiteSpace(snap.seqEvent) ? "-" : snap.seqEvent)}");
                        }
                    ...
                    if ((_lastGameTickUtc - _lastTickDiagLogUtc) > TimeSpan.FromSeconds(2))
                    {
                        ...
                        var seqLen = snap?.rawSeq?.Length ?? 0;
  ...
  try{
    ...
    return (hasLogoutVis && !hasLoginVis) ? '1' : '0';
            ...
            catch (Exception ex)
            {
                ...
                if (preview.StartsWith("messagetogame://", StringComparison.OrdinalIgnoreCase))
                    return;
        ...
        private void IngestJsConsole(JsonElement root, string source)
        {
            try
            {
                ...
                if (messageLower.Contains("loadbundle: https://") ||
                    messageLower.Contains("checkversion:") ||
                    messageLower.Contains("base: https://staticmt.net/") ||
                    messageLower.Contains("set url tracking:") ||
                    messageLower.Contains("[physics][bullet]: using wasm bullet libs.") ||
                    messageLower.Contains("[physics]: register bullet.") ||
                    messageLower.Contains("[physics]: using bullet."))
                    return;
        ...
        private static CwSnapshot? ParseCwSnapshotLoose(JsonElement root)
        {
            try
            {
                var rawSeq = FilterResultDisplaySeqWindow(GetJsonStringLoose(root, "rawSeq") ?? GetJsonStringLoose(root, "seq") ?? "");
                var snap = new CwSnapshot
                {
                    abx = GetJsonStringLoose(root, "abx") ?? "",
                    prog = GetJsonDoubleLoose(root, "prog"),
                    progSec = (int?)GetJsonLongLoose(root, "progSec"),
                    ...
                    seqSource = GetJsonStringLoose(root, "seqSource") ?? "",
                ...
                if (!snap.progSec.HasValue &&
                    string.Equals(snap.progMode, "seconds", StringComparison.OrdinalIgnoreCase))
                {
                    var secRaw = snap.progRaw ?? snap.prog;
                    if (secRaw.HasValue && double.IsFinite(secRaw.Value))
                        snap.progSec = Math.Clamp((int)Math.Round(secRaw.Value, MidpointRounding.AwayFromZero), 0, 45);
                }
    ...
    Web != null && Web.CoreWebView2 != null;
        ...
        private string GetCurrentBoardSeqForSyncLocked()
        {
            ...
            var syncDisplay = FilterResultDisplaySeqWindow(_boardSeqDisplay);
        ...
        private void ObserveNetworkGameState(string payload, bool isBinary)
        {
            ...
            lock (_roundStateLock)
            {
                ...
                Log($"[NETSEQ][OBS-RESET] reason={resetReason} | table={tableId} | shoe={gameShoe} | round={gameRound} | prevTable={_netSeqTableId} | prevShoe={_netSeqGameShoe} | prevLen={_boardSeqDisplay.Length} | prevVer={_netSeqVersion}");
                ...
                else
                {
                    ...
                    _boardSeqVersion = 0;
                    _boardSeqEvent = "";
                    _syncSeqPrefixDisplay = _boardSeqDisplay;
        ...
        private void TryProcessNetworkWinnerPacket(string payload, bool isBinary, string ownerTag, string? url)
        {
            ...
            Dispatcher.BeginInvoke(new Action(() =>
            {
                try
                {
                    ...
                    string settleDisplay = "";
                    long settleVersion = 0;
                    double balanceAfter = 0;
                    lock (_snapLock)
                    {
                        settleDisplay = FilterResultDisplaySeqWindow(_lastSnap?.rawSeq);
                        settleVersion = _lastSnap?.seqVersion ?? 0;
                        balanceAfter = _lastSnap?.totals?.A ?? 0;
                    }
                    ...
                    lock (_roundStateLock)
                    {
                        ...
                        Log($"[NETSEQ][WINNER][OBSERVE-ONLY] src={packet.OwnerTag} | table={packet.TableId} | shoe={packet.GameShoe} | round={packet.GameRound} | winner={winnerChar.Value} | netLen={_boardSeqDisplay.Length} | netVer={_netSeqVersion} | banker={packet.BankerValue} | player={packet.PlayerValue} | reason=live-seq-count-only");

                        if (string.IsNullOrWhiteSpace(settleDisplay))
                            settleDisplay = FilterResultDisplaySeqWindow(_boardSeqDisplay);
                        if (settleVersion <= 0)
                            settleVersion = _netSeqVersion > 0 ? _netSeqVersion : settleDisplay.Length;
                    }
        ...
        private static bool IsTrustedSettleAdvanceSeqEvent(string? seqEventRaw)
        {
            ...
            return evt.StartsWith("count-append-", StringComparison.OrdinalIgnoreCase);
        }
            ...
            string boardSeqEvent)
        {
            ...
            var netDisplay = FilterResultDisplaySeqWindow(_boardSeqDisplay);
            ...
            if ((staleRawDuringNoBoard && totalsAllZero) || tableSwitchChangingShoeEmptyLike)
            {
                ...
                SetSnapshotRawSeqLocked(
                    snap,
                    "",
                    _netSeqVersion,
                    string.IsNullOrWhiteSpace(_netSeqEvent) ? "table-switch-wait-bead" : _netSeqEvent,
            ...
            {
                SetSnapshotRawSeqLocked(snap, "", _netSeqVersion, "table-switch-wait-bead", "table-switch-wait-bead");
            ...
            SetSnapshotRawSeqLocked(snap, rebaseDisplay, _netSeqVersion, _netSeqEvent, "table-switch-reset");
        ...
        {
            if (string.IsNullOrWhiteSpace(_boardSeqDisplay) || _tableSwitchRebaseArmed || _initialTableEnterArmed)
                return;
            ...
            if (IsChangingShoeStatus(statusRaw))
            {
                ...
                if (!_shoeChangeRebaseArmed && _shoeChangeStatusStreak >= 2)
                {
                    ...
                    Log($"[SEQ][SHOE-ARM] rawLen=0 | seqLen={_boardSeqDisplay.Length} | seqVer={_netSeqVersion} | status=CHANGING SHOE | src={(string.IsNullOrWhiteSpace(source) ? "-" : source)} | evt={(string.IsNullOrWhiteSpace(boardSeqEvent) ? "-" : boardSeqEvent)} | streak={_shoeChangeStatusStreak}");
                }
                return;
            }
        ...
        private bool TryApplyShoeRebaseLocked(CwSnapshot snap, string source, string statusRaw, string boardSeqEvent)
        {
            ...
            SetSnapshotRawSeqLocked(
                ...
                netDisplay,
                _netSeqVersion,
                string.IsNullOrWhiteSpace(_netSeqEvent) ? (snap.seqEvent ?? "") : _netSeqEvent,
                "authority-hold");
        ...
        private void SetSnapshotRawSeqLocked(CwSnapshot snap, string rawDisplay, long versionHint, string seqEvent, string seqSource)
        {
            rawDisplay = FilterResultDisplaySeqWindow(rawDisplay);
            long seqVersion = rawDisplay.Length == 0 ? 0 : Math.Max(versionHint, rawDisplay.Length);
            string finalEvent = string.IsNullOrWhiteSpace(seqEvent) ? "raw-direct" : seqEvent;
            string finalSource = string.IsNullOrWhiteSpace(seqSource) ? "raw-direct" : seqSource;

            snap.rawSeq = rawDisplay;
            snap.seq = rawDisplay;
            snap.seqVersion = Math.Max(snap.seqVersion ?? 0, seqVersion);
            snap.seqEvent = finalEvent;
            snap.seqSource = finalSource;

            _boardSeqDisplay = rawDisplay;
            _boardSeqVersion = seqVersion;
            _boardSeqEvent = finalEvent;
            _netSeqVersion = seqVersion;
            _netSeqEvent = finalEvent;
            _netSeqSource = finalSource;
        }
        ...
        private void SyncNetworkSeqFromSnapshot(CwSnapshot snap, string source, string boardDisplay, long boardSeqVersion, string boardSeqEvent, string statusRaw)
        {
            ...
            var prevDisplay = FilterResultDisplaySeqWindow(_boardSeqDisplay);
            var prevEvent = _boardSeqEvent;
            var rawDisplay = FilterResultDisplaySeqWindow(snap.rawSeq);
            var boardDisplayFiltered = FilterResultDisplaySeqWindow(boardDisplay);
            var finalDisplay = rawDisplay.Length > 0 ? rawDisplay : boardDisplayFiltered;
            var finalEvent = string.IsNullOrWhiteSpace(boardSeqEvent)
                ? (rawDisplay.Length > 0 ? "raw-direct" : "board-fallback")
                : boardSeqEvent;
            ...
            {
                Log($"[SEQ][RAW-DIRECT] src={(string.IsNullOrWhiteSpace(source) ? "-" : source)} | rawLen={rawDisplay.Length} | boardLen={boardDisplayFiltered.Length} | evt={(string.IsNullOrWhiteSpace(finalEvent) ? "-" : finalEvent)} | status={(string.IsNullOrWhiteSpace(statusRaw) ? "-" : Shrink(statusRaw, 48))}");
            }

            if (!string.Equals(prevDisplay, finalDisplay, StringComparison.Ordinal))
            {
                string kind =
                    finalDisplay.Length > prevDisplay.Length ? "grow" :
                    finalDisplay.Length < prevDisplay.Length ? "shrink" :
                    "rewrite";
                Log(
                    $"[SEQ][TRANS] src={(string.IsNullOrWhiteSpace(source) ? "-" : source)} | kind={kind} | prevLen={prevDisplay.Length} | currLen={finalDisplay.Length} | rawLen={rawDisplay.Length} | boardLen={boardDisplayFiltered.Length} | prevEvt={(string.IsNullOrWhiteSpace(prevEvent) ? "-" : prevEvent)} | evt={(string.IsNullOrWhiteSpace(finalEvent) ? "-" : finalEvent)} | prevCount={BuildSeqCountText(prevDisplay, includeH: true)} | currCount={BuildSeqCountText(finalDisplay, includeH: true)} | prev={Shrink(prevDisplay, 96)} | curr={Shrink(finalDisplay, 96)} | status={(string.IsNullOrWhiteSpace(statusRaw) ? "-" : Shrink(statusRaw, 48))}");
            }
            ...
            if (finalDisplay.Length > 0)
            {
                if (_tableSwitchRebaseArmed || _initialTableEnterArmed)
                    ClearTableSwitchRebaseArmLocked();
                if (_shoeChangeRebaseArmed && !IsChangingShoeStatus(statusRaw))
                    ClearShoeChangeRebaseArmLocked();
            }

            SetSnapshotRawSeqLocked(snap, finalDisplay, finalVersion, finalEvent, finalSource);
        }
        ...
        private void ApplyNetworkSeqAuthorityLocked(CwSnapshot? snap)
        {
            ...
            var rawSeq = FilterResultDisplaySeqWindow(snap.rawSeq);
            var fallbackSeq = FilterResultDisplaySeqWindow(_boardSeqDisplay);
            var finalSeq = rawSeq.Length > 0 ? rawSeq : fallbackSeq;
            var finalSource = rawSeq.Length > 0
                ? "raw-direct"
                : (string.IsNullOrWhiteSpace(_netSeqSource) ? "board-authority" : _netSeqSource);
            var finalEvent = string.IsNullOrWhiteSpace(snap.seqEvent) ? _netSeqEvent : snap.seqEvent;
            var finalVersion = finalSeq.Length == 0 ? 0 : Math.Max(_netSeqVersion, finalSeq.Length);

            SetSnapshotRawSeqLocked(snap, finalSeq, finalVersion, finalEvent, finalSource);
        ...
        private CwSnapshot? CloneAuthoritativeTaskSnap()
        {
            ...
            lock (_roundStateLock) ApplyNetworkSeqAuthorityLocked(clone);
            if (clone != null)
                clone.seq = FilterPlayableSeq(clone.rawSeq);
            return clone;
        }

        private static AuthorityBetSettleInfo? CloneAuthorityBetSettleInfo(AuthorityBetSettleInfo? settle)
        {
            if (settle == null) return null;
            return new AuthorityBetSettleInfo
            {
                Serial = settle.Serial,
                ResultText = settle.ResultText,
                ResultChar = settle.ResultChar,
                SeqDisplay = settle.SeqDisplay,
                SeqVersion = settle.SeqVersion,
                SeqEvent = settle.SeqEvent,
                Reason = settle.Reason,
                TableId = settle.TableId,
                GameShoe = settle.GameShoe,
                GameRound = settle.GameRound,
                CreatedUtc = settle.CreatedUtc
            };
        }

        private long GetAuthorityBetSettleSerial()
        {
            lock (_authorityBetSettleLock)
                return _authorityBetSettleSerial;
        }

        private async Task<AuthorityBetSettleInfo?> WaitForAuthorityBetSettleAsync(long afterSerial, CancellationToken ct)
        {
            TaskCompletionSource<AuthorityBetSettleInfo?>? waiter = null;
            lock (_authorityBetSettleLock)
            {
                if (_latestAuthorityBetSettle != null && _latestAuthorityBetSettle.Serial > afterSerial)
                    return CloneAuthorityBetSettleInfo(_latestAuthorityBetSettle);

                waiter = new TaskCompletionSource<AuthorityBetSettleInfo?>(TaskCreationOptions.RunContinuationsAsynchronously);
                _authorityBetSettleWaiters.Add(waiter);
            }

            using var reg = ct.Register(() =>
            {
                lock (_authorityBetSettleLock)
                    _authorityBetSettleWaiters.Remove(waiter);
        ...
        private void PublishAuthorityBetSettleLocked(
            char resultChar,
            string resultText,
            string seqDisplay,
            long seqVersion,
            ...
            long gameRound = 0)
        {
            ...
            string normalizedResult = string.IsNullOrWhiteSpace(resultText)
                ? MapWinnerCharToResultText(resultChar)
                : resultText.Trim().ToUpperInvariant();
            string normalizedSeq = FilterResultDisplaySeqWindow(seqDisplay);
            long normalizedVersion = seqVersion > 0 ? seqVersion : normalizedSeq.Length;
            string normalizedEvent = seqEvent ?? "";
            long normalizedTableId = tableId > 0 ? tableId : (_netObservedTableId > 0 ? _netObservedTableId : _netSeqTableId);
            long normalizedGameShoe = gameShoe > 0 ? gameShoe : (_netObservedGameShoe > 0 ? _netObservedGameShoe : _netSeqGameShoe);
            long normalizedGameRound = gameRound > 0 ? gameRound : (_netObservedGameRound > 0 ? _netObservedGameRound : _netSeqLastRound);
            ...
            List<TaskCompletionSource<AuthorityBetSettleInfo?>> waitersToRelease;
            AuthorityBetSettleInfo info;
            lock (_authorityBetSettleLock)
            {
                if (string.Equals(_authorityBetSettleKey, dedupeKey, StringComparison.Ordinal))
                    return;

                _authorityBetSettleKey = dedupeKey;
                _authorityBetSettleSerial++;
                info = new AuthorityBetSettleInfo
                {
                    Serial = _authorityBetSettleSerial,
                    ResultText = normalizedResult,
                    ResultChar = resultChar,
                    SeqDisplay = normalizedSeq,
                    SeqVersion = normalizedVersion,
                    SeqEvent = normalizedEvent,
                    Reason = reason ?? "",
                    TableId = normalizedTableId,
                    GameShoe = normalizedGameShoe,
                    GameRound = normalizedGameRound,
                    CreatedUtc = DateTime.UtcNow
                };
                _latestAuthorityBetSettle = info;
                waitersToRelease = _authorityBetSettleWaiters.ToList();
                _authorityBetSettleWaiters.Clear();
            }

            Log($"[BET][AUTH][SETTLE] serial={info.Serial} | result={info.ResultText} | seqVer={info.SeqVersion} | seqEvt={(string.IsNullOrWhiteSpace(info.SeqEvent) ? "-" : info.SeqEvent)} | table={(info.TableId > 0 ? info.TableId.ToString() : "-")} | shoe={(info.GameShoe > 0 ? info.GameShoe.ToString() : "-")} | round={(info.GameRound > 0 ? info.GameRound.ToString() : "-")} | reason={(string.IsNullOrWhiteSpace(info.Reason) ? "-" : info.Reason)}");

            foreach (var waiter in waitersToRelease)
                waiter.TrySetResult(CloneAuthorityBetSettleInfo(info));
        }
    ...
    return JSON.stringify({ points:[], err:String((e&&e.message)?e.message:e) });
        ...
        private async void VaoXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                QueueBridgeProbe("vao-license-ok");
        ...
        private async Task<bool> EnsureGameContextReadyForPlayAsync(string stagePrefix)
        {
            ...
            try
            {
                ...
                await EnsureToolBridgeInjectedAsync();
                QueueBridgeProbe(stage + "-after-ensure");
                if (preferSamePage)
                ...
                {
                    await EnsureToolBridgeInjectedAsync();
                    QueueBridgeProbe(stage + "-game-ready");
                ...
                else
                {
                    ...
                    QueueBridgeProbe(stage + "-game-not-ready");
...
";
        ...
        private static bool IsKnownModernBetShellHost(string? hostRaw)
        {
            ...
            return
                ...
                host.EndsWith("zowin.vu", StringComparison.OrdinalIgnoreCase) ||
                host.EndsWith("zowin.tw", StringComparison.OrdinalIgnoreCase) ||
                host.EndsWith("zowin.ph", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static bool IsExactLiveGameFrameHref(string? rawUrl)
        {
            var href = (rawUrl ?? "").Trim();
            if (href.Length == 0)
                return false;

            var hrefL = href.ToLowerInvariant();
            return
                hrefL.Contains("/internal/livestream_page/", StringComparison.OrdinalIgnoreCase) &&
                (hrefL.Contains("c5_bcrlive_withoutoplay", StringComparison.OrdinalIgnoreCase) ||
                 hrefL.Contains("bcrlive_withoutoplay", StringComparison.OrdinalIgnoreCase) ||
                 hrefL.Contains("bcrlive", StringComparison.OrdinalIgnoreCase));
        }

        private static int ScoreLiveGameFrameHref(string? rawUrl)
        {
            var href = (rawUrl ?? "").Trim();
            if (href.Length == 0)
                return 0;
            ...
            var hrefL = href.ToLowerInvariant();
            var score = 0;
            if (IsExactLiveGameFrameHref(href))
                score += 10000;
            if (hrefL.Contains("/internal/livestream_page/", StringComparison.OrdinalIgnoreCase))
                score += 2000;
            if (hrefL.Contains("c5_bcrlive_withoutoplay", StringComparison.OrdinalIgnoreCase))
                score += 2000;
            else if (hrefL.Contains("bcrlive_withoutoplay", StringComparison.OrdinalIgnoreCase))
                score += 1500;
            else if (hrefL.Contains("bcrlive", StringComparison.OrdinalIgnoreCase))
                score += 1200;
            if (hrefL.Contains("selectedgame=baccarat", StringComparison.OrdinalIgnoreCase))
                score += 250;
            if (hrefL.Contains("/activations/baccarat", StringComparison.OrdinalIgnoreCase))
                score += 250;
            return score;
        }
        ...
        private static bool IsLikelyModernBetShellUrl(string? rawUrl)
        {
            ...
            if (IsLikelyBetGatewayUrl(rawUrl))
                return false;
            if (IsExactLiveGameFrameHref(rawUrl))
                return true;
...
(function(){
  try{
    ...
    for (var i=0;i<frames.length;i++){
      ...
      if (/\/\/(?:[^\/]+\.)?zowin\.(?:nu|vu|tw|ph)\//i.test(l) || /\/\/(?:[^\/]+\.)?game8b\.com\//i.test(l) || /\/\/bpweb\./i.test(l) || /\/\/games\./i.test(l) || /usplaynet\.com/i.test(l) || /balikko\.com/i.test(l) || /barppat\.com/i.test(l) || /restula\.com/i.test(l) || /atllat\.com/i.test(l)) out.hasKnownShell = true;
...
(function(){
  try{
    ...
    for (var i=0;i<els.length;i++){
      ...
      if (u.indexOf('zowin.nu') >= 0 || u.indexOf('zowin.vu') >= 0 || u.indexOf('zowin.tw') >= 0 || u.indexOf('zowin.ph') >= 0) s += 65;
...
(function(){
  try{
    ...
    for (var i=0;i<els.length;i++){
      ...
      if (u.indexOf('zowin.nu') >= 0 || u.indexOf('zowin.vu') >= 0 || u.indexOf('zowin.tw') >= 0 || u.indexOf('zowin.ph') >= 0) s += 72;
      if (u.indexOf('game8b.com') >= 0) s += 55;
    ...
    return best || '';
        ...
        private (bool ok, string reason) GetBetPipeReadyState(StrategyTabState tab, long runId)
        {
            ...
            bool hasSnapData =
                (snap?.progValid ?? 0) == 1 ||
                !string.IsNullOrWhiteSpace(snap?.status) ||
                !string.IsNullOrWhiteSpace(snap?.rawSeq);
            if (!hasSnapData)
        ...
        private void AutoStopTasksOnBetPipelineReset(string reason, string src)
        {
            ...
            BaccaratZoWin.Tasks.TaskUtil.ClearBetCooldown();
        ...
        private void ResetSeqSyncState(string reason, bool clearPendingRows = false, bool forceLog = false)
        {
            ...
            lock (_roundStateLock)
            {
                ...
                lock (_authorityBetSettleLock)
                {
                    _authorityBetSettleKey = "";
                    _latestAuthorityBetSettle = null;
                }
        ...
        private async void PopupFrame_WebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            try
            {
                var msg = e.TryGetWebMessageAsString() ?? "";
                if (string.IsNullOrWhiteSpace(msg)) return;
                await DispatchIncomingWebMessageAsync(msg, "popup-frame", "PopupFrame.WebMessageReceived");
            }
            catch (Exception ex)
            {
  ...
  try{
    ...
    return JSON.stringify({ err:String((e&&e.message)?e.message:e) });
        ...
        private List<(ulong id, CoreWebView2Frame frame)> GetMainArmedFramesSnapshot()
        {
            ...
            try
            {
                var orderedIds = _mainFrameBridgeArmed.Keys
                    .OrderByDescending(id =>
                    {
                        _mainFrameHrefCache.TryGetValue(id, out var href);
                        return ScoreLiveGameFrameHref(href);
                    })
        ...
        private void DropPopupFrameRef(CoreWebView2Frame? frame)
        {
            ...
            _popupFrameHrefCache.TryRemove(key, out _);
        }
        ...
        private List<(int key, CoreWebView2Frame frame)> GetPopupArmedFramesSnapshot()
        {
            ...
            try
            {
                var orderedFrames = _popupFrameRefs.ToArray()
                    .OrderByDescending(kv =>
                    {
                        _popupFrameHrefCache.TryGetValue(kv.Key, out var href);
                        return ScoreLiveGameFrameHref(href);
                    })
                    .ThenByDescending(kv => kv.Key);

                foreach (var kv in orderedFrames)
                {
                    if (kv.Value != null)
        ...
        private Task<string> ExecuteOnBetWebAsync(string js)
        {
            return Dispatcher.InvokeAsync(async () =>
            {
                ...
                bool hasExactLiveFrame = usePopupFrames
                    ? popupFrames.Any(item =>
                    {
                        _popupFrameHrefCache.TryGetValue(item.key, out var href);
                        return IsExactLiveGameFrameHref(href);
                    })
                    : mainFrames.Any(item =>
                    {
                        _mainFrameHrefCache.TryGetValue(item.id, out var href);
                        return IsExactLiveGameFrameHref(href);
                    });
                ...
                if (!string.IsNullOrWhiteSpace(firstFrameOk) && hasExactLiveFrame)
                    return firstFrameOk;
        ...
        private GameContext BuildContext(StrategyTabState tab, long runId, bool useRawWinAmount = false)
        {
            ...
            var betWhenRemainingSeconds = (tab != null && tab.RunBetWhenRemainingSeconds > 0) ? tab.RunBetWhenRemainingSeconds : _betWhenRemainingSeconds;
            ...
            return new GameContext
            {
                ...
                GetAuthoritySettleSerial = () => GetAuthorityBetSettleSerial(),
                WaitAuthoritySettleAsync = (afterSerial, token) => WaitForAuthorityBetSettleAsync(afterSerial, token),
                TabId = tab.Id,
                RunId = runId,
                ...
                BetWhenRemainingSeconds = betWhenRemainingSeconds,
                ...
                UiFinalizeMultiBet = (winners, resultDisplay) => PostToUi(() =>
                {
                ...
                // ==== 3 callback UI ====
                UiSetSide = s => PostToUi(() =>
                {
                    UpdateTabSide(tab, s);
                }),
                UiSetStake = v => PostToUi(() =>
                {
                ...
                UiWinLoss = s => PostToUi(() =>
                {
                    UpdateTabWinLoss(tab, s);
                }),
                UiSetWinLossText = text => PostToUi(() =>
                {
        ...
        private void RecordBetIssuedUi(StrategyTabState? betTab, string? tabId, string sideRaw, long amount, long roundId)
        {
            ...
            var issuedSeqDisplay = issuedSnap?.rawSeq ?? "";
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                Log("[PlayClick] begin"
                    + " | tab=" + (activeTab?.Id ?? "-")
                    + " | name=" + (string.IsNullOrWhiteSpace(activeTab?.Name) ? "-" : activeTab.Name)
                    + " | running=" + ((activeTab?.IsRunning == true) ? 1 : 0)
                    + " | hasCts=" + ((activeTab?.TaskCts != null) ? 1 : 0)
                    + " | strategy=" + (_cfg?.BetStrategyIndex.ToString() ?? "-")
                    + " | src=" + (GetBetWebViewSource(GetBetWebView()) ?? "-"));
                if (activeTab == null)
                {
                    Log("[PlayClick] blocked: no-active-tab");
                ...
                {
                    Log("[PlayClick] blocked: validate-input-failed"
                        + " | strategy=" + (_cfg?.BetStrategyIndex.ToString() ?? "-")
                        + " | seqErr=" + (LblSeqError?.Text ?? "-")
                        + " | patErr=" + (LblPatError?.Text ?? "-")
                        + " | sideErr=" + (LblSideRatioError?.Text ?? "-"));
                ...
                if (CheckLicense && (!_licenseVerified || _runExpiresAt == null || _runExpiresAt <= DateTimeOffset.Now))
                {
                    ...
                    {
                        Log("[PlayClick] blocked: EnsureLicenseOnceAsync=false");
                        return;
                    }
                }

                ResetSeqSyncState("play-start", clearPendingRows: false, forceLog: true);
                QueueBridgeProbe("play-start-pre-ensure");
                await EnsureToolBridgeInjectedAsync();
                QueueBridgeProbe("play-start-post-ensure");
                ...
                {
                    QueueBridgeProbe("play-missing-bet-before-reinject");
                ...
                if (!string.Equals(typeBet, "function", StringComparison.OrdinalIgnoreCase))
                {
                    QueueBridgeProbe("play-missing-bet-timeout");
                    Log("[DEC] Không thể vào bàn/tiêm JS trong thời gian chờ. Vui lòng thử lại.");
                    return;
                    }
                    QueueBridgeProbe("play-missing-bet-recovered");
                }
                ...
                if (!ready)
                {
                    ...
                    if (!ready)
                    {
                        QueueBridgeProbe("play-wait2-timeout");
                ...
                SyncBetWhenRemainingSecondsFromUi();
                activeTab.RunDecisionSeconds = _decisionSeconds;
                activeTab.RunBetWhenRemainingSeconds = _betWhenRemainingSeconds;
                ...
                BaccaratZoWin.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new BaccaratZoWin.Tasks.SeqParityFollowTask(),     // 1
                    1 => new BaccaratZoWin.Tasks.PatternParityTask(),       // 2
                    2 => new BaccaratZoWin.Tasks.SeqMajorMinorTask(),       // 3
                    3 => new BaccaratZoWin.Tasks.PatternMajorMinorTask(),   // 4
                    4 => new BaccaratZoWin.Tasks.SmartPrevTask(),           // 5
                    5 => new BaccaratZoWin.Tasks.RandomParityTask(),        // 6
                    6 => new BaccaratZoWin.Tasks.AiStatParityTask(),        // 7
                    7 => new BaccaratZoWin.Tasks.StateTransitionBiasTask(), // 8
                    8 => new BaccaratZoWin.Tasks.RunLengthBiasTask(),       // 9
                    9 => new BaccaratZoWin.Tasks.EnsembleMajorityTask(),    // 10
                    10 => new BaccaratZoWin.Tasks.TimeSlicedHedgeTask(),    // 11
                    11 => new BaccaratZoWin.Tasks.KnnSubsequenceTask(),     // 12
                    12 => new BaccaratZoWin.Tasks.DualScheduleHedgeTask(),  // 13
                    13 => new BaccaratZoWin.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 14
                    14 => new BaccaratZoWin.Tasks.AiExpertPanelTask(), // 15
                    15 => new BaccaratZoWin.Tasks.Top10PatternFollowTask(), // 16
                    16 => new BaccaratZoWin.Tasks.SeqParityHotBackTask(), // 17
                    _ => new BaccaratZoWin.Tasks.SmartPrevTask(),
                };
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        // đặt trong MainWindow.xaml.cs (project BaccaratZoWin)
        ...
        private void UpdateTabWin(StrategyTabState tab, double net, string moneyStrategyId)
        {
            ...
            try
            {
                BaccaratZoWin.Tasks.MoneyHelper.NotifyTempProfit(moneyStrategyId, net);
            }
            catch { /* ignore */ }
        ...
        private async Task<string> LoadAppJsAsyncFallback()
        {
            try
            {
                ...
                var resName = FindResourceName("v4_js_xoc_dia_live.js")
                              ?? "BaccaratZoWin.v4_js_xoc_dia_live.js";
        ...
        {
            await EnsureWebReadyAsync();
            QueueBridgeProbe("ensure-before");
            await EnsureBridgeRegisteredAsync();
            await InjectOnNewDocAsync();
            if (IsCurrentHostAllowUnboundHistory())
        ...
        private void DropMainFrameRef(CoreWebView2Frame? frame)
        {
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    _mainFrameHrefCache.TryRemove(id, out _);
                }
            }
            catch { }
        }
        ...
        private void ObserveTableSwitchFromFrameHref(string? hrefRaw, string stage)
        {
            ...
            lock (_roundStateLock)
            {
                ...
                {
                    Log($"[SEQ][SHOE-ARM-CLEAR] reason=table-switch-detected | stage={stage} | from={_lastBaccaratFrameKey} | to={key} | netLen={_boardSeqDisplay.Length} | netVer={_netSeqVersion} | fromHref={TrimHrefForLog(_lastBaccaratFrameHref)} | toHref={TrimHrefForLog(hrefRaw)}");
                    ClearShoeChangeRebaseArmLocked();
                }
                ...
                Log($"[SEQ][TABLE-SWITCH-ARM] stage={stage} | from={_tableSwitchFromKey} | to={_tableSwitchToKey} | netLen={_boardSeqDisplay.Length} | netVer={_netSeqVersion} | fromHref={TrimHrefForLog(_tableSwitchFromHref)} | toHref={TrimHrefForLog(_tableSwitchToHref)}");
            }
        }
        ...
        private async Task<bool> InjectGameBridgeOnFrameIfNeededAsync(CoreWebView2Frame frame, string stage)
        {
            ...
            var frameId = TryGetFrameIdSafe(frame);
            if (!string.IsNullOrWhiteSpace(probe.Href))
            {
                if (frameId > 0)
                    _mainFrameHrefCache[frameId] = probe.Href;
                if (frameRefKey != 0)
                    _popupFrameHrefCache[frameRefKey] = probe.Href;
            }
            ...
            var exactLiveFrame = IsExactLiveGameFrameHref(probe.Href);
            await frame.ExecuteScriptAsync(FRAME_SHIM);
            if (exactLiveFrame)
                await frame.ExecuteScriptAsync("try{window.__abx_force_push_start=1;window.__cw_panel_autostart=1;}catch(_){}");
            if (!string.IsNullOrEmpty(_appJs))
                await frame.ExecuteScriptAsync(_appJs);
            await frame.ExecuteScriptAsync(FRAME_AUTOSTART);
            await frame.ExecuteScriptAsync(START_PUSH_NOW);
            if (exactLiveFrame)
                _ = Task.Run(async () =>
                {
                    try
            ...
            var reason =
                exactLiveFrame ? "exact-live-frame" :
                byCocos ? "cocos" :
                byHref ? "href" :
        ...
        private async void MainFrame_WebMessageReceived_Bridge(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            try
            {
                var msg = e.TryGetWebMessageAsString() ?? "";
                if (string.IsNullOrWhiteSpace(msg)) return;
                await DispatchIncomingWebMessageAsync(msg, "main-frame", "MainFrame.WebMessageReceived");
            }
            catch (Exception ex)
            {
        ...
        private async Task TryPullPopupTickFallbackAsync()
        {
            ...
            if (_lastPopupTickPullUtc != DateTime.MinValue &&
                (now - _lastPopupTickPullUtc) <= PopupTickPullFallbackMinGap)
                return;
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(msg))
                    return;
                await DispatchIncomingWebMessageAsync(msg, "popup-pull", "PopupPull");
            }
            catch { }
            finally
            {
        ...
        private async Task TryPullMainTickFallbackAsync()
        {
            ...
            if (_lastMainTickPullUtc != DateTime.MinValue &&
                (now - _lastMainTickPullUtc) <= MainTickPullFallbackMinGap)
                return;
            var age = now - _lastGameTickUtc;
            if (age <= TimeSpan.FromMilliseconds(650)) return;
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(msg))
                    return;
                await DispatchIncomingWebMessageAsync(msg, "main-pull", "MainPull");
            }
            catch { }
            finally
            {
        ...
        private async Task<bool> WaitForBridgeAndGameDataAsync(int timeoutMs = 20000)
        {
            ...
            while ((DateTime.UtcNow - t0).TotalMilliseconds < timeoutMs)
            {
                try
                {
                    ...
                    lock (_snapLock)
                    {
                        lastSeqLen = _lastSnap?.rawSeq?.Length ?? 0;
                        lastStatus = _lastSnap?.status ?? "";
                        hasTick =
                            (_lastSnap?.rawSeq != null && _lastSnap.rawSeq.Length > 0) ||
        ...
        {
            var prev = _cfg?.BetWhenRemainingSeconds ?? 0;
            SyncBetWhenRemainingSecondsFromUi();
            UpdateTooltips();
            ...
            if (prev != _cfg.BetWhenRemainingSeconds)
                await SaveConfigAsync();
        }
        ...
        private static CwSnapshot? CloneSnapForTasks(CwSnapshot? snap)
        {
            ...
            return new CwSnapshot
            {
                ...
                seq = FilterPlayableSeq(snap.rawSeq),
                rawSeq = snap.rawSeq,
        ...
        private static bool ValidatePatternsCL(string s, out string err)
        {
            ...
            foreach (var raw in rules)
            {
                ...
                {
                    err = $"Quy tắc {idx}: độ dài <mẫu_quá_khứ> phải từ 1-20 ký tự (B/P).";
                    return false;
                }
        ...
        private static bool ValidatePatternsNI(string s, out string err)
        {
            ...
            foreach (var raw in rules)
            {
                ...
                if (lhs.Length < 1 || lhs.Length > 20)
                {
                    err = $"Quy tắc {idx}: độ dài <mẫu_quá_khứ> phải từ 1-20 ký tự (I/N).";
                    return false;
                }
