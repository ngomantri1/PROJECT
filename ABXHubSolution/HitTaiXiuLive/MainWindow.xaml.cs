...
using static HitTaiXiuLive.MainWindow;
...
namespace HitTaiXiuLive
{
    ...
    {
        private const string AppLocalDirName = "HitTaiXiuLive"; // đổi thành tên bạn muốn
        // ====== App paths ======
        ...
        const string LicenseNameGame = "hit";          // <- nhánh
...
• Ví dụ: 15 = đặt khi còn ~15% thời gian phiên.";
        ...
        private const string Wv2ZipResNameX64 = "HitTaiXiuLive.ThirdParty.WebView2Fixed_win-x64.zip";
  ...
  try{
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        {
            // _appDataDir bạn đã tạo ở Startup: %LOCALAPPDATA%\HitTaiXiuLive
    ...
    return 'ok';
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Fallback: nếu vẫn chưa có Source thì điều hướng lại URL mặc định
                if (Web != null && Web.Source == null && !string.IsNullOrWhiteSpace(_cfg.Url))
                {
                    Log("[StartupNav] Web.Source trống, điều hướng lại: " + _cfg.Url);
                    await NavigateIfNeededAsync(_cfg.Url.Trim());
                }
...
";
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                HitTaiXiuLive.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new HitTaiXiuLive.Tasks.SeqParityFollowTask(),     // 1
                    1 => new HitTaiXiuLive.Tasks.PatternParityTask(),       // 2
                    2 => new HitTaiXiuLive.Tasks.SeqMajorMinorTask(),       // 3
                    3 => new HitTaiXiuLive.Tasks.PatternMajorMinorTask(),   // 4
                    4 => new HitTaiXiuLive.Tasks.SmartPrevTask(),           // 5
                    5 => new HitTaiXiuLive.Tasks.RandomParityTask(),        // 6
                    6 => new HitTaiXiuLive.Tasks.AiStatParityTask(),        // 7
                    7 => new HitTaiXiuLive.Tasks.StateTransitionBiasTask(), // 8
                    8 => new HitTaiXiuLive.Tasks.RunLengthBiasTask(),       // 9
                    9 => new HitTaiXiuLive.Tasks.EnsembleMajorityTask(),    // 10
                    10 => new HitTaiXiuLive.Tasks.TimeSlicedHedgeTask(),    // 11
                    11 => new HitTaiXiuLive.Tasks.KnnSubsequenceTask(),     // 12
                    12 => new HitTaiXiuLive.Tasks.DualScheduleHedgeTask(),  // 13
                    13 => new HitTaiXiuLive.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 14
                    14 => new HitTaiXiuLive.Tasks.AiExpertPanelTask(), // 15
                    15 => new HitTaiXiuLive.Tasks.Top10PatternFollowTask(), // 16
                    _ => new HitTaiXiuLive.Tasks.SmartPrevTask(),
                };
        ...
        private void StopXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                StopTask();
                HitTaiXiuLive.Tasks.TaskUtil.ClearBetCooldown();
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        // đặt trong MainWindow.xaml.cs (project HitTaiXiuLive)
        ...
        private async Task<string> LoadAppJsAsyncFallback()
        {
            try
            {
                ...
                var resName = FindResourceName("v4_js_xoc_dia_live.js")
                              ?? "HitTaiXiuLive.v4_js_xoc_dia_live.js";
        ...
        private async Task<string> LoadHomeJsAsync()
        {
            try
            {
                var resName = FindResourceName("js_home_v2.js")
                              ?? "HitTaiXiuLive.js_home_v2.js"; // fallback tên logic
