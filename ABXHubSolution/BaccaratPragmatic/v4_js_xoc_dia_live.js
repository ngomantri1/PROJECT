(() => {
    ...
    function __cw_readStoredCanvasWatchVisible() {
        try {
            if (!window.localStorage)
                return null;
            var currentDefault = (CW_PANEL_VISIBLE_DEFAULT ? '1' : '0') + '|' + String(CW_PANEL_VISIBLE_DEFAULT_REV || '');
    ...
    function __cw_setCanvasWatchVisible(show) {
        ...
        try {
            if (window.localStorage) {
                window.localStorage.setItem(CW_PANEL_VISIBLE_DEFAULT_KEY, (CW_PANEL_VISIBLE_DEFAULT ? '1' : '0') + '|' + String(CW_PANEL_VISIBLE_DEFAULT_REV || ''));
    ...
    function __cw_isGamePopupPage() {
        try {
            ...
            var isLiveTablesBaccarat =
                host === 'play.livetables.io' &&
    ...
    function domPoolTailMatch(full, expected) {
        var f = domPoolNormTail(full);
        var e = domPoolNormTail(expected);
        if (!f || !e) return false;
        return f === e || f.indexOf(e) >= 0 || e.indexOf(f) >= 0 || f.slice(-e.length) === e;
    }
    function domPoolSideByPreferredTail(tail) {
        if (domPoolTailMatch(tail, DOM_POOL_PREFERRED_TAILS.BANKER)) return 'BANKER';
        if (domPoolTailMatch(tail, DOM_POOL_PREFERRED_TAILS.PLAYER)) return 'PLAYER';
        if (domPoolTailMatch(tail, DOM_POOL_PREFERRED_TAILS.TIE)) return 'TIE';
        return null;
    }
    function domPoolPreferredTailBonus(side, tail) {
        try {
            if (!side) return 0;
            var expected = DOM_POOL_PREFERRED_TAILS[side];
            if (domPoolTailMatch(tail, expected))
                return 18000;
        } catch (_) {}
        return 0;
    }
    ...
    function domReadPreferredPoolByTail(ctx) {
        try {
            ...
            return {
                ...
                tailB: bySide.BANKER ? bySide.BANKER.tail : null,
                tailP: bySide.PLAYER ? bySide.PLAYER.tail : null,
                tailT: bySide.TIE ? bySide.TIE.tail : null,
                source: String((bySide.BANKER || bySide.PLAYER || bySide.TIE || {}).source || ctx.source || 'top') + '/preferred-tail-direct',
                score: found * 12000
            };
        } catch (_) {
            return null;
    ...
    function domPoolHasPreferredTails(pool) {
        try {
            if (!pool) return false;
            var hit = 0;
            if (domPoolTailMatch(pool.tailB, DOM_POOL_PREFERRED_TAILS.BANKER)) hit++;
            if (domPoolTailMatch(pool.tailP, DOM_POOL_PREFERRED_TAILS.PLAYER)) hit++;
            if (domPoolTailMatch(pool.tailT, DOM_POOL_PREFERRED_TAILS.TIE)) hit++;
            return hit >= 2;
        } catch (_) {
            return false;
        }
    ...
    function domScanBetStakeTotalsFromVisibleText(contexts) {
        try {
            ...
            for (var ci = 0; ci < contexts.length; ci++) {
                var ctx = contexts[ci];
                var preferredPack = domReadPreferredPoolByTail(ctx);
                if (!preferredPack)
                    continue;
                var pack = {
                    B: preferredPack.B,
                    P: preferredPack.P,
                    T: preferredPack.T,
                    rawB: preferredPack.rawB,
                    rawP: preferredPack.rawP,
                    rawT: preferredPack.rawT,
                    tailB: preferredPack.tailB,
                    tailP: preferredPack.tailP,
                    tailT: preferredPack.tailT,
                    source: String(preferredPack.source || (ctx && ctx.source) || 'top') + '/preferred-only',
                    score: Number(preferredPack.score || 0) + Number(ctx && ctx._betScore || 0)
                };
                if (!best || domPoolValueCount(pack) > domPoolValueCount(best) || Number(pack.score || 0) > Number(best.score || 0))
                    best = pack;
            }
            return best;
        } catch (_) {
    ...
    function domPoolSourceIsDisplayAuthority(pool) {
        try {
            ...
            if (/\/text(?:$|[/?#])/.test(s))
                return false;
            if (/partial-blocked|partial-incomplete/.test(s))
                return false;
    ...
    function domScanBetStakeTotals(force) {
        try {
            ...
            var out = domScanBetStakeTotalsFromVisibleText(contexts) || {
                B: null,
                P: null,
                T: null,
                rawB: null,
                ...
                tailP: null,
                tailT: null,
                source: 'preferred-tail-only/none',
                score: 0
            };
            try {
                var ctxBrief = [];
                ...
                cwDbg('BETPOOL', 'preferred-tail-only', {
                    force: !!force,
            ...
            return {
                ...
                tailP: null,
                tailT: null,
                source: 'preferred-tail-only/error',
                score: 0
            };
        }
    }
    ...
    var LIVE_TABLES_BALANCE_SCOPE_SELECTOR = 'div._1xccx7s2 div._5n201i4 div._5n201i2 div._5n201i0 div._5n201i0 span.yhlecm0';
    var LIVE_TABLES_BALANCE_VALUE_TAIL = 'div._5n201i4/div._5n201i2/div._5n201i0/div._5n201i0/span.yhlecm0/span';
    function domIsLiveTablesBaccaratHref(href) {
        try {
            var h = String(href || '').toLowerCase();
            return h.indexOf('https://play.livetables.io/baccarat') === 0 ||
                h.indexOf('http://play.livetables.io/baccarat') === 0;
        } catch (_) {
            return false;
        }
    }
    function domFindLiveTablesBalanceExact() {
        try {
            ...
            for (var ci = 0; ci < contexts.length; ci++) {
                var ctx = contexts[ci];
                if (!ctx || !ctx.doc || !domIsLiveTablesBaccaratHref(ctx.href))
                    continue;
                var scopes = [];
                try {
                    scopes = ctx.doc.querySelectorAll(LIVE_TABLES_BALANCE_SCOPE_SELECTOR);
                } catch (_) {
                    scopes = [];
                }
                for (var si = 0; si < scopes.length; si++) {
                    var scope = scopes[si];
                    if (!scope || !domVisible(scope))
                        continue;
                    var moneyText = '';
                    var moneyVal = null;
                    var childSpans = [];
                    try {
                        childSpans = scope.querySelectorAll('span');
                    } catch (_) {
                        childSpans = [];
                    }
                    for (var sj = 0; sj < childSpans.length; sj++) {
                        var sp = childSpans[sj];
                        if (!sp || !domVisible(sp))
                            continue;
                        var st = domCollapse(sp.innerText || sp.textContent || '');
                        if (!st)
                            continue;
                        var sv = balanceOf(st);
                        if (sv != null && (moneyVal == null || sv > moneyVal)) {
                            moneyVal = sv;
                            moneyText = st;
                        }
                    }
                    if (moneyVal == null) {
                        var hostText = domCollapse(scope.innerText || scope.textContent || '');
                        var token = domExtractBalanceTokenFromLabelText(hostText);
                        var mv = token ? balanceOf(token) : balanceOf(hostText);
                        if (mv != null) {
                            moneyVal = mv;
                            moneyText = token || hostText;
                        }
                    }
                    if (moneyVal == null)
                        continue;
                    var score = 1000;
                    try {
                        var rr = scope.getBoundingClientRect();
                        if (rr && rr.top >= 0 && rr.top <= ((ctx.win && ctx.win.innerHeight) || 900))
                            score += 50;
                    } catch (_) {}
                    best = {
                        balance: moneyVal,
                        rawBalance: moneyText || null,
                        source: String(ctx.source || 'top') + ' :: ' + LIVE_TABLES_BALANCE_VALUE_TAIL,
                        tail: LIVE_TABLES_BALANCE_VALUE_TAIL,
                        score: score
                    };
                    break;
                }
    ...
    function domFindHudSnapshot() {
        try {
            ...
            var exactBalanceLocked = false;
            try {
                var liveTablesBal = domFindLiveTablesBalanceExact();
                if (liveTablesBal && liveTablesBal.balance != null) {
                    hud.balance = liveTablesBal.balance;
                    hud.rawBalance = liveTablesBal.rawBalance || null;
                    hud.source = liveTablesBal.source || hud.source || null;
                    exactBalanceLocked = true;
                }
                var topHud = domFindTopHudSnapshot();
                if (topHud.account)
                    hud.account = topHud.account;
                if (!exactBalanceLocked && topHud.balance != null) {
                    hud.balance = topHud.balance;
                    hud.rawBalance = topHud.rawBalance;
                }
                if ((!exactBalanceLocked || !hud.source) && topHud.source)
    ...
    var _cwSeqScriptRev = 'SEQFIX-20260506-r34-seq-bootstrap-wait';
    ...
    function brRebaseManagedByCountGuard(seq, guard, sourceTag, beforeSeq) {
        ...
        brInvalidateSeqCaches(_domSeqEvent, clean, clean);
    ...
    function brInvalidateSeqCaches(reasonTag, seqValue, rawValue) {
        var seq = brSanitizeSeq(seqValue || '');
        var raw = brSanitizeSeq(rawValue || seq || '');
        try {
        ...
        try {
            if (window.__cw_last_panel_snapshot) {
                window.__cw_last_panel_snapshot.seq = seq;
                window.__cw_last_panel_snapshot.rawSeq = raw;
                window.__cw_last_panel_snapshot.seqVersion = Number(_domSeqVersion || 0);
                window.__cw_last_panel_snapshot.seqEvent = String(_domSeqEvent || '');
                window.__cw_last_panel_snapshot.seqSource = String(reasonTag || '');
            }
        } catch (_) {}
        cwDbg('SEQFLOW', 'seq-cache-invalidate', {
            reason: String(reasonTag || ''),
            seqLen: seq.length,
            rawLen: raw.length,
            seqVersion: Number(_domSeqVersion || 0),
            seqEvent: String(_domSeqEvent || '')
        }, 0, 'seq-cache-invalidate|' + String(reasonTag || '') + '|' + Number(_domSeqVersion || 0) + '|' + seq.length);
    }
    function brResetManagedForTable(activeTitle, activeSeq, reason) {
        ...
        _domSeqAppend = _domBeadSeqManaged;
        brInvalidateSeqCaches(_domSeqEvent, _domBeadSeqManaged, _domBeadSeqPrevRaw);
        brPublishSeqState();
    ...
    function brResetManagedForTableWaitBead(activeTitle, reason) {
        ...
        _domSeqAppend = '';
        brInvalidateSeqCaches(_domSeqEvent, '', '');
        brPublishSeqState();
    ...
    function readTKSeq() {
        ...
        if (!__cw_hasCocos()) {
            ...
            var earlyManagedTitle = String(_domManagedTableTitle || '');
            if (activeTitle && earlyManagedTitle && activeTitle !== earlyManagedTitle) {
                var earlyBeforeLen = String(_domBeadSeqManaged || '').length;
                brResetManagedForTableWaitBead(activeTitle, 'table-switch-wait-bead');
                cwDbg('TABLE', 'active-table-switch-early-cache-reset', {
                    from: earlyManagedTitle,
                    to: activeTitle,
                    beforeLen: earlyBeforeLen,
                    activeSeqLen: String(activeSeq || '').length,
                    seqVersion: Number(_domSeqVersion || 0),
                    seqEvent: String(_domSeqEvent || ''),
                    buildSource: String(_cwSnapshotBuildSource || ''),
                    buildId: Number(_cwSnapshotBuildId || 0)
                }, 0, 'table-switch-early-reset|' + earlyManagedTitle + '|' + activeTitle + '|' + Number(_domSeqVersion || 0));
            }
...
/** pick first item by tail (sorted by y,x) */
    ...
    function totals(S, forceFast) {
        ...
        var stakeDom = null;
        try {
            stakeDom = domScanBetStakeTotals(!!forceFast);
        } catch (_) {
            stakeDom = null;
        }
        var cocosB = (stakeDom && stakeDom.B != null) ? stakeDom.B : (mC ? mC.val : null);
        var cocosP = (stakeDom && stakeDom.P != null) ? stakeDom.P : (mL ? mL.val : null);
        var cocosT = (stakeDom && stakeDom.T != null) ? stakeDom.T : null;
        var cocosRawB = (stakeDom && stakeDom.rawB != null) ? stakeDom.rawB : (mC ? mC.txt : null);
        var cocosRawP = (stakeDom && stakeDom.rawP != null) ? stakeDom.rawP : (mL ? mL.txt : null);
        var cocosRawT = (stakeDom && stakeDom.rawT != null) ? stakeDom.rawT : null;
        var cocosSource = (stakeDom && (stakeDom.source || stakeDom.BS || stakeDom.Source)) ? String(stakeDom.source || stakeDom.BS || stakeDom.Source) : null;
        var mSD = null;
    ...
    var UI_PROG_HOLD_MS = 14000;
    var UI_PROG_RESET_UP_TOLERANCE_SEC = 0.35;
    var UI_PROG_FORCE_ZERO_TOLERANCE_SEC = 0.05;

    function __cw_clampUiProgSec(sec) {
        var n = Number(sec);
        if (!isFinite(n))
            return null;
        if (n < 0)
    ...
    function __cw_getUiProgSourceKey(useAuthSnap, authSnap, waitingCsharpDisplay) {
        try {
            if (useAuthSnap && authSnap) {
                return [
                    'auth',
                    String(authSnap.dataMode || ''),
                    String(authSnap.progSource || window.__cw_prog_source || ''),
                    String(authSnap.progTail || window.__cw_prog_tail || ''),
                    String(authSnap.dataFramePath || authSnap.proxyChildFramePath || authSnap.framePath || ''),
                    String(authSnap.dataHref || authSnap.proxyChildHref || authSnap.href || '').split('?')[0]
                ].join('|');
            }
            return [
                waitingCsharpDisplay ? 'waiting' : 'local',
                String(window.__cw_prog_source || ''),
                String(S._progTail || window.__cw_prog_tail || ''),
                String(location.href || '').split('?')[0]
            ].join('|');
        } catch (_) {
            return useAuthSnap ? 'auth' : 'local';
        }
    }
    function __cw_getUiProgSmoothSec(rawSec, sourceKey) {
        var bounded = __cw_clampUiProgSec(rawSec);
        var now = Date.now();
        var predicted = null;
        if (S._uiProgAnchorSec != null && S._uiProgAnchorAt) {
            var currentRate = Number(S._uiProgRate || 1);
            if (!isFinite(currentRate) || currentRate <= 0)
                currentRate = 1;
            predicted = __cw_clampUiProgSec(S._uiProgAnchorSec - (((now - S._uiProgAnchorAt) / 1000) * currentRate));
        }

        if (bounded == null) {
            if (predicted != null && Number(S._uiProgLastRawAt || 0) > 0 && (now - Number(S._uiProgLastRawAt || 0)) <= UI_PROG_HOLD_MS) {
                S._uiProgSmoothSec = predicted;
                try {
                    var holdWhole = predicted <= UI_PROG_FORCE_ZERO_TOLERANCE_SEC ? 0 : Math.ceil(predicted);
                    if (S._uiProgLastDisplayWhole !== holdWhole) {
                        S._uiProgLastDisplayWhole = holdWhole;
                        cwDbg('COUNTDOWN', 'display', {
                            action: 'hold-missing',
                            raw: null,
                            predicted: predicted,
                            display: predicted,
                            displayWhole: holdWhole,
                            rawAgeMs: now - Number(S._uiProgLastRawAt || 0)
                        }, 0, 'countdown-display|' + holdWhole + '|hold');
                    }
                } catch (_) {}
                return predicted;
            }
            S._uiProgAnchorSec = null;
            S._uiProgAnchorAt = 0;
            S._uiProgLastRawAt = 0;
            S._uiProgRate = 1;
            S._uiProgSourceKey = '';
            S._uiProgSmoothSec = null;
            return null;
        }

        var key = String(sourceKey || '');
        S._uiProgLastRawAt = now;
        if (
            S._uiProgAnchorSec == null ||
            !S._uiProgAnchorAt ||
            key !== String(S._uiProgSourceKey || '') ||
            predicted == null
        ) {
            S._uiProgAnchorSec = bounded;
            S._uiProgAnchorAt = now;
            S._uiProgRate = 1;
            S._uiProgSourceKey = key;
        } else if (bounded > predicted + UI_PROG_RESET_UP_TOLERANCE_SEC) {
            S._uiProgAnchorSec = bounded;
            S._uiProgAnchorAt = now;
            S._uiProgRate = 1;
            S._uiProgSourceKey = key;
        } else if (bounded <= UI_PROG_FORCE_ZERO_TOLERANCE_SEC) {
            S._uiProgAnchorSec = 0;
            S._uiProgAnchorAt = now;
            S._uiProgRate = 1;
            S._uiProgSourceKey = key;
        } else {
            // Giữ cadence 1Hz ổn định; raw đến thưa thì panel tiếp tục tự trôi theo thời gian thật.
            S._uiProgRate = 1;
            S._uiProgSourceKey = key;
        }

        var activeRate = Number(S._uiProgRate || 1);
        if (!isFinite(activeRate) || activeRate <= 0)
            activeRate = 1;
        var smooth = __cw_clampUiProgSec(S._uiProgAnchorSec - (((now - S._uiProgAnchorAt) / 1000) * activeRate));
        S._uiProgRawSec = bounded;
        S._uiProgSmoothSec = smooth;
        S._uiProgLastDrift = predicted == null ? null : Math.abs(predicted - bounded);
        try {
            var rawWhole = bounded <= UI_PROG_FORCE_ZERO_TOLERANCE_SEC ? 0 : Math.ceil(bounded);
            if (S._uiProgLastRawWhole !== rawWhole) {
                S._uiProgLastRawWhole = rawWhole;
                cwDbg('COUNTDOWN', 'raw', {
                    raw: bounded,
                    predicted: predicted,
                    anchor: S._uiProgAnchorSec,
                    rate: activeRate,
                    sourceKey: key
                }, 0, 'countdown-raw|' + rawWhole);
            }
            var displayWhole = smooth <= UI_PROG_FORCE_ZERO_TOLERANCE_SEC ? 0 : Math.ceil(smooth);
            if (S._uiProgLastDisplayWhole !== displayWhole) {
                S._uiProgLastDisplayWhole = displayWhole;
                cwDbg('COUNTDOWN', 'display', {
                    action: 'smooth',
                    raw: bounded,
                    predicted: predicted,
                    ...
                    rawAgeMs: now - Number(S._uiProgLastRawAt || 0)
                }, 0, 'countdown-display|' + displayWhole + '|smooth');
            }
        } catch (_) {}
        return smooth;
    }
    ...
    function updatePanel() {
        ...
        var seqDisplayState = '';
        var waitingBoardBootstrap = false;
        try {
        ...
        try {
            if (useAuthSnap && authSnap) {
                seqDisplayState = String(authSnap.seqDisplayState || authSnap.displayState || '');
                var authSeqText = String(authSnap.seq || '');
                var authSeqEvent = String(authSnap.seqEvent || '');
                var authSeqSource = String(authSnap.seqSource || '');
                waitingBoardBootstrap = !!(
                    !authSeqText &&
                    (
                        seqDisplayState === 'waiting-board-bootstrap' ||
                        brSeqEventIsWaitingBoardBootstrap(authSeqEvent) ||
                        brSeqEventIsWaitingBoardBootstrap(authSeqSource)
                    )
                );
                if (waitingBoardBootstrap && !seqDisplayState)
                    seqDisplayState = 'waiting-board-bootstrap';
            }
        } catch (_) {
            seqDisplayState = '';
        ...
        if (useAuthSnap) {
            try {
                ...
                var mergedAuthTotals = mergeTotalsSnapshot(snapTotals, S && S._lastTotals ? S._lastTotals : null);
                t = mergedAuthTotals || snapTotals || {
                    B: null,
                    P: null,
                    C: null,
                    L: null,
        ...
        var displayStatus = useAuthSnap ? (waitingBoardBootstrap ? 'waiting-board-bootstrap' : String(authSnap.status || '')) : (waitingCsharpDisplay ? 'waiting-csharp-display' : String(S.status || ''));
        var displayProgIsSec = useAuthSnap ? true : !!S._progIsSec;
        var displayProgSmooth = null;
        if (displayProgIsSec && displayProg != null)
            displayProgSmooth = __cw_getUiProgSmoothSec(displayProg, __cw_getUiProgSourceKey(useAuthSnap, authSnap, waitingCsharpDisplay));
        else
            __cw_getUiProgSmoothSec(null, '');
        var progDisplayValue = (displayProgIsSec ? displayProgSmooth : displayProg);
        var progText = (progDisplayValue == null ? '--' : (displayProgIsSec ? (((progDisplayValue <= 0.0001 ? 0 : Math.ceil(progDisplayValue))) + 's') : (((progDisplayValue * 100) | 0) + '%')));
        ...
        if (useAuthSnap) {
            ...
            dataModeLine = (useCsharpDisplaySnap ? 'csharp-display/' : 'authority-snapshot/') + dataMode + ' | age=' + authAge + 'ms' + (waitingBoardBootstrap ? ' | waiting-board-bootstrap' : '');
        }
    ...
    function startPanelRenderTimer() {
        if (S.panelTimer)
            return;
...
;
    (function () {
        ...
        function mergeTotalsSnapshot(primary, fallback) {
            ...
            function poolSourceRank(t) {
                try {
                    ...
                    if (!s)
                        return 0;
                    if (s.indexOf('partial-blocked') >= 0 || s.indexOf('partial-incomplete') >= 0)
                        return 3;
            ...
            var pPoolSource = String((p && (p.BS || p.source || p.Source)) || '').toLowerCase();
            var pLocksPool = pPoolRank >= 3 && (hasPoolValue(p) || pPoolSource.indexOf('partial-blocked') >= 0 || pPoolSource.indexOf('partial-incomplete') >= 0);
        ...
        function buildSnapshotNow(sourceTag) {
            ...
            if (useSeqCache && buildSource === 'push' && !__cw_hasCocos()) {
                try {
                    var cacheProbeCards = domScanBaccaratCards();
                    var cacheProbeActive = domPickActiveCard(cacheProbeCards);
                    var cacheProbeTitle = cacheProbeActive && cacheProbeActive.title ? String(cacheProbeActive.title || '') : '';
                    var cacheProbeManagedTitle = String(_domManagedTableTitle || '');
                    if (cacheProbeTitle && cacheProbeManagedTitle && cacheProbeTitle !== cacheProbeManagedTitle) {
                        var cacheProbeBeforeLen = String(_domBeadSeqManaged || '').length;
                        useSeqCache = false;
                        try {
                            _abxSnapCache.seqState = null;
                            _abxSnapCache.seqAt = 0;
                        } catch (_) {}
                        brResetManagedForTableWaitBead(cacheProbeTitle, 'table-switch-wait-bead');
                        cwDbg('SEQFLOW', 'seq-cache-refresh-for-table-switch', {
                            from: cacheProbeManagedTitle,
                            to: cacheProbeTitle,
                            beforeLen: cacheProbeBeforeLen,
                            seqVersion: Number(_domSeqVersion || 0),
                            seqEvent: String(_domSeqEvent || ''),
                            buildId: buildId,
                            buildSource: buildSource
                        }, 0, 'seq-cache-refresh-table-switch|' + cacheProbeManagedTitle + '|' + cacheProbeTitle + '|' + Number(_domSeqVersion || 0));
                    }
                } catch (_) {}
            }
            try {
            ...
            if (brSeqEventIsWaitingBoardBootstrap(seqEvent)) {
                ...
                if (seqFreshIsRebase) {
                    if (seq && seq !== seqFresh) {
                        cwDbg('SEQFLOW', 'snapshot-fresh-rebase-over-cache', {
                            buildId: buildId,
                            source: buildSource,
                            cachedLen: String(seq || '').length,
                            freshLen: String(seqFresh || '').length,
