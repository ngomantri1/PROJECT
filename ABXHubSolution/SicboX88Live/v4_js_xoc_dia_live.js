(() => {
    'use strict';
    /* =========================================================
    CanvasWatch + MoneyMap + BetMap + TextMap + Scan1000Text
    ...
    ========================================================= */
    // Canvas Watch default visibility:
    // - false: an panel/layer debug
    ...
    function __cw_isLikelyGameUrl(ctx, score) {
        var c = ctx || __cw_urlContext();
        var hostL = __cw_safeLower(c.host);
        var sc = Number(score);
        if (!(sc >= 0))
        ...
        if (/^games\./i.test(hostL))
            return true;
        if (hostL.indexOf('swgames.club') !== -1)
            return sc >= 20;
        if (hostL.indexOf('livearena') !== -1)
            return sc >= 14;
        return sc >= 24;
    ...
    function __cw_probeGameScene(limitNodes) {
        ...
        try {
            ...
            if (info.canvases > 0)
                info.score += 2;
            info.score += uscore;
            var likelyGameUrl = __cw_isLikelyGameUrl(uctx, uscore);
            info.ok = !!(
                info.prefabKey ||
                (info.urlOk && (info.score >= 24 || info.labels > 8)) ||
                (likelyGameUrl && (info.canvases > 0 || info.labels > 0))
            );
        } catch (_) {}
        try {
        ...
        var timer = setInterval(function () {
            try {
                ...
                if (sceneStrong)
                    ...
                    strongSceneHits--;
                if (!probe.ok && hasCc && likelyGameUrl &&
                    ((Number(probe.score) || 0) >= 20 || (Number(probe.labels) || 0) > 0 || (Number(probe.canvases) || 0) > 0)) {
                    probe.ok = true;
                }
                if (!match.ok) {
                    if (tries % 24 === 0)
    ...
    function normalizePathLoose(s) {
        s = String(s == null ? '' : s).toLowerCase();
        s = s.replace(/\\/g, '/');
        s = s.replace(/\/+/g, '/');
        s = s.replace(/node_game\([^\/]*\)/g, 'node_game');
        s = s.replace(/\/+$/g, '');
        return s;
    }
    function pathLooseMatch(full, tail) {
        if (!full || !tail)
            return false;
        var f = normalizePathLoose(full);
        var t = normalizePathLoose(tail);
        if (!f || !t)
            return false;
        if (f === t || f.indexOf(t, Math.max(0, f.length - t.length)) !== -1 || t.indexOf(f, Math.max(0, t.length - f.length)) !== -1)
            return true;
        var fs = f.split('/').filter(Boolean);
        var ts = t.split('/').filter(Boolean);
        if (!fs.length || !ts.length)
            return false;
        if (fs[fs.length - 1] !== ts[ts.length - 1])
            return false;
        var maxSeg = Math.min(4, fs.length, ts.length);
        for (var seg = maxSeg; seg >= 2; seg--) {
            if (fs.slice(-seg).join('/') === ts.slice(-seg).join('/'))
                return true;
        }
        return ts.length === 1;
    }
    function readCompText(c) {
        ...
        for (var i = 0; i < keys.length; i++) {
            var k = keys[i];
            try {
                if (typeof c[k] !== 'undefined' && c[k] != null) {
                    var s = String(c[k]);
                    if (s && s.trim())
                        return s;
                }
    ...
    function nodeInGame(n) {
        ...
        try {
            ...
            if (/\/prefab_game_[0-9a-z_]+/i.test(p))
                return true;
            if ((p.indexOf('/node_dual/') !== -1 || p.indexOf('/node_game/') !== -1 || /\/prefab_game_[0-9a-z_]+/i.test(p)) &&
                (/lastresult|board_back|avatar_node|node_noti|countdown|count_down|soicau|bet_panel|chip_panel|lbl_money_total_bet_|lbl_username|lbl_money/i.test(p)))
                return true;
    ...
    var COUNTDOWN_TAIL_MAIN = normalizePathLoose(window.__cw_countdown_tail_override || 'tx_live_tamtam/canvas/root/loading_view/node_loading');
    function __cw_isCountdownPath(path) {
        var pathL = normalizePathLoose(path);
        if (!pathL)
            return false;
        if (COUNTDOWN_TAIL_MAIN && (pathL.indexOf(COUNTDOWN_TAIL_MAIN) !== -1 || pathLooseMatch(pathL, COUNTDOWN_TAIL_MAIN)))
            return true;
        return false;
    }
    function __cw_isCountdownNoisePath(pathL) {
        return /popup\/|session_history|rank_node|tip_rank_node|bet_history_node|list_player_ingame|betarea\/|userdata\/lbl_|btn_useronline\/lbl_|result_node\/.*lbl_moneywin|last_result\//i.test(pathL);
    }
    function __cw_countdownScreenScore(r, pathL) {
        var W = innerWidth || 1;
        var H = innerHeight || 1;
        var sx = Number(r && r.sx) || 0;
        var sy = Number(r && r.sy) || 0;
        var sw = Number(r && r.sw) || 0;
        var sh = Number(r && r.sh) || 0;
        var cx = sx + (sw / 2);
        var cy = sy + (sh / 2);
        var score = 0;
        if (cx >= (W * 0.50) && cx <= (W * 0.66))
            score += 28;
        else if (cx >= (W * 0.44) && cx <= (W * 0.72))
            score += 10;
        else
            score -= 12;
        if (cy >= (H * 0.20) && cy <= (H * 0.52))
            score += 28;
        else if (cy >= (H * 0.14) && cy <= (H * 0.60))
            score += 10;
        else
            score -= 12;
        if (sw >= 18 && sw <= 140)
        ...
        if (sh >= 18 && sh <= 140)
            score += 10;
        if (/\/lb_loading_spin(\/|$)/i.test(pathL))
            score += 24;
        else if (/\/node_loading(\/|$)/i.test(pathL))
            score += 14;
        return score;
    }
    function readCountdownSec() {
        ...
        walkNodes(function (n) {
            ...
            var pathL = normalizePathLoose(path);
            if (!__cw_isCountdownPath(pathL))
                return;
            if (__cw_isCountdownNoisePath(pathL))
                return;
            var comps = (n._components || []);
            for (var i = 0; i < comps.length; i++) {
                var text = readCompText(comps[i]);
                if (!text)
                    continue;
                ...
                picks.push({
                    ...
                    w: r.sw,
                    h: r.sh,
                    score: __cw_countdownTailScore(pathL, {
                            x: r.sx,
                            y: r.sy,
                            w: r.sw,
                            h: r.sh
                        }, sec, innerWidth || 0, innerHeight || 0)
                });
            }
        });
        if (!picks.length)
    ...
    function __cw_readCountdownRatioFromPath() {
        var picks = [];
        walkNodes(function (n) {
            var path = fullPath(n, 120);
            var pathL = normalizePathLoose(path);
            ...
            if (!comps || !comps.length)
                return;
            var r = wRect(n);
            var screenScore = __cw_countdownScreenScore(r, pathL);
            for (var i = 0; i < comps.length; i++) {
                var c = comps[i];
                if (!c)
                    continue;
                var ratio = null;
                var keys = ['progress', '_progress', 'fillRange', '_fillRange'];
    ...
    function __cw_parseCountdownSec(txt) {
        var m = String(txt || '').match(/^\s*(\d{1,2})\s*$/);
        if (!m)
            return null;
        var sec = parseInt(m[1], 10);
    ...
    function __cw_countdownTailScore(pathL, r, sec, W, H) {
        var score = 0;
        if (sec >= 1 && sec <= 20)
            score += 24;
            ...
            score += 4;
        if (/\/canvas\/root\//i.test(pathL))
            score += 10;
        else if (/\/canvas\//i.test(pathL))
            score += 5;
        if (/count|countdown|time|timer|clock|remain|round|session|prog/i.test(pathL))
            score += 18;
        if (/\/right\/last_result\//i.test(pathL))
            score -= 46;
        if (/\/betarea\//i.test(pathL))
            score -= 42;
        if (/popup\/|session_history|rank_node|tip_rank_node|bet_history_node|list_player_ingame|chat|userdata|btn_useronline|lbl_currentbet|lbl_moneywin/i.test(pathL))
            score -= 28;
        var xMid = (r.x || 0) + ((r.w || 0) / 2);
        var yMid = (r.y || 0) + ((r.h || 0) / 2);
        if (W > 0) {
            var xn = xMid / W;
            if (xn >= 0.50 && xn <= 0.67)
                score += 28;
        ...
        if (H > 0) {
            ...
            if (yn >= 0.20 && yn <= 0.58)
                score += 14;
            else if (yn >= 0.08 && yn <= 0.70)
                score += 6;
            else
                score -= 12;
        }
        if ((r.w || 0) >= 38 && (r.w || 0) <= 120)
            score += 18;
        else if ((r.w || 0) >= 18 && (r.w || 0) <= 140)
            score += 8;
        else if ((r.w || 0) > 180)
            score -= 8;
        if ((r.h || 0) >= 42 && (r.h || 0) <= 120)
            score += 18;
        else if ((r.h || 0) >= 18 && (r.h || 0) <= 140)
            score += 8;
        else if ((r.h || 0) > 160)
            score -= 8;
        return score;
    }
    function __cw_findCountdownTailCandidates(opts) {
        opts = opts || {};
        var out = [];
        var W = innerWidth || 0;
        var H = innerHeight || 0;
        var limit = Math.max(1, Number(opts.limit) || 20);
        var useGameFilter = (typeof opts.useGameFilter === 'undefined') ? false : !!opts.useGameFilter;
        walkNodes(function (n) {
            if (useGameFilter && !nodeInGame(n))
                return;
            var comps = (n._components || []);
            for (var i = 0; i < comps.length; i++) {
                var c = comps[i];
                if (!c)
                    continue;
                var text = readCompText(c);
                if (!String(text || '').trim())
                    continue;
                var sec = __cw_parseCountdownSec(text);
                if (sec == null)
                    continue;
                var path = fullPath(n, 120);
                var pathL = normalizePathLoose(path);
                ...
                if ((r.w || 0) <= 0 || (r.h || 0) <= 0)
                    continue;
                if ((r.w || 0) > Math.max(260, W * 0.40) || (r.h || 0) > Math.max(180, H * 0.26))
                    continue;
                if ((r.w || 0) < 18 || (r.h || 0) < 20)
                    continue;
                var score = __cw_countdownTailScore(pathL, r, sec, W, H);
                out.push({
                    score: score,
                    sec: sec,
        ...
        out.sort(function (a, b) {
            ...
            if (a.sec !== b.sec) {
                var ad = Math.abs((a.sec || 0) - 12);
                var bd = Math.abs((b.sec || 0) - 12);
                if (ad !== bd)
                    return ad - bd;
            }
            if (a.y !== b.y)
        ...
        window.__cw_last_countdown_tail_candidates = out.slice();
        try {
            __cw_tryPost({
                abx: 'cw_countdown_tail_candidates',
                count: out.length,
                top: out.slice(0, Math.min(limit, 12)),
                ts: Date.now()
            });
        } catch (_) {}
        return out.slice(0, limit);
    }
    window.__cw_findCountdownTailCandidates = function (opts) {
        return __cw_findCountdownTailCandidates(opts || {});
    };
    window.__cw_setCountdownTail = function (tail) {
        COUNTDOWN_TAIL_MAIN = normalizePathLoose(tail || '');
        window.__cw_countdown_tail_override = COUNTDOWN_TAIL_MAIN;
        return COUNTDOWN_TAIL_MAIN;
    };
    window.__cw_getCountdownTail = function () {
        return COUNTDOWN_TAIL_MAIN;
    };
    window.__cw_dumpCountdownTailCandidates = function (limit) {
        var rows = __cw_findCountdownTailCandidates({
            limit: Math.max(1, Number(limit) || 20),
            useGameFilter: false
        });
        var lines = [];
        lines.push('[FindCountdownTail] current=' + String(COUNTDOWN_TAIL_MAIN || ''));
        ...
        lines.push('Use: __cw_setCountdownTail(window.__cw_last_countdown_tail_candidates[0].tail)');
        try {
            if (typeof setCwLog === 'function')
                setCwLog(lines.join('\n'));
            if (typeof setLogHint === 'function')
                setLogHint('countdown-tail');
        } catch (_) {}
        return rows;
    };
    window.__cw_guessAndSetCountdownTail = function () {
        var rows = __cw_findCountdownTailCandidates({
            limit: 1,
            useGameFilter: false
        });
        ...
        if (!top)
            return null;
        window.__cw_setCountdownTail(top.tail);
        try {
            if (typeof setCwLog === 'function')
                setCwLog('[CountdownTail:set]\n' +
                    'score=' + top.score + ' sec=' + top.sec + '\n' +
                    'tail=' + top.tail + '\n' +
                    'x=' + Math.round(top.x) + ' y=' + Math.round(top.y) + ' w=' + Math.round(top.w) + ' h=' + Math.round(top.h));
            if (typeof setLogHint === 'function')
                setLogHint('countdown-tail-set');
        } catch (_) {}
        return top;
    };
    function __cw_rectIntersects(a, b) {
        if (!a || !b)
            return false;
        var ax2 = (a.x || 0) + Math.max(0, a.w || 0);
        var ay2 = (a.y || 0) + Math.max(0, a.h || 0);
        var bx2 = (b.x || 0) + Math.max(0, b.w || 0);
        var by2 = (b.y || 0) + Math.max(0, b.h || 0);
        return !(
            ax2 < (b.x || 0) ||
            bx2 < (a.x || 0) ||
            ay2 < (b.y || 0) ||
    ...
    function __cw_compNames(n) {
        ...
        try {
            ...
            for (var i = 0; i < comps.length; i++) {
                ...
                var nm = '';
                try {
                    nm = String((c.constructor && c.constructor.name) || c.name || '');
                } catch (_) {}
                if (!nm)
                    nm = typeof c;
                out.push(nm);
    ...
    function __cw_countdownProbeRect(opts) {
        var W = innerWidth || 1;
        var H = innerHeight || 1;
        opts = opts || {};
        return {
            x: Math.round(W * (opts.x1 != null ? opts.x1 : 0.48)),
            y: Math.round(H * (opts.y1 != null ? opts.y1 : 0.18)),
            w: Math.round(W * ((opts.x2 != null ? opts.x2 : 0.67) - (opts.x1 != null ? opts.x1 : 0.48))),
            h: Math.round(H * ((opts.y2 != null ? opts.y2 : 0.55) - (opts.y1 != null ? opts.y1 : 0.18)))
        };
    }
    function __cw_probeCountdownRegionDeep(opts) {
        opts = opts || {};
        var box = __cw_countdownProbeRect(opts);
        var out = [];
        walkNodes(function (n) {
            var r = wRect(n);
            if ((r.w || 0) <= 0 || (r.h || 0) <= 0)
                return;
            if (!__cw_rectIntersects(r, box))
                return;
            var text = '';
            var comps = (n._components || []);
            for (var i = 0; i < comps.length; i++) {
                var t = readCompText(comps[i]);
                if (String(t || '').trim()) {
                    text = String(t || '').trim();
                    break;
                }
            ...
            var score = 0;
            if (sec != null)
                score += (sec >= 1 && sec <= 20) ? 40 : 12;
            if (/label|richtext|bitmapfont|bmfont/i.test(__cw_compNames(n)))
                score += 8;
            if (/progressbar/i.test(__cw_compNames(n)))
                score += 10;
            if (/\/betarea\//i.test(pathL))
                score -= 70;
            if (/\/right\/last_result\//i.test(pathL))
                score -= 50;
            if (/popup\/|session_history|rank_node|tip_rank_node|bet_history_node|chat|list_player_ingame|userdata|btn_useronline/i.test(pathL))
                score -= 60;
            var cx = (r.x || 0) + ((r.w || 0) / 2);
            var cy = (r.y || 0) + ((r.h || 0) / 2);
            var bx = box.x + (box.w / 2);
            var by = box.y + (box.h / 2);
            score -= Math.round(Math.abs(cx - bx) / 18);
            score -= Math.round(Math.abs(cy - by) / 14);
            if ((r.w || 0) >= 26 && (r.w || 0) <= 140)
                score += 10;
            if ((r.h || 0) >= 26 && (r.h || 0) <= 140)
                score += 10;
            out.push({
                score: score,
                sec: sec,
                ...
                h: r.h,
                tail: path,
                comps: __cw_compNames(n)
            });
        });
        out.sort(function (a, b) {
            if (b.score !== a.score)
                return b.score - a.score;
            var an = (a.sec != null) ? 0 : 1;
            var bn = (b.sec != null) ? 0 : 1;
            if (an !== bn)
                return an - bn;
            return (b.w * b.h) - (a.w * a.h);
        });
        window.__cw_last_countdown_region_probe = out.slice();
        return {
            box: box,
            rows: out
        };
    }
    window.__cw_probeCountdownRegionDeep = function (opts) {
        return __cw_probeCountdownRegionDeep(opts || {});
    };
    window.__cw_dumpCountdownRegionDeep = function (opts) {
        var res = __cw_probeCountdownRegionDeep(opts || {});
        var rows = res.rows || [];
        var lines = [];
        lines.push('[CountdownRegionDeep]');
        lines.push('box=' + [res.box.x, res.box.y, res.box.w, res.box.h].join(','));
        lines.push('count=' + rows.length);
        ...
        try {
            if (typeof setCwLog === 'function')
                setCwLog(lines.join('\n'));
            if (typeof setLogHint === 'function')
                setLogHint('countdown-deep');
        } catch (_) {}
        return res;
    };
    function collectLabels() {
        var out = [];
        function collectPass(useGameFilter) {
            walkNodes(function (n) {
                if (useGameFilter && !nodeInGame(n))
                    return;
                var comps = (n._components || []);
    ...
    function collectProgress() {
        ...
        if (cd != null)
            return cd;
        var cdRatio = __cw_readCountdownRatioFromPath();
        if (cdRatio != null) {
            S._progIsSec = false;
            return clamp01(Number(cdRatio));
        }
        ...
        var bars = [];
        walkNodes(function (n) {
            var full = fullPath(n, 120);
            var fullL = normalizePathLoose(full);
            if (!__cw_isCountdownPath(fullL) && !nodeInGame(n))
                return;
        ...
        var countdownBars = cs.filter(function (b) {
            return __cw_isCountdownPath(b.fullTail || b.tail || '');
        });
        var barPick = (countdownBars[0] || cs[0] || bars[0]);
        var bar = barPick.comp;
        try {
            S._progTail = barPick.fullTail || barPick.tail || '';
        } catch (e) {}
        try {
            window.__cw_prog_tail = barPick.fullTail || barPick.tail || '';
        } catch (e) {}
        ...
        function collectPass(useGameFilter) {
            walkNodes(function (n) {
                ...
                for (var i = 0; i < comps.length; i++) {
                    var c = comps[i];
                    if (c) {
                        var s = String(readCompText(c) || '').trim();
                        if (!s)
                            continue;
    ...
    var TAIL_LAST_RESULT_ROWS = 'lastresult/results/row-';
...
/** pick first item by tail (sorted by y,x) */
    ...
    panel.innerHTML = '' +
        ...
        '<button id="bScanText">Scan1000Text</button>' +
        '<button id="bFindCdTail">FindCdTail</button>' +
        '<button id="bFindCdDeep">FindCdDeep</button>' +
    ...
    function applyCanvasWatchVisible() {
        try {
            root.style.display = window.__cw_canvas_watch_visible ? '' : 'none';
        } catch (_) {}
    }
    window.__cw_setCanvasWatchVisible = function (visible) {
        window.__cw_canvas_watch_visible = !!visible;
        applyCanvasWatchVisible();
        return window.__cw_canvas_watch_visible;
    };
    window.__cw_toggleCanvasWatchVisible = function () {
        return window.__cw_setCanvasWatchVisible(!window.__cw_canvas_watch_visible);
    };
    window.__cw_isCanvasWatchVisible = function () {
        return !!window.__cw_canvas_watch_visible;
    };
    ...
    try {
        __cw_tryPost({
            abx: 'cw_ui_state',
            state: window.__cw_canvas_watch_visible ? 'mounted_visible' : 'mounted_hidden',
            host: String(location.hostname || ''),
            href: String(location.href || ''),
            ts: Date.now()
        });
    ...
    function scan1000Text() {
        return scanText(1000);
    }
    ...
    console.log('[READY] CW merged (compat + TextMap + Scan1000Text + TK sequence + Totals by (x,tail) + standardized exports).');

    /* ---------------- tick & controls ---------------- */
    function statusByProg(p, textsHint) {
        var STATUS_TAIL = 'node_noti/lbl_noti';
        try {
            var texts = null;
            ...
            for (var i = 0; i < texts.length; i++) {
                ...
                if (!tailMatch(t.tail || '', STATUS_TAIL))
                    continue;
                var ar = (t.w || 0) * (t.h || 0);
                if (ar > bestArea) {
                    best = t;
    ...
    panel.querySelector('#bFindCdTail').onclick = function () {
        window.__cw_dumpCountdownTailCandidates(24);
    };
    panel.querySelector('#bFindCdDeep').onclick = function () {
        window.__cw_dumpCountdownRegionDeep();
    };
