...
using static SicboX88Live.MainWindow;
...
namespace SicboX88Live
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private const string AppLocalDirName = "SicboX88Live"; // đổi thành tên bạn muốn
        // ====== App paths ======
        ...
        private const string DEFAULT_URL = "https://net88.ceo/"; // URL mặc định bạn muốn
...
- Có thể nhập một phần danh sách (ví dụ chỉ TAI/XIU).";
        ...
        private record AppConfig
        {
            ...
            public string SideRateText { get; set; } = SicboX88Live.Tasks.SideRateParser.DefaultText;
        ...
        private sealed class StrategyTabState : INotifyPropertyChanged
        {
            ...
            public SicboX88Live.Tasks.IBetTask? ActiveTask { get; set; }
        ...
        private const string Wv2ZipResNameX64 = "SicboX88Live.ThirdParty.WebView2Fixed_win-x64.zip";
  ...
  try{
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        {
            // _appDataDir bạn đã tạo ở Startup: %LOCALAPPDATA%\SicboX88Live
            ...
            try
            {
                ...
                if (TxtSideRatio != null)
                {
                    var sideTxt = string.IsNullOrWhiteSpace(_cfg.SideRateText)
                        ? SicboX88Live.Tasks.SideRateParser.DefaultText
                        : _cfg.SideRateText;
        ...
        {
            return _strategyTabs.Any(t => t.IsRunning && t.ActiveTask is SicboX88Live.Tasks.JackpotMultiSideTask);
        }
                                        ...
                                        try
                                        {
                                                ...
                                                !string.Equals(seqStr, _baseSeq, StringComparison.Ordinal))
                                            {
                                                ...
                                                if (tailParity == 'C' || tailParity == 'L')
                                                {
                                                    ...
                                                    var winners = SicboX88Live.Tasks.SideRateParser.GetWinningSides(tail);
                                                    if (winners.Count == 0)
                                                        winners.Add(kqStr);
...
";
        ...
        {
            var def = SicboX88Live.Tasks.SideRateParser.DefaultText;
            if (TxtSideRatio != null)
        ...
        private GameContext BuildContext(StrategyTabState tab, bool useRawWinAmount = false)
        {
            ...
            return new GameContext
            {
                ...
                SideRateText = cfg.SideRateText ?? SicboX88Live.Tasks.SideRateParser.DefaultText,
        ...
        {
            StopAllTasks();
            SicboX88Live.Tasks.TaskUtil.ClearBetCooldown();
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                SicboX88Live.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new SicboX88Live.Tasks.SeqParityFollowTask(), // 1
                    1 => new SicboX88Live.Tasks.SeqTxFollowTask(), // 2
                    2 => new SicboX88Live.Tasks.PatternParityTask(), // 3
                    3 => new SicboX88Live.Tasks.PatternTxTask(), // 4
                    4 => new SicboX88Live.Tasks.SeqMajorMinorTask(), // 5
                    5 => new SicboX88Live.Tasks.SeqMajorMinorTxTask(), // 6
                    6 => new SicboX88Live.Tasks.PatternMajorMinorTask(), // 7
                    7 => new SicboX88Live.Tasks.PatternMajorMinorTxTask(), // 8
                    8 => new SicboX88Live.Tasks.SmartPrevTask(), // 9
                    9 => new SicboX88Live.Tasks.SmartPrevTxTask(), // 10
                    10 => new SicboX88Live.Tasks.RandomParityTask(), // 11
                    11 => new SicboX88Live.Tasks.RandomTxTask(), // 12
                    12 => new SicboX88Live.Tasks.AiStatParityTask(), // 13
                    13 => new SicboX88Live.Tasks.AiStatTxTask(), // 14
                    14 => new SicboX88Live.Tasks.StateTransitionBiasTask(), // 15
                    15 => new SicboX88Live.Tasks.StateTransitionBiasTxTask(), // 16
                    16 => new SicboX88Live.Tasks.RunLengthBiasTask(), // 17
                    17 => new SicboX88Live.Tasks.RunLengthBiasTxTask(), // 18
                    18 => new SicboX88Live.Tasks.EnsembleMajorityTask(), // 19
                    19 => new SicboX88Live.Tasks.EnsembleMajorityTxTask(), // 20
                    20 => new SicboX88Live.Tasks.TimeSlicedHedgeTask(), // 21
                    21 => new SicboX88Live.Tasks.TimeSlicedHedgeTxTask(), // 22
                    22 => new SicboX88Live.Tasks.KnnSubsequenceTask(), // 23
                    23 => new SicboX88Live.Tasks.KnnSubsequenceTxTask(), // 24
                    24 => new SicboX88Live.Tasks.DualScheduleHedgeTask(), // 25
                    25 => new SicboX88Live.Tasks.DualScheduleHedgeTxTask(), // 26
                    26 => new SicboX88Live.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 27
                    27 => new SicboX88Live.Tasks.AiOnlineNGramTxTask(GetAiNGramTxStatePath()), // 28
                    28 => new SicboX88Live.Tasks.AiExpertPanelTask(), // 29
                    29 => new SicboX88Live.Tasks.AiExpertPanelTxTask(), // 30
                    30 => new SicboX88Live.Tasks.Top10PatternFollowTask(), // 31
                    31 => new SicboX88Live.Tasks.Top10PatternFollowTxTask(), // 32
                    32 => new SicboX88Live.Tasks.SeqParityHotBackTask(), // 33
                    33 => new SicboX88Live.Tasks.SeqTxHotBackTask(), // 34
                    34 => new SicboX88Live.Tasks.JackpotMultiSideTask(), // 35
                    _ => new SicboX88Live.Tasks.SmartPrevTask(),
                };
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        // đặt trong MainWindow.xaml.cs (project SicboX88Live)
        ...
        private async Task<string> LoadAppJsAsyncFallback()
        {
            try
            {
                // Ưu tiên đọc từ đĩa để chỉnh JS khi dev không bắt buộc rebuild.
                foreach (var diskPath in EnumerateDevJsCandidates("v4_js_xoc_dia_live.js"))
                {
                    try
                    {
                        if (!File.Exists(diskPath)) continue;
                        var diskText = await File.ReadAllTextAsync(diskPath, Encoding.UTF8);
                        diskText = RemoveUtf8Bom(diskText);
                        if (!string.IsNullOrWhiteSpace(diskText))
                        {
                            Log($"[Bridge] Loaded JS from disk: {diskPath} (len={diskText.Length})");
                            return diskText;
                        }
                    }
                    catch { }
                ...
                var resName = FindResourceName("v4_js_xoc_dia_live.js")
                              ?? "SicboX88Live.v4_js_xoc_dia_live.js";
        ...
        private static IEnumerable<string> EnumerateDevJsCandidates(string fileName)
        {
            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            var yieldReturnHack = new List<string>();
            ...
            void Add(string? path)
            {
                ...
                try
                {
                    ...
                    if (seen.Add(full))
                        yieldReturnHack.Add(full);
                }
                catch { }
            }

            try { Add(Path.Combine(AppContext.BaseDirectory, fileName)); } catch { }
            try { Add(Path.Combine(Environment.CurrentDirectory, fileName)); } catch { }

            try
            {
                var dir = new DirectoryInfo(AppContext.BaseDirectory);
                for (int i = 0; i < 6 && dir != null; i++, dir = dir.Parent)
                    Add(Path.Combine(dir.FullName, fileName));
            }
            catch { }

            return yieldReturnHack;
        }
    ...
    TextNorm.U(Regex.Replace(raw ?? "", @"[,\s\-]+", "")); // bỏ , khoảng trắng, -
        ...
        private static bool ValidateSeqTX(string s, out string err)
        {
            ...
            foreach (var ch in s)
            {
                ...
                if (SicboX88Live.Tasks.TaskUtil.DigitToTx(ch) != '\0') { count++; continue; }
        ...
        private static bool ValidatePatternsTX(string s, out string err)
        {
            ...
            foreach (var raw in rules)
            {
                ...
                {
                    if (char.IsWhiteSpace(ch)) continue;
                    char tx = SicboX88Live.Tasks.TaskUtil.DigitToTx(ch);
        ...
        private bool ValidateInputsForCurrentStrategy()
        {
            ...
            {
                if (!SicboX88Live.Tasks.SideRateParser.TryParse(T(TxtSideRatio), out _, out var err))
                {
        ...
        private void SyncStrategyFieldsToUI()
        {
            ...
            if (idx == 34 && TxtSideRatio != null) TxtSideRatio.Text = _cfg.SideRateText ?? SicboX88Live.Tasks.SideRateParser.DefaultText;
        }
        ...
        private void ShowErrorsForCurrentStrategy()
        {
            ...
            if (idx == 34)
            {
                ...
                bool ok = SicboX88Live.Tasks.SideRateParser.TryParse(s, out _, out var e3);
