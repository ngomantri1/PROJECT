...
`BaccaratSexyCasino2` is the legacy WPF/WebView2 application. C# owns UI, authority selection, sequence/history and task execution; `v4_js_xoc_dia_live.js` collects provider data, exposes Canvas Watch and executes queued bets.
...
Continue with `ARCHITECTURE.md`, `MainWindow.xaml.cs`, `WebView2LiveBridge.cs`, `v4_js_xoc_dia_live.js` and `%LocalAppData%\\BaccaratSexyCasino2\\logs`.
