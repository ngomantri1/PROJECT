                ...
                case "WinUpLoseKeep":
                    {
                        ...
                        if (win == true)
                        {
                            // thắng => lên mức trước
                            step = (step + 1) % n;
                            Logger?.Invoke($"[S7] UpdateAfterRound: WIN => step {beforeStep} -> {step}");
                        }
                        else
                        {
                            // thua hoặc null => giữ nguyên mức
                            Logger?.Invoke($"[S7] UpdateAfterRound: win={win} => keep step={step}");
                        }
                ...
                case "WinUpLoseDown":
                    {
                        ...
                        {
                            step = (step + 1) % n;
                            Logger?.Invoke($"[S8] UpdateAfterRound: WIN => step {beforeStep} -> {step}");
                        }
                        else
                        {
                            step = Math.Max(0, step - 1);
                            Logger?.Invoke($"[S8] UpdateAfterRound: win={win} => step {beforeStep} -> {step}");
                        }
