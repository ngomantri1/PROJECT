...
<body><div class='note'>" + System.Net.WebUtility.HtmlEncode(note) + @"</div></body></html>";
  ...
  return (doneU?1:0)+(doneP?2:0);
            ...
            // Bấm đăng nhập sớm
            await TryAutoLoginAsync(120, force: true);
        }
    ...
    return 'ok';
        ...
        private async Task<bool> TryAutoLoginFromHomeAsync()
        {
            try
            {
                ...
                var res = await ClickLoginButtonAsync(8000); // hàm bạn đã có (không submit form)
        ...
        // ====== Auto-login runner ======
        private async Task TryAutoLoginAsync(int delayMs = 250, bool force = false)
        {
            try
            {
                var u = T(TxtUser);
        ...
        private async Task<string> ClickLoginButtonAsync(int timeoutMs = 20000)
        {
            await EnsureWebReadyAsync();
                        ...
                        if (need == "1")
                        {
                            ...
                            if (!_autoLoginBusy && (DateTime.UtcNow - _autoLoginLast).TotalSeconds > 3)
                            {
                                ...
                                try
                                {
                                    ...
                                    var res = await ClickLoginButtonAsync(15000);
