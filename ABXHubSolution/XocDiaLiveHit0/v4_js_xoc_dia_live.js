(() => {
    ...
    ========================================================= */
    // Cau hinh Canvas Watch bang bien SHOW_CANVAS_WATCH ben duoi.
    ...
    console.log('[CW] frame:', CW_FRAME_INFO, 'SHOW_CANVAS_WATCH=', SHOW_CANVAS_WATCH);
    try {
    ...
    /* ---------------- utils ---------------- */
    var V2 = (cc.v2 || cc.Vec2 || function (x, y) {
        return {
            x: x || 0,
            y: y || 0
        };
    ...
    /* ---------------- state & UI ---------------- */
    console.log('[CW] before state/ui:', CW_FRAME_INFO);
    var S = {
        running: false,
        timer: null,
        tickMs: 240,
    ...
    if (!document.body) {
        console.log('[CW] body not ready, skip panel create for now:', CW_FRAME_INFO);
        return;
    }
    ...
    // Canvas Watch an/hien theo SHOW_CANVAS_WATCH.
    ...
    window.__cw_panelRoot = root;
    window.__cw_showPanel = function (on) {
        SHOW_CANVAS_WATCH = (on !== false);
        root.style.display = SHOW_CANVAS_WATCH ? '' : 'none';
        return window.__cw_panelInfo();
    };
    window.__cw_panelInfo = function () {
        var pr = null;
        try {
            var r = panel.getBoundingClientRect();
            pr = {
    ...
    console.log('[CW] panel ready:', window.__cw_panelInfo());
