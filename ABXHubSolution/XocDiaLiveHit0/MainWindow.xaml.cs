...
using static XocDiaLiveHit0.MainWindow;
using System.Windows.Input;




namespace XocDiaLiveHit0
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private const string AppLocalDirName = "XocDiaLiveHit0"; // đổi thành tên bạn muốn
        // ====== App paths ======
        ...
        private XocDiaLiveHit0.Tasks.GameContext? _runningLegacyContext;
...
• Ví dụ: 150000 (khi lỗ ≥ 150.000đ thì dừng).";
        ...
        @"ĐẶT KHI CÒN GIÂY
• Nhập số giây còn lại trong phiên, từ 1 đến 45.
• Ý nghĩa: chỉ đặt cược khi thời gian còn lại ≤ giá trị này.
...
• Nhập số giây còn lại trong phiên, từ 1 đến 45.
• Nên để thấp để bám sát dòng tiền hai cửa.
• Ví dụ: 10 = đặt khi còn 10 giây hoặc ít hơn.";
...
- Có thể nhập một phần danh sách (ví dụ chỉ 4DO/4TRANG hoặc SAPDOI/CHAN/LE).";
        ...
        private record AppConfig
        {
            ...
            public string SideRateText { get; set; } = XocDiaLiveHit0.Tasks.SideRateParser.DefaultText;
        ...
        private sealed class StrategyTabState : INotifyPropertyChanged
        {
            ...
            public XocDiaLiveHit0.Tasks.IBetTask? ActiveTask { get; set; }
            public XocDiaLiveHit0.Tasks.GameContext? RunningContext { get; set; }
        ...
        private const string Wv2ZipResNameX64 = "XocDiaLiveHit0.ThirdParty.WebView2Fixed_win-x64.zip";
  ...
  try{
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        {
            // _appDataDir bạn đã tạo ở Startup: %LOCALAPPDATA%\XocDiaLiveHit0
        ...
        private void UpdateAccountHeaderFromGame(string? username, long? amount)
        {
            ...
            {
                if (amount.HasValue)
                    LblAmount.Text = amount.Value.ToString("N0", System.Globalization.CultureInfo.InvariantCulture);
            }
        }
        ...
        private void LoadConfig_Legacy()
        {
            try
            {
                ...
                {
                    var sideTxt = string.IsNullOrWhiteSpace(_cfg.SideRateText) ? XocDiaLiveHit0.Tasks.SideRateParser.DefaultText : _cfg.SideRateText;
                                ...
                                if (abxStr == "tick")
                                {
                                    ...
                                    if (snap != null)
                                    {
                                        ...
                                        try
                                        {
                                                ...
                                                !string.Equals(seqStr, _baseSeq, StringComparison.Ordinal))
                                            {
                                                ...
                                                if (_pendingRows.Count > 0 && accNow2.HasValue)
                                                {
                                                    ...
                                                    if (_activeTask is not XocDiaLiveHit0.Tasks.JackpotMultiSideTask)
                                                    {
                                        ...
                                        _ = Dispatcher.BeginInvoke(new Action(() =>
                                        {
                                            try
                                            {
                                                // Progress / giây còn lại
                                                if (snap.prog.HasValue)
                                                {
                                                    const double maxSec = 45.0;
                                                    var p = Math.Max(0, Math.Min(1, snap.prog.Value));
                                                    var sec = snap.progSec.HasValue
                                                        ? Math.Clamp(snap.progSec.Value, 0, (int)maxSec)
                                                        : (int)Math.Round(p * maxSec, MidpointRounding.AwayFromZero);
                                                    if (PrgBet != null)
                                                    {
                                                        if (Math.Abs(PrgBet.Maximum - maxSec) > 0.001) PrgBet.Maximum = maxSec;
                                                        PrgBet.Value = sec;
                                                    }
                                                    if (LblProg != null) LblProg.Text = $"{sec}s";
                                                }
                                                else
                                                {
                                                ...
                                                // Tổng tiền/tên nhân vật: lấy từ model, fallback trực tiếp JSON tick.
                                                var amt = snap?.totals?.A;
                                                if (!amt.HasValue &&
                                                    jrootTick.TryGetProperty("totals", out var totalsEl) &&
                                                    totalsEl.TryGetProperty("A", out var accEl) &&
                                                    accEl.ValueKind == JsonValueKind.Number &&
                                                    accEl.TryGetInt64(out var accJson))
                                                {
                                                    amt = accJson;
                                                }
                                                ...
                                                if (string.IsNullOrWhiteSpace(uname) &&
                                                    jrootTick.TryGetProperty("username", out var userEl))
                                                {
                                                    uname = userEl.GetString() ?? "";
                                                }
...
<body><div class='note'>" + System.Net.WebUtility.HtmlEncode(note) + @"</div></body></html>";
  ...
  try{
    ...
    return 'ok';
        ...
        // Bấm 'Chơi Xóc Đĩa Live' từ Home bằng nhánh C# trực tiếp.
        ...
        async void CmbMoneyStrategy_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (!_uiReady || _tabSwitching) return;
            _cfg.MoneyStrategy = GetMoneyStrategyFromUI();
            XocDiaLiveHit0.Tasks.MoneyHelper.ResetTempProfitForWinUpLoseKeep();
...
";
        ...
        {
            var def = XocDiaLiveHit0.Tasks.SideRateParser.DefaultText;
            if (TxtSideRatio != null)
        ...
        private GameContext BuildContext(StrategyTabState tab, bool useRawWinAmount = false)
        {
            ...
            return new GameContext
            {
                ...
                SideRateText = cfg.SideRateText ?? XocDiaLiveHit0.Tasks.SideRateParser.DefaultText,
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                _cfg.BetPatterns = (idx == 1) ? (_cfg.BetPatternsCL ?? "") : (idx == 3 ? (_cfg.BetPatternsNI ?? "") : "");
                ...
                XocDiaLiveHit0.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new XocDiaLiveHit0.Tasks.SeqParityFollowTask(),
                    1 => new XocDiaLiveHit0.Tasks.PatternParityTask(),
                    2 => new XocDiaLiveHit0.Tasks.SeqMajorMinorTask(),
                    3 => new XocDiaLiveHit0.Tasks.PatternMajorMinorTask(),
                    4 => new XocDiaLiveHit0.Tasks.SmartPrevTask(),
                    5 => new XocDiaLiveHit0.Tasks.RandomParityTask(),
                    6 => new XocDiaLiveHit0.Tasks.AiStatParityTask(),
                    7 => new XocDiaLiveHit0.Tasks.StateTransitionBiasTask(),
                    8 => new XocDiaLiveHit0.Tasks.RunLengthBiasTask(),
                    9 => new XocDiaLiveHit0.Tasks.EnsembleMajorityTask(),
                    10 => new XocDiaLiveHit0.Tasks.TimeSlicedHedgeTask(),
                    11 => new XocDiaLiveHit0.Tasks.KnnSubsequenceTask(),
                    12 => new XocDiaLiveHit0.Tasks.DualScheduleHedgeTask(),
                    13 => new XocDiaLiveHit0.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()),
                    14 => new XocDiaLiveHit0.Tasks.AiExpertPanelTask(),
                    15 => new XocDiaLiveHit0.Tasks.Top10PatternFollowTask(),
                    16 => new XocDiaLiveHit0.Tasks.JackpotMultiSideTask(),
                    _ => new XocDiaLiveHit0.Tasks.SmartPrevTask(),
                };
        ...
        private void StopXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (!IsAnyTabRunning())
                {
                    XocDiaLiveHit0.Tasks.TaskUtil.ClearBetCooldown();
                    Log("[Loop] stopped");
                    StopExpiryCountdown();
                    StopLeaseHeartbeat();
        ...
        private GameContext BuildContext_Legacy(bool useRawWinAmount = false)
        {
            ...
            return new GameContext
            {
                ...
                SideRateText = _cfg.SideRateText ?? XocDiaLiveHit0.Tasks.SideRateParser.DefaultText,
                UseRawWinAmount = useRawWinAmount,
                BetSeq = _cfg.BetSeq ?? "",
                ...
                UiAddWin = delta =>
                {
                    void Apply()
                    {
                        ...
                        try
                        {
                            XocDiaLiveHit0.Tasks.MoneyHelper.NotifyTempProfit(moneyStrategyId, net);
                        }
                        catch { /* ignore */ }
        ...
        private async void PlayXocDia_Click_Legacy(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                XocDiaLiveHit0.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    0 => new XocDiaLiveHit0.Tasks.SeqParityFollowTask(),     // 1
                    1 => new XocDiaLiveHit0.Tasks.PatternParityTask(),       // 2
                    2 => new XocDiaLiveHit0.Tasks.SeqMajorMinorTask(),       // 3
                    3 => new XocDiaLiveHit0.Tasks.PatternMajorMinorTask(),   // 4
                    4 => new XocDiaLiveHit0.Tasks.SmartPrevTask(),           // 5
                    5 => new XocDiaLiveHit0.Tasks.RandomParityTask(),        // 6
                    6 => new XocDiaLiveHit0.Tasks.AiStatParityTask(),        // 7
                    7 => new XocDiaLiveHit0.Tasks.StateTransitionBiasTask(), // 8
                    8 => new XocDiaLiveHit0.Tasks.RunLengthBiasTask(),       // 9
                    9 => new XocDiaLiveHit0.Tasks.EnsembleMajorityTask(),    // 10
                    10 => new XocDiaLiveHit0.Tasks.TimeSlicedHedgeTask(),    // 11
                    11 => new XocDiaLiveHit0.Tasks.KnnSubsequenceTask(),     // 12
                    12 => new XocDiaLiveHit0.Tasks.DualScheduleHedgeTask(),  // 13
                    13 => new XocDiaLiveHit0.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 14
                    14 => new XocDiaLiveHit0.Tasks.AiExpertPanelTask(), // 15
                    15 => new XocDiaLiveHit0.Tasks.Top10PatternFollowTask(), // 16
                    16 => new XocDiaLiveHit0.Tasks.JackpotMultiSideTask(), // 17
                    _ => new XocDiaLiveHit0.Tasks.SmartPrevTask(),
                };
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        // đặt trong MainWindow.xaml.cs (project XocDiaLiveHit0)
        ...
        private void EnsureDeviceId()
        {
            ...
            {
                // Safe fallback to avoid blocking login flow
        ...
        private static string? ReadWmiValue(string query, string propName)
        {
            try
            {
                // Use reflection so missing System.Management does not crash plugin runtime
        ...
        private async Task<string> LoadAppJsAsyncFallback()
        {
            try
            {
                ...
                var resName = FindResourceName("v4_js_xoc_dia_live.js")
                              ?? "XocDiaLiveHit0.v4_js_xoc_dia_live.js";
        ...
        private async void TxtDecisionSecond_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (!_uiReady || _tabSwitching || _cfg == null) return;
    ...
    TextNorm.U(Regex.Replace(raw ?? "", @"[,\s\-]+", "")); // bỏ , khoảng trắng, -
        ...
        private bool ValidateInputsForCurrentStrategy()
        {
            ...
            {
                if (!XocDiaLiveHit0.Tasks.SideRateParser.TryParse(T(TxtSideRatio), out _, out var err))
                {
        ...
        private void SyncStrategyFieldsToUI()
        {
            ...
            if (idx == 16 && TxtSideRatio != null) TxtSideRatio.Text = _cfg.SideRateText ?? XocDiaLiveHit0.Tasks.SideRateParser.DefaultText;
        }
        ...
        private void ShowErrorsForCurrentStrategy()
        {
            ...
            if (idx == 16)
            {
                ...
                bool ok = XocDiaLiveHit0.Tasks.SideRateParser.TryParse(s, out _, out var e3);
