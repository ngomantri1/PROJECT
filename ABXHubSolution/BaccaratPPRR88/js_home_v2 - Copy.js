(function () {
    ...
    // === ABX log hook: mirror console.* -> host (WebView2) để ghi file ===
    (function () {
        if (window.__abxLogHooked)
            return;
        window.__abxLogHooked = 1;

        var rawLog = console.log ? console.log.bind(console) : function () {};
        var rawInfo = console.info ? console.info.bind(console) : rawLog;
        var rawDebug = console.debug ? console.debug.bind(console) : rawLog;
        var rawWarn = console.warn ? console.warn.bind(console) : rawLog;
        var rawErr = console.error ? console.error.bind(console) : rawLog;
        var rawTable = console.table ? console.table.bind(console) : rawLog;
        var rawTrace = console.trace ? console.trace.bind(console) : rawErr;

        function serializeForHost(v) {
            try {
                if (v === undefined)
                    return "undefined";
                if (v === null)
                ...
                if (t === "object") {
                    // object/array -> JSON, cắt bớt cho an toàn
                    var s;
                    try {
                        s = JSON.stringify(v);
                    } catch (e) {
                        s = Object.prototype.toString.call(v); // [object Object]...
                    }
                    if (s && s.length > 800) {
                        s = s.slice(0, 800) + " …(+ " + (s.length - 800) + " chars)";
                    }
                    return s;
                }
                return String(v);
        ...
        function sendToHost(level, args) {
            try {
                ...
                for (var i = 0; i < args.length; i++) {
                    parts.push(serializeForHost(args[i]));
                }
                ...
                wv.postMessage(JSON.stringify(payload));
            } catch (e) {
                // im lặng nếu log bị lỗi
            }
        }

        function hookConsole(level, rawFn) {
            return function () {
                // ❌ Không mirror console.table lên C# để tránh log khổng lồ
                if (level !== "table") {
                    sendToHost(level, arguments);
                }
                try {
                    return rawFn.apply(console, arguments);
                } catch (e) {
                    // nuốt lỗi
                }
            };
        ...
        console.log = hookConsole("log", rawLog);
        console.info = hookConsole("info", rawInfo);
        console.debug = hookConsole("debug", rawDebug);
        console.warn = hookConsole("warn", rawWarn);
        console.error = hookConsole("error", rawErr);
        // DevTools vẫn hiển thị table bình thường, chỉ không gửi lên C#
        console.table = hookConsole("table", rawTable);
        console.trace = hookConsole("trace", rawTrace);
        // Bắt lỗi runtime sớm (thay cho DevTools)
    ...
    // === Detect env (stub): KHÔNG check cc / canvas nữa, chỉ log href ===
    function abxDetectEnv(label) {
        try {
            console.log('[ABX] Engine detect (stub)', label || '', {
                href: location.href
            });
        } catch (e) {
            // ignore
    ...
    // gọi auto khi js_home_v2 được inject (chỉ log nhẹ, không scan gì thêm)
    abxDetectEnv('on-load');
    setTimeout(function () {
        abxDetectEnv('after-3s');
    }, 3000);
    ...
    const CFG = {
        ...
        closePopupBtnId: 'bclosepopup',
        scanClosePopupBtnId: 'bscanclose200',
    ...
    const TAIL_LOGIN_BTN =
        'div.header_title[1]/div.header_top[1]/div.wrapper[1]/div.right-wrap[2]/div.right-wrap-top[1]/div.hd_login[1]/div.form_item[1]/span.submit_btn[1]';

    // NEW: tail root của popup đăng nhập (lấy từ Scan200ClosePopup)
    ...
    // NEW: danh sách tail các popup/thông báo cần auto tắt (CHỈ popup quảng cáo, KHÔNG gồm popup đăng nhập)
    const CLOSE_POPUP_ROOT_TAILS = [
        ...
        // Overlay bọc ngoài (đề phòng trường hợp chỉ còn overlay)
    ...
    // NEW: kiểm tra popup đăng nhập đã hiển thị hay chưa
    function isLoginPopupVisible() {
        try {
            // Ưu tiên tail lấy từ Scan200ClosePopup
            if (TAIL_LOGIN_POPUP_ROOT) {
                const root = findByTail(TAIL_LOGIN_POPUP_ROOT);
                if (root && root.offsetParent !== null)
                    return true;
            }

            // Fallback: theo class loginPopupModal
            const el = document.querySelector('.tcg_modal_wrap.loginPopupModal, .loginPopupModal');
    ...
    // NEW: quản lý vòng lặp auto click nút Đăng nhập
    function stopLoginPump() {
    ...
    function ensureLoginPump() {
        if (S.loginPopupTimer)
            return;

        S.loginPopupTimer = setInterval(() => {
            try {
                // Nếu đã hiện popup đăng nhập hoặc đã login thì dừng
                ...
                console.log('[HomeWatch] auto click nút Đăng nhập (pump)');
            } catch (_) {
                // bỏ qua lỗi nhỏ
            }
        }, 1500); // 1.5s thử lại 1 lần
    }

    // NEW: nghiệp vụ chính click nút Đăng nhập (được dùng bởi nút "Click Đăng Nhập" + C#)
    function clickLoginButton() {
        try {
            // Nếu popup login đã hiện thì KHÔNG click nữa
            if (isLoginPopupVisible()) {
                updateInfo('Popup đăng nhập đã hiển thị, không click nút Đăng nhập nữa.');
                stopLoginPump();
                return;
            }
            ...
            const btn = findLoginButton();
            if (!btn) {
                updateInfo('Không tìm thấy nút Đăng nhập trên trang.');
                return;
            }

            // Click 1 lần giống người dùng
            btn.dispatchEvent(new MouseEvent('click', {
                    bubbles: true,
                    cancelable: true
                }));
            updateInfo('Đã click nút Đăng nhập. Sẽ tiếp tục thử cho tới khi popup xuất hiện.');
            // Bật loop thử lại nếu host tiếp tục yêu cầu
            ensureLoginPump();
        } catch (e) {
    ...
    function ensureRoot() {
        ...
        // NEW: bật vòng lặp tự động tắt popup quảng cáo
        ensurePopupAutoCloser();
    }
    ...
    // NEW: đóng tất cả popup quảng cáo theo danh sách CLOSE_POPUP_ROOT_TAILS
    function closeKnownPopups() {
        try {
            CLOSE_POPUP_ROOT_TAILS.forEach(tail => {
                if (!tail)
                    return;

                const root = findByTail(tail);
                if (!root || !root.isConnected)
                    return;

                // Ưu tiên click nút đóng nếu tìm được
                let target = root.querySelector(
                        'button.close, .popup-close, .btn-close, .v--modal-close-button,' +
                        '.announce-popup-container .icon-close, .announce-popup-container .popup-close,' +
                        '.popup-bg .close, .popup-bg .btn-close');

                // Nếu không có nút, thử click vùng nền tắt popup
                if (!target) {
                    target = root.querySelector('.v--modal-background-click, .popup-bg');
                }
                ...
                if (target) {
                    ...
                    // Fallback cuối: gỡ hẳn overlay khỏi DOM
                    const overlay = root.closest('.v--modal-overlay');
            ...
            // im lặng nếu có lỗi nhỏ
        }
    }

    // NEW: loop định kỳ để cứ thấy popup quảng cáo là tắt
    function ensurePopupAutoCloser() {
        if (popupAutoTimer)
            return;
        popupAutoTimer = setInterval(closeKnownPopups, 1200); // ~1.2s kiểm tra 1 lần
    }
    ...
    // NEW: scan popup đang che màn hình (quảng cáo, modal, login...)
    function collectPopups() {
        ...
        document.querySelectorAll('body *').forEach(el => {
            ...
            // nhỏ quá và không nằm vùng giữa màn hình thì bỏ
    ...
    function renderPopups() {
        ...
        if (!ov)
            return;
        const list = getOrdered('popup');
        S.items.popup = list;
        list.forEach(it => {
            ov.appendChild(mkBox(it, 'popup'));
        });
    }
    window.__abx_overlay_rerender = function () {
        try {
            clearOverlay();
            if (S.showL)
                renderLinks();
    ...
    // format thông tin 1 phần tử để dùng chung cho panel + log scan
    function formatScanItem(kind, item) {
        try {
            ...
            const r = rectOf(el);
            const txt = (item && item.text)
             ? clip(item.text, kind === 'text' ? 180 : 140)
             : clip(textOf(el), kind === 'text' ? 180 : 140);
    ...
    function showInfo(kind, item) {
        try {
            const info = formatScanItem(kind, item);
            if (info)
                updateInfo(info);
        } catch (_) {}
    }
    ...
    function getOrdered(kind) {
        let list = (S.items[kind] && S.items[kind].length)
         ...
         : assignOrder((
                kind === 'link'
                 ? collectLinks()
                 : (kind === 'text' ? collectTexts() : collectPopups())).sort(smallFirst));
    ...
    function scanLinks(limit) {
        ...
        const list = all.slice(0, Math.max(1, Math.min(limit || 500, all.length)));

        console.group('[link] Scan ' + list.length + '/' + all.length);
        list.forEach(it => {
            const info = formatScanItem('link', it);
            if (info)
                console.log('Idx  : ' + it.ord + '\n' + info);
        });

        // bảng cho DevTools (KHÔNG gửi lên C# nữa vì đã chặn ở hookConsole)
        try {
            console.table(list.map(it => {
                    const r = rectOf(it.el);
                    return {
                        index: it.ord,
                        tag: (it.tag || (it.el.tagName || '')).toString().toLowerCase(),
                        class: (it.el.className || '').toString(),
        ...
        // ✅ NEW: gửi data scan sang host để ghi log rõ ràng
        try {
            var wv = window.chrome && window.chrome.webview;
            if (wv && typeof wv.postMessage === 'function') {
                var rows = list.map(function (it) {
                    ...
                    return {
                        ...
                        kind: 'link',
                        tag: (it.tag || (it.el.tagName || '')).toString().toLowerCase(),
                        cls: (it.el.className || '').toString(),
                        text: clip(textOf(it.el), 80),
                        href: (it.el.getAttribute('href') || ''),
                        src: 'visible',
                        x: r.x,
                        y: r.y,
    ...
    function scanTexts(limit) {
        ...
        list.forEach(it => {
            const info = formatScanItem('text', it);
            if (info)
                console.log('Idx  : ' + it.ord + '\n' + info);
        });

        // bảng cho DevTools (KHÔNG gửi lên C#)
        try {
            console.table(list.map(it => {
                    const r = rectOf(it.el);
                    return {
            ...
            if (wv && typeof wv.postMessage === 'function') {
                var rows = list.map(function (it) {
                    ...
                    return {
                        ...
                        text: clip(it.text || textOf(it.el), 80),
                        src: it.src || 'visible',
                        href: (it.el.getAttribute && it.el.getAttribute('href')) || '',
                        x: r.x,
                        y: r.y,
                        w: r.w,
                        h: r.h,
    ...
    function scanClosePopups(limit) {
        ...
        list.forEach(it => {
            const info = formatScanItem('popup', it);
            if (info)
                console.log('Idx  : ' + it.ord + '\n' + info);
        });
        ...
        // gửi sang C# giống Scan200Links/Text
        try {
            var wv = window.chrome && window.chrome.webview;
            if (wv && typeof wv.postMessage === 'function') {
                var rows = list.map(function (it) {
    ...
    // Tìm root popup từ tail, có fallback theo class (publicModal, ...)
    function findPopupRootFromTail(tail) {
        ...
        // 2) Fallback: lấy đoạn cuối cùng của tail -> selector theo class
        try {
            const parts = String(tail).split('/').filter(Boolean);
            const last = parts[parts.length - 1] || '';
            // ví dụ: "div.tcg_modal_wrap.publicModal[1]" -> "div.tcg_modal_wrap.publicModal"
        ...
        // 3) Fallback đặc biệt cho popup quảng cáo RR88 (publicModal)
        try {
    ...
    function closeKnownPopupRoots() {
        ...
        CLOSE_POPUP_ROOT_TAILS.forEach((tail) => {
            try {
                ...
                // Ưu tiên nút close quen thuộc bên trong popup
                let target = root.querySelector(
                        '.v--modal-close, .modal__close, .modal-close, .btn-close, .close, [data-action="close"]');

                // Nếu chưa có thì quét các button bên trong, ưu tiên text Đóng / Tắt / Close / OK / Xác nhận
                if (!target) {
                    const btns = root.querySelectorAll('button, .base-button.btn, [role="button"], a');
                ...
                // Nếu vẫn chưa có -> click nền overlay (v--modal-background-click)
                if (!target) {
                    let p = root;
    ...
    function closeAdsAndCovers() {
        tryCloseCommonOverlays();
        closeKnownPopupRoots(); // auto đóng các popup đã biết (RR88 publicModal,...)
        // Không giữ ẩn quá lâu tại trang Home, chỉ peel khi click
    }

    // NEW: login popup helpers
    function isLoginPopupVisible() {
        try {
            ...
            const sel = '.popup-login, .login-popup, .modal-login, .login__popup, .login-popup-wrap, .login-form-popup, .hd_login.active, .loginPopupModal';
    ...
    // NEW: chuẩn hoá tìm nút đăng nhập trên header
    function findLoginHeaderButton() {
        ...
        if (!btn) {
            // Ưu tiên layout RR88: div.header .hd_login .submit_btn
            btn = document.querySelector(
                    'div.header .hd_login .submit_btn, header .hd_login .submit_btn, header.menu .user-not-login .base-button.btn');
        }
    ...
    // nếu chỗ khác trong file có dùng findLoginButton() thì tự động trỏ về bản mới
    function findLoginButton() {
        return findLoginHeaderButton();
    }

    // NEW: click 1 lần, dùng chung cho auto-click
    function clickLoginButtonOnce() {
        const btn = findLoginHeaderButton();
    ...
    // NEW: chỉ setup probe sau khi đã bắt đầu click login
    ...
    // NEW: quản lý timer auto click login
    ...
    function startLoginAutoClick() {
        // nếu popup đã mở thì khỏi auto-click
        if (isLoginPopupVisible()) {
            stopLoginAutoClick('Popup đăng nhập đã hiển thị — không cần auto click.');
            return;
        }

        // bóc bớt quảng cáo / cover trước khi click
        closeAdsAndCovers();
        ...
        const tick = () => {
            if (isLoginPopupVisible()) {
                stopLoginAutoClick('Popup đăng nhập đã hiển thị — dừng auto click nút "Đăng nhập".');
                return;
            }
            ...
            clickLoginButtonOnce();
        };

        tick(); // bắn phát đầu tiên
        S.loginPopupTimer = setInterval(tick, 1200);
        updateInfo('Đang auto click nút "Đăng nhập" cho tới khi popup login xuất hiện (tối đa ' + Math.round(MAX_MS / 1000) + 's)...');
    }
    ...
    // NEW: setup timer auto tắt popup/thông báo theo danh sách tail
