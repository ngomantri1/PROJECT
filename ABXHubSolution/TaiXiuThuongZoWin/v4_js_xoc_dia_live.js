...
/** pick first item by tail (sorted by y,x) */
        ...
        function chanLeDebugLines(list, title) {
            ...
            var lines = ['(' + (title || 'Tai/Xiu candidates by tail') + ') idx\ttxt\tval\tx\ty\tsx\tsy\tsw\tsh'];
            ...
            if (!arr.length) {
                ...
                lines.push('pick TAI(minX): ' + Math.round(xOf(min)) + " -> '" + min.txt + "'");

                lines.push('pick XIU(maxX): ' + Math.round(xOf(max)) + " -> '" + max.txt + "'");
    ...
    function updatePanel() {
        ...
        var base =
            ...
            '• TÊN NHÂN VẬT : ' + (t.N != null && String(t.N).trim() ? String(t.N).trim() : '--') + '|TK : ' + fmt(t.A) + '|TÀI: ' + fmt(t.T) + '|SẤP ĐÔI: ' + fmt(t.SD) + '|XỈU :' + fmt(t.X) + '|TỨ TRẮNG: ' + fmt(t.TT) + '|3 TRẮNG: ' + fmt(t.T3T) + '|3 ĐỎ: ' + fmt(t.T3D) + '|TỨ ĐỎ: ' + fmt(t.TD) + '\n' +
    ...
    function scan200Money() {
        ...
        console.log(money);
        debugChanLeLog(moneyRaw, 'Tai/Xiu by MoneyMap');
    }
    function scan200Bet() {
        var btns = collectButtons().filter(function (b) {
            return b.w >= 16 && b.h >= 12;
    ...
    function scan200Text() {
        var texts = buildTextRects().slice(0, 200)
         ...
         if (!texts.length)
             lines.push('(empty)');
         lines.push('');
         lines = lines.concat(chanLeDebugLines(buildMoneyFromTextRects(), 'Tai/Xiu by TextMap'));
        setCwLog(lines.join('\n'));
    ...
    function normalizeSide(raw) {
        ...
        if (!s)
            return 'XIU'; // giữ nguyên hành vi cũ: không nhận => mặc định Xỉu
        if (s === 'TAI' || s === 'EVEN')
            return 'TAI';
        if (s === 'XIU' || s === 'ODD')
            return 'XIU';
    ...
    var BET_TAILS = {
        TAI: 'dual/Canvas/node_dual/root/node_game(need_to_put_games_in_here)/prefab_game_14/root/node_general(use_in_both_mode)/table/bet_entries/ig_xocdia_chan',
        XIU: 'dual/Canvas/node_dual/root/node_game(need_to_put_games_in_here)/prefab_game_14/root/node_general(use_in_both_mode)/table/bet_entries/ig_xocdia_le',
            ...
            try {
                ...
                _pushTimer = setInterval(function () {
                    ...
                    var st = (typeof statusByProg === 'function') // tính status theo rule mới
                     ? statusByProg(p) : '';
                    var t = readTotalsSafe();
                    var snap = {
                        abx: 'tick',
                        prog: p,
                        totals: t,
                        username: (t && t.N != null) ? String(t.N) : null,
                        seq: readSeqSafe(),
