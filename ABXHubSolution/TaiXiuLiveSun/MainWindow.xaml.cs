...
namespace TaiXiuLiveSun
{
    ...
    {
        private static ImageSource? LoadByResourceOrPack(string resKey, string relativePath)
        {
            try
            {
                if (System.Windows.Application.Current?.Resources != null &&
                    System.Windows.Application.Current.Resources[resKey] is ImageSource resImg)
                    return resImg;
            }
            catch { }
            return FallbackIcons.LoadPackImage(relativePath);
        }

        public object Convert(object value, Type t, object p, CultureInfo c)
        {
            var u = TextNorm.U(value?.ToString() ?? "");
            if (u == "TAI" || u == "T" || u == "BIG") return LoadByResourceOrPack("ImgTAI", "Assets/side/TAI.png");
            if (u == "XIU" || u == "X" || u == "SMALL") return LoadByResourceOrPack("ImgXIU", "Assets/side/XIU.png");
    ...
    public sealed class KetQuaToIconConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type t, object p, CultureInfo c)
        {
            var u = TextNorm.U(value?.ToString() ?? "");
            if (u == "TAI" || u == "T" || u == "BIG")
                return FallbackIcons.LoadPackImage("Assets/side/TAI.png");
            if (u == "XIU" || u == "X" || u == "SMALL")
                return FallbackIcons.LoadPackImage("Assets/side/XIU.png");
...
- Dãy mặc định đầy đủ:
  ...
  CHAN:1
  LE:1
- Có thể nhập một phần danh sách (ví dụ chỉ TAI/XIU).";
        // =========================================================
        ...
        private record AppConfig
        {
            ...
            public int StrategySchemaVersion { get; set; } = 2;
            public int BetStrategyIndex { get; set; } = 8; // mặc định "9. Bám cầu trước (thông minh) cho chẵn lẻ"
            ...
            public string BetSeqTX { get; set; } = "";        // cho Chiến lược 2
            public string BetSeqNI { get; set; } = "";        // cho Chiến lược 5
            public string BetSeqNITX { get; set; } = "";      // cho Chiến lược 6
            public string BetPatternsCL { get; set; } = "";   // cho Chiến lược 3
            public string BetPatternsTX { get; set; } = "";   // cho Chiến lược 4
            public string BetPatternsNI { get; set; } = "";   // cho Chiến lược 7
            public string BetPatternsNITX { get; set; } = ""; // cho Chiến lược 8
        ...
        // Map ảnh KẾT QUẢ theo mã số mới: 0/1/2/3
        private readonly Dictionary<char, ImageSource> _resultDigitIconMap = new();
  ...
  try{
    ...
    (function tick(){
      ...
      setTimeout(tick, delay);
        ...
        private string GetAiNGramTxStatePath()
        {
            var aiDir = System.IO.Path.Combine(_appDataDir, "ai");
            System.IO.Directory.CreateDirectory(aiDir);
            return System.IO.Path.Combine(aiDir, "ngram_state_tx_v1.json");
        }
        ...
        // ====== Config I/O ======
        private static int MigrateOldStrategyIndexToNew(int oldIdx)
        {
            return oldIdx switch
            {
                0 => 0,   // 1) Seq CL
                1 => 2,   // 2) Pattern CL
                2 => 4,   // 3) Seq NI CL
                ...
                5 => 10,  // 6) Random CL
                6 => 12,  // 7) AI Stat CL
                7 => 14,  // 8) State Transition CL
                8 => 16,  // 9) RunLength CL
                9 => 18,  // 10) Ensemble CL
                10 => 20, // 11) TimeSliced CL
                11 => 22, // 12) KNN CL
                12 => 24, // 13) DualSchedule CL
                13 => 26, // 14) AI NGram CL
                14 => 28, // 15) AI15 CL
                15 => 30, // 16) Top10 CL
                16 => 34, // 17) Jackpot
                17 => 32, // 18) SeqHot CL
        ...
        private static void MigrateStrategySchema(AppConfig cfg)
        {
            if (cfg == null) return;
            if (cfg.StrategySchemaVersion >= 2) return;

            cfg.BetStrategyIndex = MigrateOldStrategyIndexToNew(cfg.BetStrategyIndex);
            cfg.StrategySchemaVersion = 2;
        }
        ...
        private void ApplyActiveTabToUi()
        {
            ...
            try
            {
                ...
                if (CmbBetStrategy != null)
                    CmbBetStrategy.SelectedIndex = (_cfg.BetStrategyIndex >= 0 && _cfg.BetStrategyIndex <= 34) ? _cfg.BetStrategyIndex : 8;
                                    ...
                                    if (snap != null)
                                    {
                                        ...
                                        try
                                        {
                                                ...
                                                !string.Equals(seqStr, _baseSeq, StringComparison.Ordinal))
                                            {
                                                ...
                                                if (tailParity == 'C' || tailParity == 'L')
                                                {
                                                    ...
                                                    var winners = TaiXiuLiveSun.Tasks.SideRateParser.GetWinningSides(tail);
                                                    if (winners.Count == 0)
                                                        winners.Add(kqStr);
                                        ...
                                        _ = Dispatcher.BeginInvoke(new Action(() =>
                                        {
                                            try
                                            {
                                                ...
                                                // Kết quả gần nhất từ chuỗi seq (mã số 0/1/2/3)
                                                var seqStrLocal = snap.seq ?? "";
                                                char last = (seqStrLocal.Length > 0) ? seqStrLocal[^1] : '\0';
                                                SetLastResultUI(last != '\0' ? last.ToString() : "");
...
<body><div class='note'>" + System.Net.WebUtility.HtmlEncode(note) + @"</div></body></html>";
    ...
    return 'ok';
        ...
        private void UpdateTooltips()
        {
            ...
            AttachTip(TxtChuoiCau,
                (idx == 0 || idx == 1) ? TIP_SEQ_CL :
                (idx == 4 || idx == 5) ? TIP_SEQ_NI :
                "Chọn chiến lược 1, 2, 5 hoặc 6 để nhập Chuỗi cầu.");

            AttachTip(TxtTheCau,
                (idx == 2 || idx == 3) ? TIP_THE_CL :
                (idx == 6 || idx == 7) ? TIP_THE_NI :
                "Chọn chiến lược 3, 4, 7 hoặc 8 để nhập Thế cầu.");
            // ==== BẮT ĐẦU: Tooltip cho chiến lược đặt cược ====
            string tip = GetStrategyTooltipText(idx);
        ...
        private static string GetStrategyTooltipText(int idx)
        {
            return idx switch
            {
                ...
                2 => "3) Thế cầu C/L.",
                3 => "4) Thế cầu T/X.",
                4 => "5) Chuỗi cầu I/N cho chẵn/lẻ.",
                5 => "6) Chuỗi cầu I/N cho tài/xỉu.",
                6 => "7) Thế cầu I/N cho chẵn/lẻ.",
                7 => "8) Thế cầu I/N cho tài/xỉu.",
                8 => "9) Bám cầu trước cho chẵn/lẻ.",
                9 => "10) Bám cầu trước cho tài/xỉu.",
                10 => "11) Ngẫu nhiên cho chẵn/lẻ.",
                11 => "12) Ngẫu nhiên cho tài/xỉu.",
                12 => "13) AI thống kê C/L.",
                13 => "14) AI thống kê T/X.",
                14 => "15) Xu hướng chuyển trạng thái C/L.",
                15 => "16) Xu hướng chuyển trạng thái T/X.",
                16 => "17) Run-length C/L.",
                17 => "18) Run-length T/X.",
                18 => "19) Ensemble C/L.",
                19 => "20) Ensemble T/X.",
                20 => "21) Lịch chẻ 10 tay C/L.",
                21 => "22) Lịch chẻ 10 tay T/X.",
                22 => "23) KNN chuỗi con C/L.",
                23 => "24) KNN chuỗi con T/X.",
                24 => "25) Lịch hai lớp C/L.",
                25 => "26) Lịch hai lớp T/X.",
                26 => "27) AI n-gram C/L.",
                27 => "28) AI n-gram T/X.",
                28 => "29) AI15 panel C/L.",
                29 => "30) AI15 panel T/X.",
                30 => "31) Top10 tích lũy C/L.",
                31 => "32) Top10 tích lũy T/X.",
                32 => "33) Chuỗi cầu C/L hay về.",
                33 => "34) Chuỗi cầu T/X hay về.",
                34 => "35) Đánh các cửa ăn nổ hũ.",
  ...
  try{
    ...
    Web != null && Web.CoreWebView2 != null;
        ...
        private sealed class CdpClickPoint
        {
        ...
        private sealed class CdpPoint2
        {
            public double x { get; set; }
            public double y { get; set; }
        }

        private sealed class CdpBetTargetPlan
        {
            ...
            public string pointFamily { get; set; } = "";
            public List<CdpClickPoint> points { get; set; } = new();
        }

        private sealed class CdpBetStepPlan
        {
            public long denom { get; set; }
            public int count { get; set; }
            public string nodeName { get; set; } = "";
            public CdpPoint2? raw { get; set; }
            public CdpPoint2? size { get; set; }
            public List<CdpClickPoint> points { get; set; } = new();
        }

        private sealed class CdpBetPlan
        {
            public bool ok { get; set; }
            public string error { get; set; } = "";
            public string side { get; set; } = "";
            public long amount { get; set; }
            public bool transformOk { get; set; }
            public double chipPitch { get; set; }
            public CdpBetTargetPlan betTarget { get; set; } = new();
            public List<CdpBetStepPlan> steps { get; set; } = new();
        }

        private sealed class CdpChipSelectionState
        {
            public long? value { get; set; }
            public bool anyToggle { get; set; }
            public string nodeName { get; set; } = "";
        }

        private sealed class CdpFocusAttemptResult
        {
        ...
        private sealed class CdpSideDeltaResult
        {
            public bool ok { get; set; }
            public double delta { get; set; }
            public string sideHit { get; set; } = "";
            public double sideHitDelta { get; set; }
        }

        private sealed class JsChipFocusResult
        {
            public bool ok { get; set; }
            public long amount { get; set; }
            public string nodeName { get; set; } = "";
            public string labelName { get; set; } = "";
            public bool touchOk { get; set; }
            public bool emitOk { get; set; }
            public bool rectOk { get; set; }
            public string error { get; set; } = "";
        }

        private async Task<T?> ExecJsJsonAsync<T>(string js)
        {
            var raw = await ExecJsAsyncStr(js);
            ...
            {
                Log("[CDP][JS-JSON] " + ex.Message);
                return default;
            }
        }

        private async Task<CdpBetPlan?> GetCdpBetPlanAsync(string side, long amount)
        {
            var js = "(function(){try{" +
                     "var fn=window.__cw_buildCdpBetPlan;" +
                     "var r=fn?fn(" + JsonSerializer.Serialize(side) + "," + amount + "):{ok:false,error:'missing___cw_buildCdpBetPlan'};" +
                     "return JSON.stringify(r);" +
                     "}catch(e){return JSON.stringify({ok:false,error:String(e&&e.message||e)})}})();";
            return await ExecJsJsonAsync<CdpBetPlan>(js);
        }

        private async Task<string> GetBetTargetDebugRawAsync(string side)
        {
            var js = "(function(){try{" +
                     "var fn=window.__cw_getBetTargetDebug;" +
                     "var r=fn?fn(" + JsonSerializer.Serialize(side) + "):{ok:false,error:'missing___cw_getBetTargetDebug'};" +
                     "return JSON.stringify(r);" +
                     "}catch(e){return JSON.stringify({ok:false,error:String(e&&e.message||e)})}})();";
            return await ExecJsAsyncStr(js);
        }

        private async Task<Dictionary<string, string>> GetChipDigestAsync()
        {
            var js = "(function(){try{return JSON.stringify((window.__cw_getChipStateDigest&&window.__cw_getChipStateDigest())||{});}catch(e){return '{}';}})();";
            return await ExecJsJsonAsync<Dictionary<string, string>>(js) ?? new Dictionary<string, string>();
        }

        private async Task<CdpChipSelectionState> GetChipSelectionForHostAsync()
        {
            var js = "(function(){try{return JSON.stringify((window.__cw_getChipSelectionForHost&&window.__cw_getChipSelectionForHost())||{});}catch(e){return '{}';}})();";
            return await ExecJsJsonAsync<CdpChipSelectionState>(js) ?? new CdpChipSelectionState();
        }

        private async Task<JsChipFocusResult> FocusChipViaJsAsync(long denom)
        {
            var js = "(function(){try{return JSON.stringify((window.__cw_focusChipForHost&&window.__cw_focusChipForHost(" + denom.ToString(CultureInfo.InvariantCulture) + "))||{});}catch(e){return JSON.stringify({ok:false,error:String(e&&e.message||e)})}})();";
            return await ExecJsJsonAsync<JsChipFocusResult>(js) ?? new JsChipFocusResult();
        }

        private async Task<Dictionary<string, double>> GetTotalsForHostAsync()
        {
            var js = "(function(){try{return JSON.stringify((window.__cw_getTotalsForHost&&window.__cw_getTotalsForHost())||{});}catch(e){return '{}';}})();";
            return await ExecJsJsonAsync<Dictionary<string, double>>(js) ?? new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
        }

        private static bool HasChipDigestChanged(Dictionary<string, string>? before, Dictionary<string, string>? after, long denom)
        {
            var key = denom.ToString(CultureInfo.InvariantCulture);
            var b = (before != null && before.TryGetValue(key, out var bv)) ? (bv ?? "") : "";
            var a = (after != null && after.TryGetValue(key, out var av)) ? (av ?? "") : "";
            return !string.Equals(a, b, StringComparison.Ordinal);
        }

        private static string GetChipDigestValue(Dictionary<string, string>? digest, long denom)
        {
            var key = denom.ToString(CultureInfo.InvariantCulture);
            return (digest != null && digest.TryGetValue(key, out var v)) ? (v ?? "") : "";
        }

        private static string ShortDigest(string? s, int maxLen = 140)
        {
            s ??= "";
        ...
        private static string FormatChipSelection(CdpChipSelectionState? state)
        {
            if (state == null)
                return "null";
            var value = state.value.HasValue ? state.value.Value.ToString(CultureInfo.InvariantCulture) : "null";
            return $"value={value},toggle={(state.anyToggle ? 1 : 0)},node={state.nodeName}";
        }

        private static string FormatPoint(CdpClickPoint pt)
        {
            return $"{pt.tag}@({pt.x:0.0},{pt.y:0.0})";
        }

        private static string FormatPointList(IEnumerable<CdpClickPoint>? points, int take = 8)
        {
            if (points == null) return "";
            return string.Join("; ", points.Take(take).Select(FormatPoint));
        }

        private static string FindChangedChipKeys(Dictionary<string, string>? before, Dictionary<string, string>? after)
        {
            ...
            {
                var b = (before != null && before.TryGetValue(k, out var bv)) ? (bv ?? "") : "";
                var a = (after != null && after.TryGetValue(k, out var av)) ? (av ?? "") : "";
        ...
        private static double GetSideTotal(Dictionary<string, double>? totals, string side)
        {
            if (totals == null || totals.Count == 0)
                return 0;
            var want = (side ?? "").Trim().ToUpperInvariant();
            foreach (var kv in totals)
            {
                if (string.Equals((kv.Key ?? "").Trim(), want, StringComparison.OrdinalIgnoreCase))
                    return kv.Value;
            }
            return 0;
        }

        private static CdpSideDeltaResult ComputeSideDeltaResult(string targetSide, Dictionary<string, double>? before, Dictionary<string, double>? after, long expectedDelta)
        {
            var sides = new[] { "TAI", "XIU", "CHAN", "LE" };
            var target = (targetSide ?? "").Trim().ToUpperInvariant();
            var targetDelta = GetSideTotal(after, target) - GetSideTotal(before, target);
            if (targetDelta > 0.5)
            {
                return new CdpSideDeltaResult
                {
                    ok = Math.Abs(targetDelta - expectedDelta) <= 1.0,
                    delta = targetDelta,
                    sideHit = target,
                    sideHitDelta = targetDelta
                };
            }
            ...
            {
                var delta = GetSideTotal(after, side) - GetSideTotal(before, side);
                if (delta > bestDelta + 0.5)
                {
                    bestDelta = delta;
                    bestSide = side;
                }
            ...
            return new CdpSideDeltaResult
            {
                ...
                delta = 0,
                sideHit = bestSide,
                sideHitDelta = bestDelta
            };
        }

        private async Task<bool> DispatchCdpMouseClickAsync(double x, double y)
        {
            if (Web?.CoreWebView2 == null)
                return false;
            ...
            try
            {
                await Web.CoreWebView2.CallDevToolsProtocolMethodAsync("Input.dispatchMouseEvent", move);
                await Web.CoreWebView2.CallDevToolsProtocolMethodAsync("Input.dispatchMouseEvent", down);
                await Web.CoreWebView2.CallDevToolsProtocolMethodAsync("Input.dispatchMouseEvent", up);
                return true;
            }
            catch (Exception ex)
            {
                Log("[CDP][CLICK] " + ex.Message);
                return false;
            }
        }

        private async Task<CdpSideDeltaResult> WaitForSideDeltaAsync(string side, Dictionary<string, double> before, long expectedDelta, int timeoutMs)
        {
            var sw = Stopwatch.StartNew();
            while (sw.ElapsedMilliseconds < timeoutMs)
            {
                await Task.Delay(120);
                var cur = await GetTotalsForHostAsync();
                var res = ComputeSideDeltaResult(side, before, cur, expectedDelta);
                if (res.delta > 0.5 || res.sideHitDelta > 0.5)
                    return res;
            }
            return new CdpSideDeltaResult();
        }

        private async Task<CdpFocusAttemptResult> TryFocusChipViaCdpAsync(CdpBetStepPlan step)
        {
            if (step.points == null || step.points.Count == 0)
            {
                return new CdpFocusAttemptResult
                {
                    canProceed = false,
                    verified = false,
                    mode = "no-points",
                    detail = "focus point list empty"
                };
            }

            var before = await GetChipDigestAsync();
            var beforeSel = await GetChipSelectionForHostAsync();
            var beforeValue = GetChipDigestValue(before, step.denom);
            Log($"[CDP][FOCUS] start denom={step.denom} node={step.nodeName} raw=({step.raw?.x:0.0},{step.raw?.y:0.0}) size=({step.size?.x:0.0},{step.size?.y:0.0}) points={FormatPointList(step.points, 20)} before={ShortDigest(beforeValue)} sel={FormatChipSelection(beforeSel)}");

            var jsFocus = await FocusChipViaJsAsync(step.denom);
            Log($"[JS-FOCUS] denom={step.denom} ok={jsFocus.ok} node={jsFocus.nodeName} label={jsFocus.labelName} touchOk={jsFocus.touchOk} emitOk={jsFocus.emitOk} rectOk={jsFocus.rectOk} error={jsFocus.error}");
            if (jsFocus.ok)
            {
                await Task.Delay(120);
                var afterJs = await GetChipDigestAsync();
                var afterJsSel = await GetChipSelectionForHostAsync();
                var afterJsValue = GetChipDigestValue(afterJs, step.denom);
                var changedJs = HasChipDigestChanged(before, afterJs, step.denom);
                var changedJsKeys = FindChangedChipKeys(before, afterJs);
                var toggleExactJs = afterJsSel.anyToggle && afterJsSel.value == step.denom;
                var verifyByDigestJs = !afterJsSel.anyToggle && (changedJs || !string.IsNullOrWhiteSpace(changedJsKeys));
                Log($"[JS-FOCUS] denom={step.denom} toggleExact={toggleExactJs} sel={FormatChipSelection(afterJsSel)} changed={changedJs} changedKeys={changedJsKeys} before={ShortDigest(beforeValue)} after={ShortDigest(afterJsValue)}");
                if (toggleExactJs || verifyByDigestJs)
                {
                    var viaJs = toggleExactJs ? "js-toggle" : "js-digest";
                    Log($"[CDP][FOCUS] denom={step.denom} node={step.nodeName} ok via={viaJs} changedKeys={changedJsKeys}");
                    return new CdpFocusAttemptResult
                    {
                        canProceed = true,
                        verified = true,
                        mode = viaJs,
                        detail = $"node={jsFocus.nodeName};changedKeys={changedJsKeys}"
                    };
                }

                Log($"[CDP][FOCUS] denom={step.denom} js-chip-node dispatched; totals-delta will decide");
                return new CdpFocusAttemptResult
                {
                    canProceed = true,
                    verified = false,
                    mode = "js-chip-node",
                    detail = $"node={jsFocus.nodeName};touch={jsFocus.touchOk};emit={jsFocus.emitOk};rect={jsFocus.rectOk}"
                };
            }

            var tryIndex = 0;
            foreach (var pt in step.points)
            {
                tryIndex++;
                Log($"[CDP][FOCUS][TRY] denom={step.denom} idx={tryIndex}/{step.points.Count} point={FormatPoint(pt)}");
                if (!await DispatchCdpMouseClickAsync(pt.x, pt.y))
                {
                    Log($"[CDP][FOCUS][TRY] denom={step.denom} idx={tryIndex} dispatch_failed");
                    continue;
                }
                await Task.Delay(110);
                var after = await GetChipDigestAsync();
                var afterSel = await GetChipSelectionForHostAsync();
                var afterValue = GetChipDigestValue(after, step.denom);
                var changed = HasChipDigestChanged(before, after, step.denom);
                var changedKeys = FindChangedChipKeys(before, after);
                var changedAny = !string.IsNullOrWhiteSpace(changedKeys);
                var toggleExact = afterSel.anyToggle && afterSel.value == step.denom;
                var verifyByDigest = !afterSel.anyToggle && (changed || changedAny);
                Log($"[CDP][FOCUS][TRY] denom={step.denom} idx={tryIndex} toggleExact={toggleExact} sel={FormatChipSelection(afterSel)} changed={changed} changedKeys={changedKeys} before={ShortDigest(beforeValue)} after={ShortDigest(afterValue)}");
                if (toggleExact || verifyByDigest)
                {
                    var via = toggleExact ? "toggle" : "digest";
                    Log($"[CDP][FOCUS] denom={step.denom} node={step.nodeName} tag={pt.tag} ok via={via} changedKeys={changedKeys}");
                    return new CdpFocusAttemptResult
                    {
                        canProceed = true,
                        verified = true,
                        mode = via,
                        detail = $"tag={pt.tag};changedKeys={changedKeys}"
                    };
                }
            }
            ...
            if (step.denom == 1000)
            {
                Log("[CDP][FOCUS] denom=1000 not verified, allow default selection");
                return new CdpFocusAttemptResult
                {
                    canProceed = true,
                    verified = false,
                    mode = "default-1000",
                    detail = "allow default selection"
                };
            }

            Log($"[CDP][FOCUS] denom={step.denom} verify failed final={ShortDigest(beforeValue)} sel={FormatChipSelection(beforeSel)}");
            Log($"[CDP][FOCUS] denom={step.denom} unverified-proceed totals-delta will decide");
            return new CdpFocusAttemptResult
            {
                canProceed = true,
                verified = false,
                mode = "unverified-proceed",
                detail = $"finalSel={FormatChipSelection(beforeSel)}"
            };
        }

        private async Task<bool> PlaceBetViaCdpAsync(string side, long amount, int roundId, string tabId)
        {
            await EnsureWebReadyAsync();
            if (Web?.CoreWebView2 == null)
                return false;

            var plan = await GetCdpBetPlanAsync(side, amount);
            if (plan == null)
            {
                Log("[CDP][PLAN] empty");
                return false;
            ...
            if (!plan.ok)
            {
                Log("[CDP][PLAN] fail: " + (plan.error ?? "unknown"));
                if (string.Equals(plan.error, "bet_target_not_found", StringComparison.OrdinalIgnoreCase))
                {
                    var debugRaw = await GetBetTargetDebugRawAsync(side);
                    if (!string.IsNullOrWhiteSpace(debugRaw))
                        Log("[CDP][PLAN][BET-DEBUG] " + ShortDigest(debugRaw, 4200));
                }
                return false;
            }
            if (plan.betTarget == null || plan.betTarget.points == null || plan.betTarget.points.Count == 0)
            {
                Log("[CDP][PLAN] missing bet target points");
                return false;
            }
            if (plan.steps == null || plan.steps.Count == 0)
            {
                Log("[CDP][PLAN] no chip steps");
                return false;
            }

            var needBetTraceLog =
                !plan.transformOk ||
                string.IsNullOrWhiteSpace(plan.betTarget?.sourceTag) ||
                !plan.betTarget.sourceTag.StartsWith("near-", StringComparison.OrdinalIgnoreCase);
            if (needBetTraceLog)
            {
                var debugRaw = await GetBetTargetDebugRawAsync(side);
                if (!string.IsNullOrWhiteSpace(debugRaw))
                    Log("[CDP][PLAN][BET-TRACE] " + ShortDigest(debugRaw, 2600));
            }

            Log($"[CDP][PLAN] side={plan.side} amount={plan.amount} transformOk={plan.transformOk} chipPitch={plan.chipPitch:0.0} steps={plan.steps.Count} betNode={plan.betTarget.nodeName} betSource={plan.betTarget.sourceTag} pointFamily={plan.betTarget.pointFamily} selector={plan.betTarget.selectorPath} board={plan.betTarget.boardPath} target={FormatPointList(plan.betTarget.points)}");
            foreach (var s in plan.steps)
            {
                Log($"[CDP][PLAN][STEP] denom={s.denom} count={s.count} node={s.nodeName} raw=({s.raw?.x:0.0},{s.raw?.y:0.0}) size=({s.size?.x:0.0},{s.size?.y:0.0}) points={FormatPointList(s.points)}");
            }
            ...
            for (var si = 0; si < plan.steps.Count; si++)
            {
                ...
                if (step.count <= 0)
                    continue;
                var focusRes = await TryFocusChipViaCdpAsync(step);
                if (!focusRes.canProceed)
                    return false;

                for (var turn = 0; turn < step.count; turn++)
                {
                    var before = await GetTotalsForHostAsync();
                    var dispatchAny = false;
                    var betOk = false;
                    for (var bi = 0; bi < plan.betTarget.points.Count; bi++)
                    {
                        var pt = plan.betTarget.points[bi];
                        Log($"[CDP][BET][TRY] side={side} denom={step.denom} turn={turn + 1}/{step.count} idx={bi + 1}/{plan.betTarget.points.Count} point={FormatPoint(pt)} focusVerified={(focusRes.verified ? 1 : 0)} focusMode={focusRes.mode}");
                        if (!await DispatchCdpMouseClickAsync(pt.x, pt.y))
                        {
                            Log($"[CDP][BET][TRY] side={side} denom={step.denom} turn={turn + 1}/{step.count} idx={bi + 1} dispatch_failed");
                            continue;
                        }
                        dispatchAny = true;
                        var deltaRes = await WaitForSideDeltaAsync(side, before, step.denom, 1700);
                        Log($"[CDP][BET][TRY] side={side} denom={step.denom} turn={turn + 1}/{step.count} idx={bi + 1} delta={deltaRes.delta} ok={(deltaRes.ok ? 1 : 0)} hitSide={deltaRes.sideHit} hitDelta={deltaRes.sideHitDelta}");
                        if (deltaRes.ok)
                        {
                            Log($"[CDP][BET] ok side={side} denom={step.denom} delta={deltaRes.delta} turn={turn + 1}/{step.count} focusVerified={(focusRes.verified ? 1 : 0)} focusMode={focusRes.mode} point={FormatPoint(pt)}");
                            betOk = true;
                            break;
                        }
                        if (deltaRes.sideHitDelta > 0.5)
                        {
                            Log($"[CDP][BET] verify failed side={side} denom={step.denom} delta={deltaRes.delta} hitSide={deltaRes.sideHit} hitDelta={deltaRes.sideHitDelta} focusVerified={(focusRes.verified ? 1 : 0)} focusMode={focusRes.mode} point={FormatPoint(pt)}");
                            return false;
                        }
                    }
                    if (!dispatchAny)
                    {
                        Log($"[CDP][BET] target click failed side={side} denom={step.denom} turn={turn + 1}");
                        return false;
                    }
                    if (!betOk)
                    {
                        Log($"[CDP][BET] verify failed side={side} denom={step.denom} delta=0 focusVerified={(focusRes.verified ? 1 : 0)} focusMode={focusRes.mode} tried={plan.betTarget.points.Count}");
                        return false;
                    }
                    await Task.Delay(90);
                }
...
";
        ...
        private void UpdateBetStrategyUi()
        {
            try
            {
                var idx = CmbBetStrategy?.SelectedIndex ?? 8;
                if (RowChuoiCau != null)
                    RowChuoiCau.Visibility = (idx == 0 || idx == 1 || idx == 4 || idx == 5) ? Visibility.Visible : Visibility.Collapsed;
                if (RowTheCau != null)
                    RowTheCau.Visibility = (idx == 2 || idx == 3 || idx == 6 || idx == 7) ? Visibility.Visible : Visibility.Collapsed;
                if (RowSideRatio != null)
                    RowSideRatio.Visibility = (idx == 34) ? Visibility.Visible : Visibility.Collapsed;
            }
            catch { }
        }
        ...
        async void CmbBetStrategy_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (!_uiReady || _tabSwitching) return;
            _cfg.BetStrategyIndex = CmbBetStrategy?.SelectedIndex ?? 8;
            await SaveConfigAsync();
        }
        private GameContext BuildContext(StrategyTabState tab, bool useRawWinAmount = false)
        {
            ...
            return new GameContext
            {
                ...
                PlaceBetCdpAsync = (side, amount, roundId, tabId) => Dispatcher.InvokeAsync(() => PlaceBetViaCdpAsync(side, amount, roundId, tabId)).Task.Unwrap(),
                Log = (s) => Log(s),
        ...
        private async void PlayXocDia_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                TaiXiuLiveSun.Tasks.IBetTask task = _cfg.BetStrategyIndex switch
                {
                    ...
                    1 => new TaiXiuLiveSun.Tasks.SeqTxFollowTask(), // 2
                    2 => new TaiXiuLiveSun.Tasks.PatternParityTask(), // 3
                    3 => new TaiXiuLiveSun.Tasks.PatternTxTask(), // 4
                    4 => new TaiXiuLiveSun.Tasks.SeqMajorMinorTask(), // 5
                    5 => new TaiXiuLiveSun.Tasks.SeqMajorMinorTxTask(), // 6
                    6 => new TaiXiuLiveSun.Tasks.PatternMajorMinorTask(), // 7
                    7 => new TaiXiuLiveSun.Tasks.PatternMajorMinorTxTask(), // 8
                    8 => new TaiXiuLiveSun.Tasks.SmartPrevTask(), // 9
                    9 => new TaiXiuLiveSun.Tasks.SmartPrevTxTask(), // 10
                    10 => new TaiXiuLiveSun.Tasks.RandomParityTask(), // 11
                    11 => new TaiXiuLiveSun.Tasks.RandomTxTask(), // 12
                    12 => new TaiXiuLiveSun.Tasks.AiStatParityTask(), // 13
                    13 => new TaiXiuLiveSun.Tasks.AiStatTxTask(), // 14
                    14 => new TaiXiuLiveSun.Tasks.StateTransitionBiasTask(), // 15
                    15 => new TaiXiuLiveSun.Tasks.StateTransitionBiasTxTask(), // 16
                    16 => new TaiXiuLiveSun.Tasks.RunLengthBiasTask(), // 17
                    17 => new TaiXiuLiveSun.Tasks.RunLengthBiasTxTask(), // 18
                    18 => new TaiXiuLiveSun.Tasks.EnsembleMajorityTask(), // 19
                    19 => new TaiXiuLiveSun.Tasks.EnsembleMajorityTxTask(), // 20
                    20 => new TaiXiuLiveSun.Tasks.TimeSlicedHedgeTask(), // 21
                    21 => new TaiXiuLiveSun.Tasks.TimeSlicedHedgeTxTask(), // 22
                    22 => new TaiXiuLiveSun.Tasks.KnnSubsequenceTask(), // 23
                    23 => new TaiXiuLiveSun.Tasks.KnnSubsequenceTxTask(), // 24
                    24 => new TaiXiuLiveSun.Tasks.DualScheduleHedgeTask(), // 25
                    25 => new TaiXiuLiveSun.Tasks.DualScheduleHedgeTxTask(), // 26
                    26 => new TaiXiuLiveSun.Tasks.AiOnlineNGramTask(GetAiNGramStatePath()), // 27
                    27 => new TaiXiuLiveSun.Tasks.AiOnlineNGramTxTask(GetAiNGramTxStatePath()), // 28
                    28 => new TaiXiuLiveSun.Tasks.AiExpertPanelTask(), // 29
                    29 => new TaiXiuLiveSun.Tasks.AiExpertPanelTxTask(), // 30
                    30 => new TaiXiuLiveSun.Tasks.Top10PatternFollowTask(), // 31
                    31 => new TaiXiuLiveSun.Tasks.Top10PatternFollowTxTask(), // 32
                    32 => new TaiXiuLiveSun.Tasks.SeqParityHotBackTask(), // 33
                    33 => new TaiXiuLiveSun.Tasks.SeqTxHotBackTask(), // 34
                    34 => new TaiXiuLiveSun.Tasks.JackpotMultiSideTask(), // 35
  ...
  try{
    ...
    window.__abx_lockMouse = function(on){
      try{ return on ? add() : remove(); } catch(e){ return 'err:'+e; }
        ...
        private void InitResultDigitIcons()
        {
            if (_resultDigitIconMap.Count > 0)
                return;

            string asm = GetType().Assembly.GetName().Name!;

            var i0 = FallbackIcons.LoadPackImage("Assets/side/XIUCHAN.png") ?? LoadImgSafe(
                $"pack://application:,,,/{asm};component/Assets/side/XIUCHAN.png",
                "pack://application:,,,/Assets/side/XIUCHAN.png",
                "pack://application:,/Assets/side/XIUCHAN.png"
            );
            if (i0 != null) _resultDigitIconMap['0'] = i0;

            var i1 = FallbackIcons.LoadPackImage("Assets/side/XIULE.png") ?? LoadImgSafe(
                $"pack://application:,,,/{asm};component/Assets/side/XIULE.png",
                "pack://application:,,,/Assets/side/XIULE.png",
                "pack://application:,/Assets/side/XIULE.png"
            );
            ...
            var i3 = FallbackIcons.LoadPackImage("Assets/side/TAILE.png") ?? LoadImgSafe(
                $"pack://application:,,,/{asm};component/Assets/side/TAILE.png",
        ...
        private void SetLastResultUI(string? result)
        {
            ...
            if (digit != '\0')
            {
                if (_resultDigitIconMap.TryGetValue(digit, out var digitIcon) && digitIcon != null && ImgKetQua != null)
                {
                    ImgKetQua.Source = digitIcon;
                    ImgKetQua.Visibility = Visibility.Visible;
                    if (LblKetQua != null) LblKetQua.Visibility = Visibility.Collapsed;
                    return;
                }

                // Nếu thiếu ảnh map số thì fallback chữ số để dễ debug.
                ShowText(digit.ToString());
                return;
            }
        ...
        private void SetLastSideUI(string? result)
        {
            ...
            bool isTai = s == "TAI" || s == "T";
            bool isXiu = s == "XIU" || s == "X";
        ...
        private static string NormalizeSide(string s)
        {
            ...
            if (u == "T" || u == "TAI" || u == "BIG") return "TAI";
            if (u == "X" || u == "XIU" || u == "SMALL") return "XIU";
            return (s ?? "").Trim();
        }
    ...
    TextNorm.U(Regex.Replace(raw ?? "", @"[,\s\-]+", "")); // bỏ , khoảng trắng, -
        ...
        // --- Chuỗi T/X: T,X hoặc số 0..6 (0,1=>X; 2..6=>T); 2..100 ký tự ---
        private static bool ValidateSeqTX(string s, out string err)
        {
            err = "";
            if (string.IsNullOrWhiteSpace(s))
            {
            ...
            foreach (var ch in s)
            {
                ...
                if (TaiXiuLiveSun.Tasks.TaskUtil.DigitToTx(ch) != '\0') { count++; continue; }
                err = "Chỉ cho phép khoảng trắng và ký tự T/X hoặc số 0..6.";
                return false;
            }

            if (count < 2 || count > 100)
            {
                err = "Độ dài 2–100 ký tự (tính theo T/X, bỏ qua khoảng trắng).";
                return false;
            }
        ...
        // --- Thế cầu T/X: từng dòng "<mẫu> - <đặt>", mẫu gồm T/X hoặc số 0..6 ---
        private static bool ValidatePatternsTX(string s, out string err)
        {
            ...
            foreach (var raw in rules)
            {
                ...
                var m = System.Text.RegularExpressions.Regex.Match(
                    line,
                    @"^\s*([TXtx0-6\s]+)\s*(?:->|-)\s*([TXtx0-6\s]+)\s*$",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase);

                if (!m.Success)
                {
                    err = $"Quy tắc {idx} không hợp lệ: “{line}”. Dạng đúng: <mẫu> -> <chuỗi cầu> hoặc <mẫu>-<chuỗi cầu>; chỉ dùng T/X hoặc 0..6.";
                    return false;
                }
                ...
                {
                    if (char.IsWhiteSpace(ch)) continue;
                    char tx = TaiXiuLiveSun.Tasks.TaskUtil.DigitToTx(ch);
                    if (tx == 'T' || tx == 'X') lhsBuf.Append(tx);
                ...
                foreach (char ch in rhsRaw)
                {
                    if (char.IsWhiteSpace(ch)) continue;
                    char tx = TaiXiuLiveSun.Tasks.TaskUtil.DigitToTx(ch);
                    if (tx == 'T' || tx == 'X') rhsBuf.Append(tx);
                    else { err = $"Quy tắc {idx}: <chuỗi cầu> chỉ gồm T/X hoặc 0..6."; return false; }
                }
                if (rhsBuf.Length < 1)
                {
        ...
        private bool ValidateInputsForCurrentStrategy()
        {
            ...
            else if (idx == 1) // 2. Chuỗi T/X
            {
                if (!ValidateSeqTX(T(TxtChuoiCau), out var err))
                {
                    SetError(LblSeqError, err);
                    BringBelow(TxtChuoiCau);
                    return false;
            ...
            else if (idx == 4 || idx == 5) // 5,6. Chuỗi I/N
            {
            ...
            else if (idx == 6 || idx == 7) // 7,8. Thế I/N
            {
            ...
            else if (idx == 34) // 35. Cửa đặt & tỉ lệ
            {
        ...
        private void SyncStrategyFieldsToUI()
        {
            ...
            else if (idx == 1) { if (TxtChuoiCau != null) TxtChuoiCau.Text = _cfg.BetSeqTX ?? ""; }
            else if (idx == 4) { if (TxtChuoiCau != null) TxtChuoiCau.Text = _cfg.BetSeqNI ?? ""; }
            else if (idx == 5) { if (TxtChuoiCau != null) TxtChuoiCau.Text = _cfg.BetSeqNITX ?? ""; }

            if (idx == 2) { if (TxtTheCau != null) TxtTheCau.Text = _cfg.BetPatternsCL ?? ""; }
            else if (idx == 3) { if (TxtTheCau != null) TxtTheCau.Text = _cfg.BetPatternsTX ?? ""; }
            else if (idx == 6) { if (TxtTheCau != null) TxtTheCau.Text = _cfg.BetPatternsNI ?? ""; }
            else if (idx == 7) { if (TxtTheCau != null) TxtTheCau.Text = _cfg.BetPatternsNITX ?? ""; }
            if (idx == 34 && TxtSideRatio != null) TxtSideRatio.Text = _cfg.SideRateText ?? TaiXiuLiveSun.Tasks.SideRateParser.DefaultText;
        }
        ...
        private async void TxtChuoiCau_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            if (idx == 1) _cfg.BetSeqTX = txt;    // Chiến lược 2: Chuỗi T/X
            if (idx == 4) _cfg.BetSeqNI = txt;    // Chiến lược 5: Chuỗi N/I CL
            if (idx == 5) _cfg.BetSeqNITX = txt;  // Chiến lược 6: Chuỗi N/I TX
        ...
        private async void TxtTheCau_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            // Lưu tách bạch cho từng chiến lược
            if (idx == 2) _cfg.BetPatternsCL = txt;    // Chiến lược 3: Thế C/L
            if (idx == 3) _cfg.BetPatternsTX = txt;    // Chiến lược 4: Thế T/X
            if (idx == 6) _cfg.BetPatternsNI = txt;    // Chiến lược 7: Thế N/I CL
            if (idx == 7) _cfg.BetPatternsNITX = txt;  // Chiến lược 8: Thế N/I TX
        ...
        private void ShowErrorsForCurrentStrategy()
        {
            ...
            if (idx == 0 || idx == 1 || idx == 4 || idx == 5)
            {
                ...
                bool ok;
                string? e1;
                if (idx == 0) ok = ValidateSeqCL(s, out e1);
                else if (idx == 1) ok = ValidateSeqTX(s, out e1);
                else ok = ValidateSeqNI(s, out e1);
            ...
            if (idx == 2 || idx == 3 || idx == 6 || idx == 7)
            {
                ...
                bool ok;
                string? e2;
                if (idx == 2) ok = ValidatePatternsCL(s, out e2);
                else if (idx == 3) ok = ValidatePatternsTX(s, out e2);
                else ok = ValidatePatternsNI(s, out e2);
