...
export type ScanRun = {id:string;profile_name:string;mode_requested:string;mode_validated:string;status:string;current_step:string;progress:number;counters:Record<string,number>;results:any;validation:any;error_message:string;started_at:string|null;finished_at:string|null;created_at:string;steps:ScanStep[];candidates:Candidate[]}
export type Notification = {id:number;level:string;title:string;message:string;created_at:string}
export type DashboardData = {profile:Profile|null;latest_run:ScanRun|null;latest_successful_run:ScanRun|null;notifications:Notification[]}
export type HealthStatus = {status:string;service:string;time:string}
